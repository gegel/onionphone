...
\end{itemize}
OnionPhone provides \emph{Perfect Forward Secrecy} (uses a fresh key for each call) and \linebreak \emph{Full  Deniability} for using long-term public keys of participants (as a fact and a content of the conversation): like a deniable SKEME \cite{RGK06} protocol uses for initial key exchange.
...
The OnionPhone source (available on \textsf{github}:\cite{Gegel141}):
\begin{itemize}
...
\item \hspace*{2mm}\emph{Step 2:} Edit the TorChat configuration file  \textsf{/torchat/bin/Tor/torrc.txt}:
...
\item \hspace*{2mm}\emph{Step 3:}  Right click on \textsf{myself}  icon of TorChat contact list and copy ID to clipboard. Edit the OnionPhone configuration file \textsf{conf.txt}: specify our Onion address using copied ID, for example:
\begin{verbatim}
...
\subsection{Usage}
\begin{itemize}
\item To accept an incoming call press \textsf{Enter}.
\item To make an outgoing call as a guest to guest (without use of personal public keys) type command:  \texttt{-Oremote\_onion\_address} and press \textsf{Enter} then wait 10-30 sec for connection over Tor to be established.
\item To toggle continuous voice transmission press \textsf{Enter}. Hold down / release the \textsf{Tab}  for Push-to-Talk mode.
\item To apply the voice codec from 1 to 18 use the command  \texttt{-Ccodec\_number}. Smaller numbers correspond to low bitrate codecs, greater ones are for high quality. Numbers from 16 to 18 correspond to the variable bit rate codecs.
\item To enable security vocoder use the command \texttt{-Qmode}, where \textsf{mode=3} corresponds to  ``whisper"  voice (recommended), modes 6-255 correspond to ``robot" etc. To deactivate the vocoder use the command \texttt{-Q-3}.
\item To use the chat feature  type a message and send it by pressing \textsf{Enter}.
\item To switch to direct UDP connection use the command \texttt{-S} (both parties must do this).
\item To return into Tor from direct UDP connection use the command  \texttt{-O}.
\item To end the call use the command \texttt{-H}.
\item To exit the OnionPhone use the command \texttt{-X} or hit \textsf{Esc} twice for emergency exit.
\end{itemize}
...
\end{itemize}
\section{Menu}
Console menu allows to quickly enter commands which are most often performed by the user. The menu consists of 9 sections. Navigation between sections is performed using the \textsf{Up} and \textsf{Down} keys. Selecting an item in the section is done using \textsf{Left} and \textsf{Right} keys. The selected item is applied using the \textsf{Enter} key. Menu sections are:
\begin{enumerate}
...
\end{enumerate}
\section{Commands}
The command should start with a ``\textbf{-}" symbol followed by a capital letter followed by an optional parameter. No spaces can be inserted between letter and parameter. Some commands can contain several subcommands separated by spaces. After typing the commands press \textsf{Enter} for processing. While typing use \textsf{Back} to correct the typed characters or \textsf{Del} to cancel and clear the command string.
\subsection{Address book management}

\texttt{-V[filter] }displays all the entries of the address book containing the substring \textsf{filter}. Displays all uncommented entries if the parameter is omitted.

\noindent \texttt{-E[name]} outputs the command to call to the subscriber \textsf{name} defined in the address book. Outputs the most recently used call command (redials) if the parameter is omitted.

\subsection{Application and Connection management}
\texttt{-H} gracefully terminates the call.

\noindent  \texttt{-X} gracefully shuts down the program.
\subsection{Sound management}

\texttt{-C[codec] }sets the codec for outgoing voice stream. Parameter \textsf{codec} is codec number from 1 to 18. If the parameter is ``?", the commands shows the number of the current codec. If omitted, the default codec is applied.

\noindent \texttt{-J[jitter]} sets a fixed size of buffer to compensate transport jitter. Parameter \textsf{jitter} is average buffering delay in milliseconds. If you use the parameter=``?" the current value is displayed.  If it's ``-1" the minimal buffering is applied (lower latency but bad resistance to jitter).  If the parameter is omitted the automatic buffering adaptation to the communication channel will be used (default).

\noindent \texttt{-Q[voice]} sets the style of  voice preprocessing. Parameter \textsf{voice} is:  ``0"-no automatic microphone gain control (AGC) and no noise suppressing (NPP), ``1"-applies AGC, ``2"-applies NPP, ``3" - applies a vocoder in ``whisper" mode, ``4" - in ``high" mode, ``5"- in ``deep" mode,  ``6"-``255" -  in ``robot" mode with different pitch, ``-1"-disables AGC, ``-2"-disables NPP, ``-3"  - disables the  vocoder (unchanged voice will be sent), ``?" - show current state of voice processing.

\subsection{Key sharing}

\texttt{-K[name]} sends specified public key \textsf{name} from the our phone book to remote side. This command is only possible if the connection is active. If parameter is omitted own public key specified in the configuration file is sent. The keys are received and automatically added to the address book with option \texttt{-L}  without parameter (untrusted). The user can view the address book and check out all the untrusted keys later (check the PGP signature into key file) and edit the trust level to that key.
\subsection{Authentication}


\texttt{-Yaccess} specifies password for private key correspondent for \textsf{our\_name} used for processing all incoming and default outgoing calls. If private key is in encrypted format then this option must be applied immediately after OnionPhone is started otherwise you can not accept incoming calls.  If the parameter is space applied  key access will be cleared. If the parameter is omitted applied key access will be checked for validity for specified secret key corresponding to our default name.

The alternative way to introduce access is to specify the password as \textsf{Our\_secret\_access} parameter in the configuration file. In this case it is recommended to store OnionPhone folder  in a protected place (TrueCrypt container etc.). The access also can be temporarily applied to the current outgoing call using \texttt{-Yaccess} into the  call extended command \texttt{-N} (see below).

\noindent \texttt{-P[password]} applies a common password for authentication and initiates it. Passwords can contain from 3 to 31 characters and must be the same for both parties except for the last two characters: one of the participants swaps their places. The password can be used both during the established connection and when dialing (see the extended command for calls ), as well as defined for each user in the address book (in this case it will be applied automatically when an incoming call from that person occurred). If parameter omitted applied password will be cleared.
...
After a successful switch to direct UDP-connection the Onion connection is still available but not actually used for communication . At any time any user can stop the direct connection and return to the use of the Onion connection. To do this at least one of the participants must execute command \texttt{-O} with no parameter.

\subsection{Extended call command}

\texttt{-N[name] destination\_subcommad [options\_subcommands]} initiates a connection with the subscriber \textsf{name} (the subscriber’s public key must be added  to the address book). If the name is omitted the default \textsf{guest} will be used. Public and private keys for the guest included in the OnionPhone by default and are available for each user (and also for the attacker). The authentication using \textsf{guest }key is not performed  and the danger of a MitM-attack is present in the case of initiation of direct connection. In the case of  connection over Tor the key agreement is going on inside Tor so MitM is likely impossible (while you trust Tor).

Parameter \textsf{destination\_command} specifies the type of transport (UDP, TCP, Onion) and the address of the recipient (IP or hostname and optionally port number separated by ``:"), for example:
\begin{verbatim}
...
-Or4kxspnzpnsel4fu
\end{verbatim}
Connection type \texttt{-U} initiates direct UDP-connection, \texttt{-T} for direct TCP-connection, \texttt{-O} for TCP-connection over Tor. Address can be specified as a string IP, domain name or Onion address  with or without the suffix $.onion$. The port can be specified after the address separated by a colon. If no port is specified the default port  17447 will be used.

Also, these commands can be used independently to install unauthenticated connections \textsf{guest}  to \textsf{guest}.

Subcommands  \texttt{-P[password]} (see above) and \texttt{-I[our\_name]} can follow, separated by spaces:
\textsf{our\_name} parameter specifies how the name of the originator should appear to the recipient. If this option is omitted \textsf{Our\_name} from the configuration file is sent instead. If \texttt{-I }option is used without a parameter the default name \textsf{guest} will be used (see above). If the parameter is specified it will be used as a name for the current session. In this case the initiator should have files of both public and private keys for this name. In addition this public key must be added to the address book. Of course the recipient also must  have a public key for this name and this key must be added in their address book otherwise the connection will not be established.

Option \texttt{-Yaccess} can  follow specifying the password for the secret key \textsf{our\_name}. Note that this password temporary (only for this call) overwrites  access to default private key used for incoming calls.
If both parties connect for the first time, the only key they have in common is probably the guest key. Thus the first connection must be initiated as untrusted \textsf{guest} to \textsf{guest}:
\begin{verbatim}
-N -I -Oonion_address
\end{verbatim}
This connection is still encrypted with the session key but its resistance to MitM depends fully on Tor. Also the parties can't be sure of the authenticity of each other (except for the possible verification Onion addresses similar to TorChat). Preinstalled untrusted connection can share PGP-signed long-term public keys using \texttt{-K} command (this is denied because keys are public and attacker can't be sure that customers really declare own key: user can also send multiple keys of other various users) . Received keys will be automatically added to the address books and marked as untrusted. After manually checking the PGP signatures users can set the desired level of trust by editing contacts entries in the address book file. Later user can re-initialize the connection to be fully trusted, for example:
\begin{verbatim}
-Nalice -Or4kxspnzpnsel4fu -Ibob
\end{verbatim}\subsection{Others commands}
\texttt{-RI} command runs voice self-test (audio loopback) and useful for checking the sound quality in your system. While sound test is running there are no sound transmission to remote side. \texttt{-R} command cancels sound test and enables sound transmission.
\texttt{-RI }command shows the OnionPhone current status, \texttt{-RL} command provides measurement of the latency (connection must be active), \texttt{-RS} command discovers local and external UDP listening interfaces (if UDP listener was enabled in configuration settings).
\section{Key management}
OnionPhone uses public keys to authenticate the subscribers to each other using PGP. Also keys in this format will be used in other projects in the future (audio chat group, encryption radio and over-GSM-audio channels, etc.).   Console utility \texttt{./addkey} is useful for key management:

\hspace*{2mm}\emph{Step 1}: To generate a new key pair run:
\begin{verbatim}
./addkey -Gname [-Yaccess] [options]
\end{verbatim}
where \textsf{name} is your identifier, \textsf{access} is password that will be used to encrypt your private key and options will be passed to other participants. If \texttt{-Y}  is omitted the private key will be stored unencrypted. In this case it is recommended to store OnionPhone folder in a protected place (TrueCrypt container etc.).

The most common use is option \texttt{-Oour\_onion\_address} to present own address. Upon receipt of your key this option  will be automatically copied in address books of other participants. For example, after the command
\begin{verbatim}
./addkey -Galice -Y1234 -Or4kxspnzpnsel4fu
\end{verbatim}
files \textsf{alice.sec} (the private key encrypted  using password \textsf{1234}) and \textsf{alice} (the public key) will be created in the folder \textsf{keys}.
...
./addkey -Aalice
\end{verbatim}
To use this key as its own default edit the OnionPhone configuration file \textsf{conf.txt} specifying the key's name: \texttt{Our\_name=alice}

\hspace*{2mm}\emph{Step 4}: Now you can send the key to other users. For transmission of the key establish an un-authenticated call \textsf{guest} to \textsf{guest} with the other party (see above) and execute
\texttt{-Kalice} (or \texttt{-K} without parameter if this key is assigned as your own default). Key will be automatically added to the remote address book with the lowest possible level of trust. After manual PGP signature check of the the key file other parties can set the desired trust level  by editing \texttt{-L} parameter to the corresponding entries in the address book file \textsf{keys / contacts.txt}, for example:
\begin{verbatim}
[alice] {FNrjEjGmlZtvKXzBQkNIDA ==} #alice -Or4kxspnzpnsel4fu -L1
\end{verbatim}
You can also pass the key in any other way (for example, by email). The recipient verifies PGP signature in the key file and determines the necessary trust level, then add the key to the address book indicating the trust level as \texttt{-Llevel}, for example:
\begin{verbatim}
./addkey -Aalice -L1
\end{verbatim}
...
For each packet Keccak Sponge initiates and absorbs 128-bits session key $s$, 32-bits counter $ctr$ and 8-bits flag $O$ is $0$ for an originator and $1$ for an acceptor of the call,  then squeezes gamma $g=H(s \parallel ctr \parallel  O)$.

Gamma XOR-ed with plainest $p$ (packet bytes except first byte as a header $h$) provides ciphertext: $c=g \oplus p$.
After packet was encrypted 32-bits MAC is computed: $mac=H(s \parallel  ctr \parallel  O \parallel  h \parallel  c)$,
...
\textbf{Private key} is a binary file containing 256 bits ECDH25519 secret. Private key can be saved unencrypted (32 bytes) or encrypted with password (64 bytes). Both formats are acceptable but encrypted format requires the use of command \texttt{-Yaccess} when working with the OnionPhone.
Symmetric key derived from the password is used for private key encryption: a Keccak Sponge is initialised, which then absorbs  the \textsf{salt} string ``\$OnionPhone\_salt", then absorbs \textsf{password} string 32768 times in a row, and then squeezes a 16-bytes key:
$s=H(salt \parallel  password \parallel  password \parallel  \ldots \parallel  password)$.

To perform encryption Keccak Sponge is initialized, then absorbs key $s$ and randomly generates 16-bytes $nonce$, then squeezes a 32-bytes gamma: $g=H(s \parallel  nonce)$.
Gamma is XOR-ed  with private key body $x$ to produce encrypted data: $c=g \oplus x$.
After packet was encrypted  16-bytes MAC is computed: $mac=H(c \parallel  s \parallel  nonce)$.
Encrypted private key is $c, nonce, mac$.

\textbf{Key pair ``guest"}  is a private key as a 32 bytes binary representation of string ``\textsf{Guest secret http://torfone.org}" and corresponding public key. These keys are included by default and can be used for performing primary unauthenticated connection with an unknown receiver.

\subsection{Deniability authenticated Key agreement}
The core of implemented protocol is \emph{SKEME}. The shared secret is derived only using the Diffie - Hellman exchange. Public parameters are signed by another key outputted by another protocol performed in parallel. The original \emph{SKEME} \cite{Krawczyk96} uses the \emph{Abadi} protocol \cite{AF03} as auxiliary. For this purpose we use a slightly modified \emph{KEA+} protocol\cite{LM06} . This modification is known as the \emph{TripleDH} and is used, for example, as a initial key exchange (IKE) in the \emph{Axolotl} protocol\cite{TM13} .
...
\>searches contact in $id$-list matching $D_A =? H(id_? \parallel V^b \parallel  V^w)$\\
...
\item $A \rightarrow B: $\hspace*{10mm} $N_1$, $L=H(N_1 \parallel  N_2 \parallel  s)$
\item $B \rightarrow A: $\hspace*{10mm} $L=H(N_2 \parallel  N_1 \parallel  s)$
\end{enumerate}
...
\begin{enumerate}
\item $A \rightarrow B: $\hspace*{10mm} $U1_A=H( O \parallel  s \parallel  pass )$

where $s$ is shared secret, $O$ is byte $0$ for originator and $1$  for acceptor of call;

\item $B: $ Checks $U1_A$, rejects invalid;

$B \rightarrow A: $\hspace*{10mm} $U1_B=H( (O+2) \parallel  s \parallel  pass )$, $U2_B=H( (O+2)\parallel  s \parallel  pass \parallel  p )$

\item $A: $ Checks $U1_B$, rejects invalid; checks $U2_B$, warns invalid;

$A \rightarrow B: $\hspace*{10mm} $U2_A=H( O \parallel  s \parallel  pass \parallel  q )$
...
$B \Rightarrow A: $\hspace*{10mm} $I_R=H( (O+4) \parallel  s )$
\item $A: $ Checks $I_R$, rejects invalid; sends using channel 2:

$A \Rightarrow B: $\hspace*{10mm} $I_S=H( (O+4) \parallel  s )$
