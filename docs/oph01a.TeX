...
Additional features: Implemented noise suppressors (NPP7 and SPEEX) of environment sounds and automatic mic gain control. Built-in LPC vocoder \cite{BZ11}  featuring irreversible voice chage (robot, breathy etc.). Specially designed dynamic adaptive buffer useful for smart jitter compensation in high latency Onion environment. Radio mode (Push to Talk)  and voice control mode (Voice Active Detector with generation a short signal when transmission is completed) are available.
...
\item To enable security vocoder use the command \textbf{-Qmode}, where $mode=3$ corresponds to  ``breathy"  voice (recommended), modes 6-255 correspond to ``robot" etc. To deactivate the vocoder use the command \textbf{-Q-3}.
