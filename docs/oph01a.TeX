...
Normally OnionPhone establishes a connection over Tor network. Despite the use of  Tor as a secure transport OnionPhone provides own layes of a p2p encryption and multifactor authentication. This makes it possible to use OnionPhone for direct connections outside Tor.
...
The IKE looks resistant to UKS and wPFS attacks. Note that full deniability property conflicts with KCI resistance. Therefore after the leak of  user's private key the attacker can impersonate anybody to him. In our case the attack can be mounted only when the victim had never been in contact with the impersonated party and does not have their real public key yet. Fortunately the IKE is sensitive only to the leak of ECDH private key but not to the leak of  PGP private key used to sign.
...
First Alice and then Bob computes and send each other their randomly-looking hidden identifiers $D_A=H(id_A | B^v )$  and $ D_B=H(id_B | V^w )$, session public keys and nonce. Note that Alice the first commits her $X$, $N_A$ values and opens them only after receiving $Y$, $N_B$ from Bob.

Then participants share the common auxiliary key $K=H(W^a | B^v | W^v)=H(A^w | V^b | V^w)$ and compute authenticators signing the public session parameters:  $M_A=H(K | V | W | X | Y | N_A | N_B)$ and $M_B=H(K | W | V | Y | X | N_B | N_A)$. After the exchange and checking of authenticators both participants can be assured of the identity of each other. Finally  participants share the session key $s=H(X^y) \oplus N_A \oplus N_B = H(Y^x) \oplus N_A \oplus N_B$ and optionally reveal $K$.
...
searches contact in $id$-list matching $D_A$
