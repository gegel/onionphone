...
void mf_bpvc_ana(float mf_speech[], float mf_fpitch[], float bpvc[], float pitch[])
{
    ...
    mf_polflt(&mf_speech[mf_PIT_FR_BEG],&mf_bpf_den[0],&mf_sigbuf[mf_PIT_BEG],
	   BPF_ORD,mf_PIT_P_FR);
    mf_v_equ(&mf_bpfdel[0][0],&mf_sigbuf[mf_PIT_BEG+mf_FRAME-BPF_ORD],BPF_ORD);
    mf_zerflt(&mf_sigbuf[mf_PIT_BEG],&mf_bpf_num[0],&mf_sigbuf[mf_PIT_BEG],
	   BPF_ORD,mf_PIT_P_FR);
    
    *pitch = mf_frac_pch(&mf_sigbuf[mf_FIRST_CNTR],
				&bpvc[0],mf_fpitch[0],5,mf_PITCHMIN,mf_PITCHMAX,mf_LMIN);
    
    for (j = 1; j < mf_NUM_PITCHES; j++) {
	temp = mf_frac_pch(&mf_sigbuf[mf_FIRST_CNTR],
			&pcorr,mf_fpitch[j],5,mf_PITCHMIN,mf_PITCHMAX,mf_LMIN);
	...
	mf_v_equ(&mf_sigbuf[mf_PIT_BEG-BPF_ORD],&mf_bpfdel[j][0],BPF_ORD);
	mf_polflt(&mf_speech[mf_PIT_FR_BEG],&mf_bpf_den[j*(BPF_ORD+1)],&mf_sigbuf[mf_PIT_BEG],
	       BPF_ORD,mf_PIT_P_FR);
	mf_v_equ(&mf_bpfdel[j][0],&mf_sigbuf[mf_PIT_BEG+mf_FRAME-BPF_ORD],BPF_ORD);
	mf_zerflt(&mf_sigbuf[mf_PIT_BEG],&mf_bpf_num[j*(BPF_ORD+1)],&mf_sigbuf[mf_PIT_BEG],
	       BPF_ORD,mf_PIT_P_FR);
	...
	mf_v_equ(&mf_sigbuf[mf_PIT_BEG-ENV_ORD],&mf_envdel[j][0],ENV_ORD);
	mf_envelope(&mf_sigbuf[mf_PIT_BEG],temp,&mf_sigbuf[mf_PIT_BEG],mf_PIT_P_FR);
...
void mf_mf_bpvc_ana_init(int fr, int pmin, int pmax, int nbands, int num_p, int lmin)
{
...
float mf_gain_ana(float sigin[], float pitch, int minlength, int maxlength)
{
    ...
    gain = 10.0*log10(0.01 + (v_mf_magsq(&sigin[-(length/2)],length) / length));
    if (gain < MINGAIN)
      gain = MINGAIN;
...
void mf_noise_est(float gain,float *mf_noise_gain,float up,float down,float min,float max)
...
void mf_noise_sup(float *gain,float mf_noise_gain,float max_noise,float max_atten,float nfact)
...
void mf_mf_q_bpvc_dec(float *bpvc,int *bpvc_index,int uv_flag,int mf_NUM_BANDS)
...
void mf_q_gain(float *gain,int *gain_index,float GN_QLO,float GN_QUP,float GN_QLEV)
...
void mf_mf_q_gain_dec(float *gain,int *gain_index,float GN_QLO,float GN_QUP,float GN_QLEV)
{
...
void mf_scale_adj(float *mf_speech, float gain, float *mf_premf_v_scale, int length, int SCALEOVER)

{
	...
	mf_speech[i-1] *= ((scale*i + *mf_premf_v_scale*(SCALEOVER-i))
			      * (1.0/SCALEOVER) );
    }
