...
float *mf_vq_lspw(float *w,float *lsp,float *a,int p)
{
    int j;
        ...
        for(j=0; j < p; j++)
        {
            w[j] = (float)pow((double)mf_lpc_aejw(a,lsp[j]*M_PI,p),(double)-0.3);
            if (j == 8)
                w[j] *= 0.64;
            else if (j == 9)
                w[j] *= 0.16;
...
float mf_vq_ms4(float *cb, float *u, float *u_est, int *levels, int ma, int stages, int p, float *w, float *u_hat, int *a_indices,int max_inner)
{
    ...
    for(m=1,s=0; s < stages; s++)
    {
        ...
        for(c=0; c < ma; c++)
        {
            /* get the error from the parent node and subtract off
               the codebook value */
            (void)mf_v_equ(&n_errors[c*p],&p_errors[n_parents[c]*p],p);
            (void)mf_v_sub(&n_errors[c*p],&cb_currentstage[n_indices[c*stages+s]*p],p);

            /* get the indices that were used for the parent node */
            (void)mf_mf_v_equ_int(&n_indices[c*stages],&p_indices[n_parents[c]*stages],s);
        }
    ...
    if (a_indices)
    {
        (void)mf_mf_v_equ_int(a_indices,&n_indices[c*stages],stages);
    }
    if (u_hat)
    {
        if (u_est)
        ...
        for(s=0; s < stages; s++)
        {
            (void)mf_v_add(u_hat,&cb_currentstage[n_indices[c*stages+s]*p],p);
