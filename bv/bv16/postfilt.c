                ...
                Word16 *e)   /* output: enhanced speech signal          */
{
   ...
   for(ppt=pptmin+1; ppt<=pptmax; ppt++)
   {
      ...
      t2 = bv_sub(t1, t2);
      if (t2>=0) a0 = L_bv_shr(a0, t2);
      if (t2<0)  a1 = L_bv_shl(a1, t2); 
   ...
   else
   {
      ...
      t0 = t0+t2-15;
      t0 = t0-15;
      if (t0<0) aa = bv_shl(aa, bv_sub(0,t0));
      else aa = bv_shr(aa, t0);
   }
   ...
   *ma_a = intround(a0);
   ...
   for(n=0; n<NINT; n++)
   {
      ...
      e[n] = intround(bv_L_mac(bv_L_mac(L_bv_mult(bi1c, s[XQOFF+n-ppnew]), bi1p, s[XQOFF+n-(*pp_prv)]), bi0, s[XQOFF+n]));
   }
   for(n=NINT; n<FRSZ; n++)
   {
