              ...
              Word16  *out)
{
   ...
   for(n=0; n<FRSZ; n++)
   {
      ...
      acc = bv_L_mac(acc, ds->bq_last[0], d[LTMOFF+n-ds->pp_last+1]);
      acc = bv_L_mac(acc, ds->bq_last[1], d[LTMOFF+n-ds->pp_last]);
      acc = bv_L_mac(acc, ds->bq_last[2], d[LTMOFF+n-ds->pp_last-1]);
   ...
   {
      acc = L_bv_mult0( -AttnFacPLCG, bv_sub(ds->cfecount, HoldPLCG-1));
