                   ...
                   struct BV16_Encoder_State *cstate) /* (i/o) Coder State */
{
   ...
   for (i=0;i<FRSZD;i++) {
      for (k=0;k<DECF;k++) {
         a0 = L_bv_shr(bv_L_deposit_h(*sp++),10);
         fp2_h = fp1_h-1;
         fp2_l = fp1_l-1;
         for (j=0;j<DFO;j++)  
            a0=L_bv_sub(a0,Mpy_32(*fp2_h--,*fp2_l--,adf_h[j+1],adf_l[j+1]));
         a0 = L_bv_shl(a0, 2);             /* adf Q13 */
      ...
      for (j=0;j<DFO;j++)
         a0=L_bv_add(a0,Mpy_32_16(*fp2_h--,*fp2_l--,bdf[j+1]));
   ...
   lp0 = lxwd;
   new_exp = bv_sub(bv_norm_l(a1), 3);       /* headroom to avoid overflow */
   exp = bv_sub(cstate->xwd_exp,new_exp); /* increase in bit-resolution */
   ...
   for (i=0;i<XDOFF;i++) 
      xwd[i] = bv_shr(cstate->xwd[i], exp);
   
   /* fill-in new exponent */
   fp0 = xwd + XDOFF;
   for (i=0;i<FRSZD;i++) 
      fp0[i] = intround(L_bv_shl(lp0[i],new_exp));
   
   /* update signal memory for next frame */
   exp0 = 1;
   for (i=0;i<XDOFF;i++) {
      exp1 = bv_abs_s(xwd[FRSZD+i]);
      
      if (exp1 > exp0) 
         exp0 = exp1;
   }
   exp0 = bv_sub(bv_norm_s(exp0),3); /* extra exponent for next frame */
   ...
   if (exp >=0)
   {
      for (i=0;i<XDOFF-FRSZD;i++)  
         cstate->xwd[i] = bv_shl(cstate->xwd[i+FRSZD], exp);
   }
   else
   {
      exp = -exp;
      if (exp >=15)
         exp = 15;
      for (i=0;i<XDOFF-FRSZD;i++)  
         cstate->xwd[i] = bv_shr(cstate->xwd[i+FRSZD], exp);
   }
   for (;i<XDOFF;i++) 
      cstate->xwd[i] = bv_shl(xwd[FRSZD+i],exp0);
   
   
   cstate->xwd_exp = bv_add(new_exp, exp0);
   ...
   for (i=0;i<(LXD-MAXPPD1);i++) {
      a0 = bv_L_mac0(a0, *fp1, *fp1);
      a1 = bv_L_mac0(a1, *fp0++, *fp1++);
   }
   cor[M1-1] = a1;
   energy[M1-1] = a0;
   energy_exp[M1-1] = bv_norm_l(energy[M1-1]);
   energy_man[M1-1] = bv_extract_h(L_bv_shl(energy[M1-1], energy_exp[M1-1]));
   s0 = cor2_exp[M1-1] = bv_norm_l(a1);
   t0 = bv_extract_h(L_bv_shl(a1, s0));
   cor2[M1-1] = bv_extract_h(L_bv_mult(t0, t0));
   
   if (a1 < 0) 
      cor2[M1-1] = bv_negate(cor2[M1-1]);
   ...
   for (i=M1;i<M2;i++) {
      ...
      for (j=0;j<(LXD-MAXPPD1);j++) 
         a1 = bv_L_mac0(a1,*fp0++,*fp1++); 
      cor[i] = a1;
      a0 = bv_L_msu0(a0, *fp2, *fp2);
      a0 = bv_L_mac0(a0, *fp3, *fp3);
      fp2--; fp3--;
      energy[i] = a0;
      energy_exp[i] = bv_norm_l(energy[i]);
      energy_man[i] = bv_extract_h(L_bv_shl(energy[i], energy_exp[i]));
      s0 = cor2_exp[i] = bv_norm_l(a1);
      t0 = bv_extract_h(L_bv_shl(a1, s0));
      cor2[i] = bv_extract_h(L_bv_mult(t0, t0));
      
      if (a1 < 0) 
         cor2[i] = bv_negate(cor2[i]);
   }
   ...
   while ((npeaks < MAX_NPEAKS) && (n<MAXPPD)) {
      ...
      if (cor[n]>0) { 
         a0   = L_bv_mult(energy_man[n-1],cor2[n]);
         a1   = L_bv_mult(energy_man[n], cor2[n-1]);
         exp0 = bv_shl(bv_sub(cor2_exp[n], cor2_exp[n-1]),1);
         exp0 = bv_add(exp0, energy_exp[n-1]);
         exp0 = bv_sub(exp0, energy_exp[n]);
  ...
  /* Use quadratic interpolation to find the interpolated cor[] and
     ...
     n=idx[i];
     a0=L_bv_sub(L_bv_shr(L_bv_add(cor[n+1],cor[n-1]),1),cor[n]);
     L_Extract(a0, &ah, &al);
     a0=L_bv_shr(L_bv_sub(cor[n+1],cor[n-1]),1);
     L_Extract(a0, &bh, &bl);
     cc=cor[n];
     ...
     a0   = L_bv_mult(energy_man[n-1],cor2[n+1]);
     a1   = L_bv_mult(energy_man[n+1], cor2[n-1]);
     exp0 = bv_shl(bv_sub(cor2_exp[n+1], cor2_exp[n-1]),1);
     exp0 = bv_add(exp0, energy_exp[n-1]);
     exp0 = bv_sub(exp0, energy_exp[n+1]);
     ...
     if (a0 > a1) {  /* if right side */
        ...
        deltae = L_bv_shr(L_bv_sub(energy[n+1], eni), 2);
        
        for (k = 0; k < HDECF; k++) {
           a0=L_bv_add(L_bv_add(Mpy_32_16(ah,al,x2[k]),Mpy_32_16(bh,bl,x[k])),cc);
           eni = L_bv_add(eni, deltae);
           a1 = eni;
           exp0 = bv_norm_l(a0);
           s0 = bv_extract_h(L_bv_shl(a0, exp0));
           s0 = bv_extract_h(L_bv_mult(s0, s0));
           e2 = energym_exp;
           t0 = energym_man;
           a2 = L_bv_mult(t0, s0);
           e3 = bv_norm_l(a1);
           t1 = bv_extract_h(L_bv_shl(a1, e3));
           a3 = L_bv_mult(t1, cor2m);
           exp1 = bv_shl(bv_sub(exp0, cor2m_exp),1);
           exp1 = bv_add(exp1, e2);
           exp1 = bv_sub(exp1, e3);
        ...
        deltae = L_bv_shr(L_bv_sub(energy[n-1], eni), 2);
        for (k = 0; k < HDECF; k++) {
           a0=L_bv_add(L_bv_sub(Mpy_32_16(ah,al,x2[k]),Mpy_32_16(bh,bl,x[k])),cc);
           eni = L_bv_add(eni, deltae);
           a1=eni;
     ...
     interpolated peak */ 
     plag[i]=bv_add(bv_shl(bv_add(idx[i],1),2),im); /* lag of interp. peak */
     cor2i[i]=cor2m;
     ...
     a0 = L_bv_mult(cor2m,energymax_man);
     a1 = L_bv_mult(cor2max, energyi_man[i]);
     exp0 = bv_shl(bv_sub(cor2m_exp, cor2max_exp),1);
     exp0 = bv_add(exp0, energymax_exp);
     exp0 = bv_sub(exp0, energyi_exp[i]);
  ...
  for (i=0;i<npeaks;i++) {  /* loop thru the peaks before the largest peak */
     ...
     if (bv_abs_s(bv_sub(plag[i],cstate->cpplast)) <= maxdev) {
        a0 = L_bv_mult(cor2i[i],energym_man);
        a1 = L_bv_mult(cor2m, energyi_man[i]);
        exp0 = bv_shl(bv_sub(cor2i_exp[i], cor2m_exp),1);
        exp0 = bv_add(exp0, energym_exp);
        exp0 = bv_sub(exp0, energyi_exp[i]);
  ...
  has large enough peaks at every bv_multiple of its lag */
  i=0;
  ...
  while (2*plag[i] < mplth) {
     ...
     /* If threshold exceeded, test peaks at bv_multiples of this lag */
     a0 = L_bv_mult(cor2i[i],energymax_man);
     t1 = bv_extract_h(L_bv_mult(energyi_man[i], threshold));
     a1 = L_bv_mult(cor2max, t1);
     exp0 = bv_shl(bv_sub(cor2i_exp[i], cor2max_exp),1);
     exp0 = bv_add(exp0, energymax_exp);
     exp0 = bv_sub(exp0, energyi_exp[i]);
     ...
     if (a0 > a1) {
        ...
        j=i+1;
        k=0;
        s=bv_shl(plag[i],1); /* initialize t to twice the current lag */
        
        while (s<=mplth) { /* loop thru all bv_multiple lag <= mplth */
           
           mpflag=0;   /* initialize bv_multiple pitch flag to 0 */
           t0 = bv_bv_mult_r(s,MPDTH); 
           a=bv_sub(s, t0);   /* bv_multiple pitch range lower bound */
           b=bv_add(s, t0);   /* bv_multiple pitch range upper bound */
           while (j < npeaks) { /* loop thru peaks with larger lags */
              ...
              if (plag[j] > a) { /* if current peak lag within range, */
                 ...
                 a0 = L_bv_mult(cor2i[j],energymax_man);
                 if (k<4) 
                    t1 = MPTH[k];
                 else 
                    t1 = MPTH4;
                 t1 = bv_extract_h(L_bv_mult(t1, energyi_man[j]));
                 a1 = L_bv_mult(cor2max, t1);
                 exp0 = bv_shl(bv_sub(cor2i_exp[j], cor2max_exp),1);
                 exp0 = bv_add(exp0, energymax_exp);
                 exp0 = bv_sub(exp0, energyi_exp[j]);
           ...
           /* if no qualified peak found at this bv_multiple lag */
           ...
           k++;
           s = bv_add(s, plag[i]); /* update s to the next bv_multiple pitch lag */
           
        }
        /* if there is a qualified peak at every bv_multiple of plag[i], */
  ...
  if (im != -1) {   /* if there is at least one peak around last pitch */
     ...
     if (im < imax) { /* if lag of this peak < lag of global max, */
        a0 = L_bv_mult(cor2m,energymax_man);
        t1 = bv_extract_h(L_bv_mult(energym_man, LPTH2));
        a1 = L_bv_mult(cor2max, t1);
        exp0 = bv_shl(bv_sub(cor2m_exp, cor2max_exp),1);
        exp0 = bv_add(exp0, energymax_exp);
        exp0 = bv_sub(exp0, energym_exp);
        ...
        if (a0 > a1) {
           ...
           for (k=2; k<=5;k++) { /* check if current candidate pitch */
              s=bv_mult(plag[imax],invk[k-2]); /* is a bv_sub-bv_multiple of */
              t0 = bv_bv_mult_r(s,SMDTH);
              a=bv_sub(s, t0);      /* the time lag of */
              b=bv_add(s, t0);       /* the global maximum peak */
        ...
        a0 = L_bv_mult(cor2m,energymax_man);
        t1 = bv_extract_h(L_bv_mult(energym_man, LPTH1));
        a1 = L_bv_mult(cor2max, t1);
        exp0 = bv_shl(bv_sub(cor2m_exp, cor2max_exp),1);
        exp0 = bv_add(exp0, energymax_exp);
        exp0 = bv_sub(exp0, energym_exp);
