               ...
               Word32   *lg_el)
{
   ...
   *lgq = L_bv_add(L_bv_shr(bv_L_deposit_h(lgpecb[gidx]), 2), elg);  /* Q25 */
   ...
   i = bv_shr(bv_sub(bv_shr(bv_extract_h(L_bv_sub(prevlg[0],level)),9),LGLB),1); 
   /* get column index */
   if (i >= NGB) {
      i = NGB - 1;
   } else if (i < 0) {
   ...
   n = bv_shr(bv_sub(bv_shr(bv_extract_h(L_bv_sub(prevlg[0],prevlg[1])),9),LGCLB),1); 
   /* get row index */
   if (n >= NGCB) {
      n = NGCB - 1;
   } else if (n < 0) {
...
void gainplc(Word32 E, Word16 *lgeqm, Word32 *lgqm)
{
   ...
   if (E > TMinlg*FRSZ)
   {
      ...
      lg = L_bv_add(L_bv_shl(bv_L_deposit_h(exponent),9), 
         L_bv_shr(bv_L_deposit_h(fraction),6));    /* Q25 */
      lg = L_bv_sub(lg, 178574274); /* 178574274 = log2(1/SFSZL) Q 25 */
   }
   else
      lg = 0;   /* Minlg */
   
   mrlg = L_bv_shr(bv_L_deposit_h(lgmean),2); /* Q25 */
   mrlg = L_bv_sub(lg, mrlg);  /* Q25 */
   
   elg = 0;
   for(k=0; k<GPO; k++)
      elg = bv_L_mac0(elg, lgp[k], lgeqm[k]);  /* Q26 */
   
   elg = L_bv_shr(elg,1);                    /* Q25 */
   
   /* predicted log-gain error */
   lge = intround(L_bv_shl(L_bv_sub(mrlg, elg),2));   /* Q11 */
