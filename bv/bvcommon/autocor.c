              ...
              Word16   m)       /* (i) : LPC order             */    
{
   ...
   for(n=0; n<l_window; n++)
      buf[n] = bv_bv_mult_r(x[n], window[n]);

   /* First find shifting to prevent overflow */
   a0 = 1;
   for(n=0; n<l_window; n++){
      y_shift = bv_shr(buf[n], 4);  // guarantees no overflow in sum of squares (l_window is 160)
      a0 = bv_L_mac0(a0, y_shift, y_shift);
   }

   /* Calculate necessary shift */
   shift = bv_sub(4, bv_shr(bv_norm_l(a0),1));
   if(shift < 0)
      shift = 0;
   ...
   a0 = 1;
   for(n=0; n<l_window; n++){ 
      buf[n] = bv_shr(buf[n], shift);
      a0 = bv_L_mac0(a0, buf[n], buf[n]);
   }

   /* Normalize autocorrelation */
   shift = bv_norm_l(a0);
   r[0] = L_bv_shl(a0, shift);

   /* Calculate autocorrelation coefficients r[1], r[2], ..., r[m] */
   for(j=1; j<=m; j++){
      a0 = 0;
      for(n=0; n<l_window-j; n++)
         a0 = bv_L_mac0(a0, buf[n], buf[n+j]);
      r[j] = L_bv_shl(a0, shift);
   }
