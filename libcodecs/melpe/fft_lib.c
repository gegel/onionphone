	...
	while (n > mmax) {
		...
		for (m = 1; m < mmax; m += 2) {
			for (i = m; i <= n; i = add(i, istep)) {
				...
				sQR = data[j];
				data[i] =
				    extract_h(L_add(L_deposit_h(sPR), L_tempr));
				data[j] =
				    extract_h(L_sub(L_deposit_h(sPR), L_tempr));
				...
				sPI = data[i + 1];
				data[i + 1] =
				    extract_h(L_sub(L_deposit_h(sPI), L_tempi));
				data[j + 1] =
				    extract_h(L_add(L_deposit_h(sPI), L_tempi));
			}
