			  ...
			  Shortword m)
{
	...
	for (i = 0; i < m; i++) {
		...
		if (comp_data_shift(vk[i], vk_shift[i], 26214, -3) < 0) {
			...
			L_sum =
			    L_add(L_sum, L_shl(L_mult(vk_shift[i], 9864), 10));
			...
			L_sum =
			    L_sub(L_sum, L_shr(L_deposit_h(8520), vk_shift[i]));
			/* 8520 == 0.26 Q15 */
			       ...
			       Shortword sn_lt, Shortword sn_lt_shift)
{
	...
	if (n_flag)		/* RM: adaptive modification of ksi limit (10/98) */
		...
		if (sn_lt_shift > 0) {
			L_sum =
			    L_add(L_deposit_l(sn_lt),
				  L_shr(X05_Q15, sn_lt_shift));
			shift = sn_lt_shift;
		} else {
			L_sum =
			    L_add(L_shl(L_deposit_l(sn_lt), sn_lt_shift),
				  X05_Q15);
			shift = 0;
		}
...
static void smoothing_win(Shortword initial_noise[])
{
	...
	for (i = ENH_WINLEN - WTR_FRONT_LEN + 1; i < ENH_WINLEN; i++)
		initial_noise[i] =
		    mult(initial_noise[i], wtr_front[ENH_WINLEN - i]);
...
static void smoothed_periodogram(Shortword YY_av, Shortword yy_shift)
{
	...
	for (i = 0; i < ENH_VEC_LENF; i++) {
		...
		if (shift > 0) {
			...
			L_tmp =
			    L_sub(L_shr
				  (L_deposit_h(smoothedspect[i]), abs_s(shift)),
				  L_deposit_h(tmpns));
			shift = noise_shift[i];
		}
		...
		if (smav_shift > 0) {
			L_sum =
			    L_shr(L_mult(tmpalpha, smoothedspect[i]),
				  smav_shift);
			...
			L_sum =
			    L_add(L_sum,
				  L_shl(L_mult(temp, YY[i]), smav_shift));
		}
		if (L_sum < 1)
			L_sum = 1;
		shift = norm_l(L_sum);
			      ...
			      Shortword bias_sub_shift[])
{
	...
	for (i = 0; i < ENH_VEC_LENF; i++) {
		...
		if (av__shift > 0) {
			L_max1 =
			    L_add(L_shr
				  (L_mult(beta_var, var_sp_av[i]), av__shift),
				  L_sum);
		...
		if (av2__shift > 0) {
			L_max2 =
			    L_add(L_shr
				  (L_mult(beta_var, var_sp_2[i]), av2__shift),
		...
		if (shift3 > 0) {
			L_sum =
			    L_sub(L_deposit_h(var_sp_2[i]),
				  L_shr(L_sum, shift3));
			shift4 = av2_shift[i];
		} else {
			L_sum =
			    L_sub(L_shl(L_deposit_h(var_sp_2[i]), shift3),
				  L_sum);
...
void enh_init()
{
	...
	temp_yy[1] = 0;
	temp_yy[ENH_WINLEN] =
	    L_shr(L_mult(ybuf[ENH_WINLEN], ybuf[ENH_WINLEN]), 1);
	...
	for (i = 0; i < ENH_WINLEN / 2 - 1; i++) {
		...
		ybuf[ENH_WINLEN + 2 * i + 3] =
		    negate(ybuf[ENH_WINLEN - 2 * i - 2 + 1]);
	}
...
static void process_frame(Shortword inspeech[], Shortword outspeech[])
{
	...
	/* ---- Compute YY_av ---- */
	L_sum =
	    L_shl(L_deposit_l(YY[0]), sub(7, sub(max_YY_shift, YY_shift[0])));
	L_sum =
	    L_add(L_sum,
		  L_shl(L_deposit_l(YY[ENH_VEC_LENF - 1]),
	...
	for (i = 1; i < ENH_VEC_LENF - 1; i++) {
		...
		if (temp1 > 0)
			L_sum =
			    L_add(L_sum,
				  L_shr(L_deposit_l(gamaK[i]), sub(temp1, 7)));
		else {
		...
		for (i = 0; i < ENH_VEC_LENF; i++) {
			...
			if (comp_data_shift
			    (gamaK[i], gamaK_shift[i], ENH_INV_NOISE_BIAS,
			     0) > 0) {
				L_sum =
				    L_shr(L_deposit_h(ENH_INV_NOISE_BIAS),
					  gamaK_shift[i]);
