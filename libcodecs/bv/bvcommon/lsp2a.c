...
{				/* (o) Q12 : predictor coefficients (order = 10)  */
	...
	a[0] = 4096;		// Q12
	a0 = L_bv_add(p[1], q[1]);	// Q23
	a[1] = intround(L_bv_shl(a0, 4));	// Q12 - includes 0.5 factor of a[1] = 0.5*(p[1]+q[1])
	for (i = 1, n = 2; i < LPCO; i++, n++) {
		a0 = L_bv_add(p[i], p[n]);	// Q23
		a0 = L_bv_add(a0, q[n]);	// Q23
		a0 = L_bv_sub(a0, q[i]);	// Q23
		a[n] = intround(L_bv_shl(a0, 4));	// Q12 a[n] = 0.5 * (p[i] + p[n] + q[n] - q[i]);
	}
...
{				/* Q15 */
	...
	for (n = 1; n <= (LPCO >> 1); n++) {
		...
		for (i = 2 * n; i >= 2; i--) {
			...
			f[i] = L_bv_add(f[i], f[i - 2]);	// Q23 f[i] += f[i-2]
			a0 = Mpy_32_16(hi, lo, c);	// Q22
			f[i] = L_bv_sub(f[i], L_bv_shl(a0, 1));	// Q23 f[i] += f[i-2] - c*f[i-1];
		}
		f[1] = bv_L_msu(f[1], c, 256);	// Q23 f[1] -= c;
	}
