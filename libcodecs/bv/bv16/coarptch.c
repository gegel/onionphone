...
{				/* (i/o) Coder State */
	...
	for (i = 0; i < DFO; i++)
		tmp_h[i] = cstate->dfm_h[2 * i + 1];
	for (i = 0; i < DFO; i++)
		tmp_l[i] = cstate->dfm_h[2 * i];
	...
	energy_man[M1 - 1] =
	    bv_extract_h(L_bv_shl(energy[M1 - 1], energy_exp[M1 - 1]));
	...
	for (i = M1; i < M2; i++) {
		...
		energy_man[i] =
		    bv_extract_h(L_bv_shl(energy[i], energy_exp[i]));
	...
	while ((npeaks < MAX_NPEAKS) && (n < MAXPPD)) {
		...
		if (cor[n] > 0) {
			...
			if (a0 > a1) {
				...
				a1 = L_bv_mult(energy_man[n], cor2[n + 1]);
				exp0 =
				    bv_shl(bv_sub(cor2_exp[n], cor2_exp[n + 1]),
					   1);
				exp0 = bv_add(exp0, energy_exp[n + 1]);
				exp0 = bv_sub(exp0, energy_exp[n]);
	...
	for (i = 0; i < npeaks; i++) {
		...
		n = idx[i];
		a0 = L_bv_sub(L_bv_shr(L_bv_add(cor[n + 1], cor[n - 1]), 1),
			      cor[n]);
		L_Extract(a0, &ah, &al);
		...
		if (a0 > a1) {	/* if right side */
			...
			for (k = 0; k < HDECF; k++) {
				a0 = L_bv_add(L_bv_add
					      (Mpy_32_16(ah, al, x2[k]),
					       Mpy_32_16(bh, bl, x[k])), cc);
			...
			while (s <= mplth) {	/* loop thru all bv_multiple lag <= mplth */
				...
				while (j < npeaks) {	/* loop thru peaks with larger lags */
					...
					if (plag[j] > a) {	/* if current peak lag within range, */
						...
						a0 = L_bv_mult(cor2i[j],
							       energymax_man);
						if (k < 4)
							t1 = MPTH[k];
						else
							t1 = MPTH4;
						t1 = bv_extract_h(L_bv_mult
								  (t1,
								   energyi_man
								   [j]));
						a1 = L_bv_mult(cor2max, t1);
