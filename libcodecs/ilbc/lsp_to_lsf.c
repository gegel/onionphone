...
void WebRtcIlbcfix_Lsp2Lsf(int16_t * lsp,	/* (i) lsp vector -1...+1 in Q15 */
    ...
    )
{
	...
	for (i = m - 1; i >= 0; i--) {
		 ...
		 */
		while ((((int32_t) (*cosTblPtr) - (*lspPtr)) < 0)
		       && (k > 0)) {
			k -= 1;
			cosTblPtr--;
		}
		...
		tmp =
		    (int16_t)
		    WEBRTC_SPL_MUL_16_16_RSFT(WebRtcIlbcfix_kAcosDerivative[k],
					      diff, 11);
