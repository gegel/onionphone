...
void WebRtcIlbcfix_AbsQuant(iLBC_Enc_Inst_t * iLBCenc_inst,
			    /* (i) Encoder instance */
    ...
    )
{
	...
	WebRtcSpl_MemSetW16(syntOutBuf, 0,
			    LPC_FILTERORDER + STATE_SHORT_LEN_30MS);
	...
	WebRtcSpl_FilterARFastQ12(in, in_weighted,
				  weightDenum, LPC_FILTERORDER + 1,
				  quantLen[0]);
	WebRtcSpl_FilterARFastQ12(&in[quantLen[0]], &in_weighted[quantLen[0]],
				  &weightDenum[LPC_FILTERORDER + 1],
				  LPC_FILTERORDER + 1, quantLen[1]);

	WebRtcIlbcfix_AbsQuantLoop(syntOut,
				   in_weighted,
				   weightDenum, quantLen, iLBC_encbits->idxVec);
