    ...
    )
{
	...
	w00 =
	    WebRtcSpl_DotProductWithScale(current, current, ENH_BLOCKL, scale);
	w11 =
	    WebRtcSpl_DotProductWithScale(surround, surround, ENH_BLOCKL,
					  scale);
	w10 =
	    WebRtcSpl_DotProductWithScale(surround, current, ENH_BLOCKL, scale);

	if (w00 < 0)
		w00 = WEBRTC_SPL_WORD32_MAX;
	if (w11 < 0)
		w11 = WEBRTC_SPL_WORD32_MAX;
	...
	if (w11prim > 64) {
		endiff = WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32)
					       WebRtcSpl_DivW32W16(w00prim,
								   w11prim), 6);
	...
	if ((6 - scale + scale1) > 31) {
		...
		crit =
		    WEBRTC_SPL_SHIFT_W32(WEBRTC_SPL_MUL
					 (ENH_A0,
					  WEBRTC_SPL_RSHIFT_W32(w00prim, 14)),
					 -(6 - scale + scale1));
	}
	...
	if (errs > crit) {
		...
		w11w00 = WEBRTC_SPL_MUL_16_16((WebRtc_Word16)
					      WEBRTC_SPL_SHIFT_W32(w11, -scale),
					      (WebRtc_Word16)
					      WEBRTC_SPL_SHIFT_W32(w00,
								   -scale));

		w10w10 = WEBRTC_SPL_MUL_16_16((WebRtc_Word16)
					      WEBRTC_SPL_SHIFT_W32(w10, -scale),
					      (WebRtc_Word16)
					      WEBRTC_SPL_SHIFT_W32(w10,
								   -scale));

		w00w00 = WEBRTC_SPL_MUL_16_16((WebRtc_Word16)
					      WEBRTC_SPL_SHIFT_W32(w00, -scale),
					      (WebRtc_Word16)
					      WEBRTC_SPL_SHIFT_W32(w00,
								   -scale));
		...
		if (w00w00 > 65536) {
			...
			/* denom is in Q16 */
			denom =
			    WebRtcSpl_DivW32W16(endiff,
						(WebRtc_Word16)
						WEBRTC_SPL_RSHIFT_W32(w00w00,
								      16));
		} else {
			denom = 65536;
		}
		...
		if (denom > 7) {	/* eliminates numerical problems
			...
			if (scale > 0) {
				...
				denomW16 =
				    (WebRtc_Word16) WEBRTC_SPL_RSHIFT_W32(denom,
									  scale);

				/* num in Q(34-scale) */
				num =
				    WEBRTC_SPL_RSHIFT_W32(ENH_A0_MINUS_A0A0DIV4,
							  scale);
			} else {
				/* denomW16 is in Q16 */
			...
			A = (WebRtc_Word16)
			    WebRtcSpl_SqrtFloor(WebRtcSpl_DivW32W16
						(num, denomW16));
			...
			if ((w00prim > 0) && (w10prim > 0)) {
				w11_div_w00 =
				    WebRtcSpl_DivW32W16(w10prim,
							(WebRtc_Word16)
							w00prim);

				if (WebRtcSpl_GetSizeInBits(w11_div_w00) +
				    WebRtcSpl_GetSizeInBits(A) > 31) {
					B_W32 = 0;
				} else {
					B_W32 =
					    (WebRtc_Word32) 1073741824 -
					    (WebRtc_Word32) ENH_A0DIV2 -
