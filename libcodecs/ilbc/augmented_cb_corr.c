...
void WebRtcIlbcfix_AugmentedCbCorr(WebRtc_Word16 * target,	/* (i) Target vector */
...
{				/* (i) Scale factor to use for
	...
	for (lagcount = low; lagcount <= high; lagcount++) {
		...
		(*crossDotPtr) =
		    WebRtcSpl_DotProductWithScale(target, buffer - lagcount,
						  ilow, scale);

		/* Compute dot product on the interpolated samples */
		(*crossDotPtr) +=
		    WebRtcSpl_DotProductWithScale(target + ilow, iSPtr, 4,
						  scale);
		...
		(*crossDotPtr) +=
		    WebRtcSpl_DotProductWithScale(targetPtr, buffer - lagcount,
						  SUBL - lagcount, scale);
		crossDotPtr++;
	}
}
