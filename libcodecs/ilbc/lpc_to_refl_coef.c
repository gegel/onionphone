...
void WebRtcSpl_LpcToReflCoef(int16_t * a16, int use_order,
			     int16_t * k16)
{
	int m, k;
	...
	for (m = use_order - 1; m > 0; m--) {
		// (1 - k^2) in Q30
		tmp_inv_denom32 =
		    ((int32_t) 1073741823) - WEBRTC_SPL_MUL_16_16(k16[m],
									k16[m]);
		// (1 - k^2) in Q15
		tmp_inv_denom16 =
		    (int16_t) WEBRTC_SPL_RSHIFT_W32(tmp_inv_denom32, 15);
		...
		for (k = 1; k <= m; k++) {
			...
			tmp32[k] =
			    WEBRTC_SPL_LSHIFT_W32((int32_t) a16[k], 16)
			    -
