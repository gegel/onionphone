...
void WebRtcIlbcfix_GetSyncSeq(WebRtc_Word16 * idata,	/* (i) original data */
    ...
    )
{
	...
	WebRtcIlbcfix_NearestNeighbor(lagBlock + hl, plocs,
				      (WebRtc_Word16) WEBRTC_SPL_MUL_16_16(2,
									   (centerStartPos
									    +
									    centerEndPos)),
				      periodl);
	...
	blockStartPos[hl] =
	    (WebRtc_Word16) WEBRTC_SPL_MUL_16_16(4, centerStartPos);
	...
	for (q = hl - 1; q >= 0; q--) {
		blockStartPos[q] =
		    blockStartPos[q + 1] - period[lagBlock[q + 1]];

		WebRtcIlbcfix_NearestNeighbor(lagBlock + q, plocs,
					      (WebRtc_Word16) (blockStartPos[q]
							       +
							       (WebRtc_Word16)
		...
		if ((blockStartPos[q] -
		     (WebRtc_Word16) WEBRTC_SPL_MUL_16_16(4,
							  ENH_OVERHANG)) >= 0) {
