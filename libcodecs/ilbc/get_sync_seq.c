...
void WebRtcIlbcfix_GetSyncSeq(int16_t * idata,	/* (i) original data */
    ...
    )
{
	...
	WebRtcIlbcfix_NearestNeighbor(lagBlock + hl, plocs,
				      (int16_t) WEBRTC_SPL_MUL_16_16(2,
									   (centerStartPos
									    +
									    centerEndPos)),
				      periodl);
	...
	blockStartPos[hl] =
	    (int16_t) WEBRTC_SPL_MUL_16_16(4, centerStartPos);
	...
	for (q = hl - 1; q >= 0; q--) {
		...
		if ((blockStartPos[q] -
		     (int16_t) WEBRTC_SPL_MUL_16_16(4,
							  ENH_OVERHANG)) >= 0) {
