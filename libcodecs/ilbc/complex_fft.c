		...
		while (l < n) {
			...
			for (m = 0; m < l; ++m) {
				...
				for (i = m; i < n; i += istep) {
					...
					tr32 =
					    WEBRTC_SPL_RSHIFT_W32((WEBRTC_SPL_MUL_16_16(wr, frfi[2 * j])
								   -
					...
					frfi[2 * j] =
					    (WebRtc_Word16)
					    WEBRTC_SPL_RSHIFT_W32(qr32 - tr32,
								  1);
					frfi[2 * j + 1] =
					    (WebRtc_Word16)
					    WEBRTC_SPL_RSHIFT_W32(qi32 - ti32,
								  1);
					frfi[2 * i] =
					    (WebRtc_Word16)
					    WEBRTC_SPL_RSHIFT_W32(qr32 + tr32,
...
"r"(wri), "r"(frfi_r), "r"(CFFTRND));
...
#else
					tr32 =
					    WEBRTC_SPL_MUL_16_16(wr,
								 frfi[2 * j])
					    - WEBRTC_SPL_MUL_16_16(wi,
								   frfi[2 * j +
									1]) +
					    CFFTRND;
					...
					tr32 =
					    WEBRTC_SPL_RSHIFT_W32(tr32,
								  15 - CFFTSFT);
					ti32 =
					    WEBRTC_SPL_RSHIFT_W32(ti32,
								  15 - CFFTSFT);

					qr32 =
					    ((WebRtc_Word32) frfi[2 * i]) <<
					    CFFTSFT;
					qi32 =
					    ((WebRtc_Word32) frfi[2 * i + 1]) <<
					    CFFTSFT;
		...
		if (mode == 0) {
			...
			for (m = 0; m < l; ++m) {
				...
				for (i = m; i < n; i += istep) {
					...
					tr32 =
					    WEBRTC_SPL_RSHIFT_W32((WEBRTC_SPL_MUL_16_16_RSFT(wr, frfi[2 * j], 0)
								   -
					...
					ti32 = WEBRTC_SPL_RSHIFT_W32((WEBRTC_SPL_MUL_16_16_RSFT(wr, frfi[2 * j + 1], 0)
								      +
								      WEBRTC_SPL_MUL_16_16_RSFT
								      (wi,
								       frfi[2 *
