    ...
    )
{
	...
	if (mode) {		/* the data are good */
...
#ifndef WEBRTC_BIG_ENDIAN
		WebRtcIlbcfix_SwapBytes(bytes, iLBCdec_inst->no_of_words,
					swapped);
		last_bit =
		    WebRtcIlbcfix_UnpackBits(swapped, iLBCbits_inst,
					     iLBCdec_inst->mode);
#else
		last_bit =
		    WebRtcIlbcfix_UnpackBits(bytes, iLBCbits_inst,
					     iLBCdec_inst->mode);
#endif
		...
		if (mode) {	/* No bit errors was detected, continue decoding */
			...
			WebRtcIlbcfix_LsfCheck(lsfdeq, LPC_FILTERORDER,
					       iLBCdec_inst->lpc_n);
			WebRtcIlbcfix_DecoderInterpolateLsp(syntdenum,
							    weightdenum, lsfdeq,
							    LPC_FILTERORDER,
							    iLBCdec_inst);

			/* Decode the residual using the cb and gain indexes */
			WebRtcIlbcfix_DecodeResidual(iLBCdec_inst,
						     iLBCbits_inst, decresidual,
						     syntdenum);
	...
	if ((*iLBCdec_inst).use_enhancer == 1) {	/* Enhancer activated */
		...
		(*iLBCdec_inst).last_lag =
		    WebRtcIlbcfix_EnhancerInterface(data, decresidual,
						    iLBCdec_inst);

		/* synthesis filtering */

		/* Set up the filter state */
		WEBRTC_SPL_MEMCPY_W16(&data[-LPC_FILTERORDER],
				      iLBCdec_inst->syntMem, LPC_FILTERORDER);

		if (iLBCdec_inst->mode == 20) {
			/* Enhancer has 40 samples delay */
			i = 0;
			WebRtcSpl_FilterARFastQ12(data, data,
			...
			for (i = 1; i < iLBCdec_inst->nsub; i++) {
				WebRtcSpl_FilterARFastQ12(data + i * SUBL,
							  data + i * SUBL,
							  syntdenum + (i -
								       1) *
							  (LPC_FILTERORDER + 1),
	...
	WEBRTC_SPL_MEMCPY_W16(iLBCdec_inst->old_syntdenum,
			      syntdenum,
			      iLBCdec_inst->nsub * (LPC_FILTERORDER + 1));
