    ...
    )
{
	...
	maxtemp =
	    WebRtcSpl_MaxAbsValueW32(corrVecTemp, (int16_t) corrdim);
	...
	tloc =
	    WebRtcSpl_MaxIndexW32(corrVecUps,
				  (int16_t) (ENH_UPS0 * corrdim));

	/* make vector can be upsampled without ever running outside
	   bounds */
	*updStartPos =
	    (int16_t) WEBRTC_SPL_MUL_16_16(searchSegStartPos,
						 4) + tloc + 4;
