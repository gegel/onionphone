...
void WebRtcIlbcfix_GetLspPoly(WebRtc_Word16 * lsp,	/* (i) LSP in Q15 */
			      WebRtc_Word32 * f)
{				/* (o) polonymial in Q24 */
	...
	for (i = 2; i <= 5; i++) {
		...
		for (j = i; j > 1; j--) {
			...
			high =
			    (WebRtc_Word16) WEBRTC_SPL_RSHIFT_W32(fPtr[-1], 16);
			low =
			    (WebRtc_Word16) WEBRTC_SPL_RSHIFT_W32(fPtr[-1] -
								  WEBRTC_SPL_LSHIFT_W32
								  (((WebRtc_Word32) high), 16), 1);

			tmpW32 =
			    WEBRTC_SPL_LSHIFT_W32(WEBRTC_SPL_MUL_16_16
						  (high, (*lspPtr)),
						  2) +
			    WEBRTC_SPL_LSHIFT_W32(WEBRTC_SPL_MUL_16_16_RSFT
						  (low, (*lspPtr), 15), 2);
		...
		(*fPtr) -=
		    (WebRtc_Word32)
		    WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32) (*lspPtr), 10);
