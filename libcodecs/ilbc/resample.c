...
static void WebRtcSpl_32khzTo22khzIntToShort(const WebRtc_Word32 * In,
					     WebRtc_Word16 * Out,
					     const WebRtc_Word32 K);

void WebRtcSpl_32khzTo22khzIntToInt(const WebRtc_Word32 * In,
				    WebRtc_Word32 * Out, const WebRtc_Word32 K);
				    ...
				    WebRtc_Word32 * tmpmem)
{
	...
	for (k = 0; k < SUB_BLOCKS_22_16; k++) {
		...
		/////
		WebRtcSpl_UpBy2ShortToInt(in, 220 / SUB_BLOCKS_22_16,
					  tmpmem + 16, state->S_22_44);
		...
		WebRtcSpl_Resample44khzTo32khz(tmpmem + 8, tmpmem,
					       40 / SUB_BLOCKS_22_16);
				    ...
				    WebRtc_Word32 * tmpmem)
{
	...
	for (k = 0; k < SUB_BLOCKS_16_22; k++) {
		...
		/////
		WebRtcSpl_UpBy2ShortToInt(in, 160 / SUB_BLOCKS_16_22,
					  tmpmem + 8, state->S_16_32);
		...
		WebRtcSpl_32khzTo22khzIntToShort(tmpmem, out,
						 20 / SUB_BLOCKS_16_22);
				   ...
				   WebRtc_Word32 * tmpmem)
{
	...
	for (k = 0; k < SUB_BLOCKS_8_22; k++) {
		...
		WebRtcSpl_32khzTo22khzIntToInt(tmpmem + 10, tmpmem,
					       10 / SUB_BLOCKS_8_22);
...
static void WebRtcSpl_DotProdIntToInt(const WebRtc_Word32 * in1,
				      const WebRtc_Word32 * in2,
				      const WebRtc_Word16 * coef_ptr,
				      WebRtc_Word32 * out1,
				      WebRtc_Word32 * out2)
{
...
// compute two inner-products and store them to output array
static void WebRtcSpl_DotProdIntToShort(const WebRtc_Word32 * in1,
					const WebRtc_Word32 * in2,
					const WebRtc_Word16 * coef_ptr,
					WebRtc_Word16 * out1,
					WebRtc_Word16 * out2)
				    ...
				    WebRtc_Word32 * Out, const WebRtc_Word32 K)
{
	...
	for (m = 0; m < K; m++) {
		...
		WebRtcSpl_DotProdIntToInt(&In[0], &In[22],
					  kCoefficients32To22[0], &Out[1],
					  &Out[10]);

		// sum and accumulate filter coefficients and input samples
		WebRtcSpl_DotProdIntToInt(&In[2], &In[20],
					  kCoefficients32To22[1], &Out[2],
					  &Out[9]);

		// sum and accumulate filter coefficients and input samples
		WebRtcSpl_DotProdIntToInt(&In[3], &In[19],
					  kCoefficients32To22[2], &Out[3],
					  &Out[8]);

		// sum and accumulate filter coefficients and input samples
		WebRtcSpl_DotProdIntToInt(&In[5], &In[17],
					  kCoefficients32To22[3], &Out[4],
					  &Out[7]);

		// sum and accumulate filter coefficients and input samples
		WebRtcSpl_DotProdIntToInt(&In[6], &In[16],
					  kCoefficients32To22[4], &Out[5],
					  &Out[6]);
				      ...
				      const WebRtc_Word32 K)
{
	...
	for (m = 0; m < K; m++) {
		...
		// sum and accumulate filter coefficients and input samples
		WebRtcSpl_DotProdIntToShort(&In[0], &In[22],
					    kCoefficients32To22[0], &Out[1],
					    &Out[10]);

		// sum and accumulate filter coefficients and input samples
		WebRtcSpl_DotProdIntToShort(&In[2], &In[20],
					    kCoefficients32To22[1], &Out[2],
					    &Out[9]);

		// sum and accumulate filter coefficients and input samples
		WebRtcSpl_DotProdIntToShort(&In[3], &In[19],
					    kCoefficients32To22[2], &Out[3],
					    &Out[8]);

		// sum and accumulate filter coefficients and input samples
		WebRtcSpl_DotProdIntToShort(&In[5], &In[17],
					    kCoefficients32To22[3], &Out[4],
					    &Out[7]);

		// sum and accumulate filter coefficients and input samples
		WebRtcSpl_DotProdIntToShort(&In[6], &In[16],
					    kCoefficients32To22[4], &Out[5],
					    &Out[6]);
