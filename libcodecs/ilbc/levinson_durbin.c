...
WebRtc_Word16 WebRtcSpl_LevinsonDurbin(WebRtc_Word32 * R, WebRtc_Word16 * A,
				       WebRtc_Word16 * K, WebRtc_Word16 order)
{
	WebRtc_Word16 i, j;
	...
	WebRtc_Word16 A_hi[SPL_LEVINSON_MAXORDER + 1],
	    A_low[SPL_LEVINSON_MAXORDER + 1];
	// LPC coefficients for next iteration
	WebRtc_Word16 A_upd_hi[SPL_LEVINSON_MAXORDER + 1],
	...
	for (i = 2; i <= order; i++) {
		...
		temp1W32 += (WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32) R_hi[i], 16)
			     + WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32) R_low[i],
						     1));
		...
		if ((WebRtc_Word32) WEBRTC_SPL_ABS_W16(K_hi) >
		    (WebRtc_Word32) 32750) {
		...
		for (j = 1; j < i; j++) {
			// temp1W32 = A[j] in Q27
			temp1W32 =
			    WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32) A_hi[j], 16)
			    + WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32) A_low[j],
						    1);
			...
			A_upd_hi[j] =
			    (WebRtc_Word16) WEBRTC_SPL_RSHIFT_W32(temp1W32, 16);
			A_upd_low[j] =
			    (WebRtc_Word16)
	...
	for (i = 1; i <= order; i++) {
		...
		A[i] =
		    (WebRtc_Word16) WEBRTC_SPL_RSHIFT_W32((temp1W32 << 1) +
							  (WebRtc_Word32) 32768,
							  16);
	}
