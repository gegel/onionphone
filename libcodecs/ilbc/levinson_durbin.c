...
int16_t WebRtcSpl_LevinsonDurbin(int32_t * R, int16_t * A,
				       int16_t * K, int16_t order)
{
	int16_t i, j;
	// Auto-correlation coefficients in high precision
	int16_t R_hi[SPL_LEVINSON_MAXORDER + 1],
	    R_low[SPL_LEVINSON_MAXORDER + 1];
	// LPC coefficients in high precision
	int16_t A_hi[SPL_LEVINSON_MAXORDER + 1],
	    A_low[SPL_LEVINSON_MAXORDER + 1];
	...
	memzero(R_hi, (SPL_LEVINSON_MAXORDER + 1) * sizeof(int16_t));
	memzero(R_low, (SPL_LEVINSON_MAXORDER + 1) * sizeof(int16_t));
	...
	K_low = (int16_t) WEBRTC_SPL_RSHIFT_W32((temp1W32
						       -
						       WEBRTC_SPL_LSHIFT_W32((int32_t) K_hi, 16)),
						1);
	...
	A_low[1] = (int16_t) WEBRTC_SPL_RSHIFT_W32((temp1W32
							  -
							  WEBRTC_SPL_LSHIFT_W32((int32_t) A_hi[1], 16)),
						   1);

	// Alpha = R[0] * (1-K^2)

	temp1W32 =
	    (((WEBRTC_SPL_MUL_16_16(K_hi, K_low) >> 14) +
	      WEBRTC_SPL_MUL_16_16(K_hi, K_hi))
	...
	for (i = 2; i <= order; i++) {
		...
		for (j = 1; j < i; j++) {
			// temp1W32 is in Q31
			temp1W32 +=
			    ((WEBRTC_SPL_MUL_16_16(R_hi[j], A_hi[i - j]) << 1)
			     +
			     (((WEBRTC_SPL_MUL_16_16(R_hi[j], A_low[i - j]) >>
				15)
			       +
			       (WEBRTC_SPL_MUL_16_16(R_low[j], A_hi[i - j]) >>
				15)) << 1));
		}

		temp1W32 = WEBRTC_SPL_LSHIFT_W32(temp1W32, 4);
		temp1W32 += (WEBRTC_SPL_LSHIFT_W32((int32_t) R_hi[i], 16)
			     + WEBRTC_SPL_LSHIFT_W32((int32_t) R_low[i],
						     1));
		...
		if ((int32_t) WEBRTC_SPL_ABS_W16(K_hi) >
		    (int32_t) 32750) {
		...
		for (j = 1; j < i; j++) {
			// temp1W32 = A[j] in Q27
			temp1W32 =
			    WEBRTC_SPL_LSHIFT_W32((int32_t) A_hi[j], 16)
			    + WEBRTC_SPL_LSHIFT_W32((int32_t) A_low[j],
						    1);

			// temp1W32 += K*A[i-j] in Q27
			temp1W32 += ((WEBRTC_SPL_MUL_16_16(K_hi, A_hi[i - j])
				      +
				      (WEBRTC_SPL_MUL_16_16(K_hi, A_low[i - j])
				       >> 15)
				      +
				      (WEBRTC_SPL_MUL_16_16(K_low, A_hi[i - j])
				       >> 15)) << 1);
			...
			A_upd_low[j] =
			    (int16_t)
			    WEBRTC_SPL_RSHIFT_W32((temp1W32 -
						   WEBRTC_SPL_LSHIFT_W32((int32_t) A_upd_hi[j], 16)),
						  1);
		}
		...
		A_upd_low[i] =
		    (int16_t)
		    WEBRTC_SPL_RSHIFT_W32((temp3W32 -
		...
		temp1W32 = ((WEBRTC_SPL_MUL_16_16(Alpha_hi, tmp_hi)
			     ...
			     +
			     (WEBRTC_SPL_MUL_16_16(Alpha_low, tmp_hi) >> 15)) <<
			    1);
	...
	for (i = 1; i <= order; i++) {
		...
		A[i] =
		    (int16_t) WEBRTC_SPL_RSHIFT_W32((temp1W32 << 1) +
							  (int32_t) 32768,
							  16);
	}
