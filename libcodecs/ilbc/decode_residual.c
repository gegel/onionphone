...
void WebRtcIlbcfix_DecodeResidual(iLBC_Dec_Inst_t * iLBCdec_inst,
    ...
    )
{
	...
	WebRtcIlbcfix_StateConstruct(iLBC_encbits->idxForMax,
				     iLBC_encbits->idxVec,
				     &syntdenum[(iLBC_encbits->startIdx -
						 1) * (LPC_FILTERORDER + 1)],
	...
	if (Nfor > 0) {
		...
		WEBRTC_SPL_MEMCPY_W16(mem + CB_MEML - STATE_LEN,
				      decresidual + (iLBC_encbits->startIdx -
						     1) * SUBL, STATE_LEN);
	...
	if (Nback > 0) {
		...
		meml_gotten =
		    SUBL * (iLBCdec_inst->nsub + 1 - iLBC_encbits->startIdx);
