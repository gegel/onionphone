    ...
    )
{
	...
	for (n = (iLBCenc_inst->nsub - 1); n > 0; n--) {
		(*seqEnPtr) =
		    WebRtcSpl_DotProductWithScale(ssqPtr, ssqPtr, 76, scale1);
		ssqPtr += 40;
		seqEnPtr++;
	}
	...
	maxW32 =
	    WebRtcSpl_MaxValueW32(ssqEn,
				  (WebRtc_Word16) (iLBCenc_inst->nsub - 1));
	...
	if (iLBCenc_inst->mode == 20) {
		ssqPtr =
		    (WebRtc_Word16 *) WebRtcIlbcfix_kStartSequenceEnrgWin + 1;
	} else {
		ssqPtr = (WebRtc_Word16 *) WebRtcIlbcfix_kStartSequenceEnrgWin;
	}
	for (n = (iLBCenc_inst->nsub - 1); n > 0; n--) {
		(*seqEnPtr) =
		    WEBRTC_SPL_MUL(((*seqEnPtr) >> scale1), (*ssqPtr));
		seqEnPtr++;
		ssqPtr++;
	}
	...
	pos =
	    WebRtcSpl_MaxIndexW32(ssqEn,
				  (WebRtc_Word16) (iLBCenc_inst->nsub - 1)) + 1;
