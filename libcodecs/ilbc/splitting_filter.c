...
static const uint16_t WebRtcSpl_kAllPassFilter1[3] =
    { 6418, 36982, 57261 };
static const uint16_t WebRtcSpl_kAllPassFilter2[3] =
    { 21333, 49062, 63010 };
			   ...
			   int32_t * filter_state2)
{
	...
	for (i = 0, k = 0; i < kBandFrameLength; i++, k += 2) {
		half_in2[i] =
		    WEBRTC_SPL_LSHIFT_W32((int32_t) in_data[k], 10);
		half_in1[i] =
		    WEBRTC_SPL_LSHIFT_W32((int32_t) in_data[k + 1], 10);
	}
			    ...
			    int32_t * filter_state2)
{
	...
	for (i = 0; i < kBandFrameLength; i++) {
		tmp =
		    (int32_t) low_band[i] + (int32_t) high_band[i];
