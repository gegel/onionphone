...
static const WebRtc_UWord16 WebRtcSpl_kAllPassFilter1[3] =
    { 6418, 36982, 57261 };
static const WebRtc_UWord16 WebRtcSpl_kAllPassFilter2[3] =
    { 21333, 49062, 63010 };
...
void WebRtcSpl_AllPassQMF(WebRtc_Word32 * in_data,
			  const WebRtc_Word16 data_length,
			  WebRtc_Word32 * out_data,
			  const WebRtc_UWord16 * filter_coefficients,
			  WebRtc_Word32 * filter_state)
{
	...
	out_data[0] =
	    WEBRTC_SPL_SCALEDIFF32(filter_coefficients[0], diff,
				   filter_state[0]);
	...
	in_data[0] =
	    WEBRTC_SPL_SCALEDIFF32(filter_coefficients[1], diff,
				   filter_state[2]);
	for (k = 1; k < data_length; k++) {
	...
	out_data[0] =
	    WEBRTC_SPL_SCALEDIFF32(filter_coefficients[2], diff,
				   filter_state[4]);
	for (k = 1; k < data_length; k++) {
...
void WebRtcSpl_AnalysisQMF(const WebRtc_Word16 * in_data,
			   WebRtc_Word16 * low_band, WebRtc_Word16 * high_band,
			   ...
			   WebRtc_Word32 * filter_state2)
{
	...
	for (i = 0, k = 0; i < kBandFrameLength; i++, k += 2) {
		half_in2[i] =
		    WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32) in_data[k], 10);
		half_in1[i] =
		    WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32) in_data[k + 1], 10);
	}

	// All pass filter even and odd samples, independently.
	WebRtcSpl_AllPassQMF(half_in1, kBandFrameLength, filter1,
			     WebRtcSpl_kAllPassFilter1, filter_state1);
	WebRtcSpl_AllPassQMF(half_in2, kBandFrameLength, filter2,
			    ...
			    WebRtc_Word32 * filter_state2)
{
	...
	for (i = 0; i < kBandFrameLength; i++) {
		tmp =
		    (WebRtc_Word32) low_band[i] + (WebRtc_Word32) high_band[i];
