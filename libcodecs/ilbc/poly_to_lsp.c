...
void WebRtcIlbcfix_Poly2Lsp(WebRtc_Word16 * a,	/* (o) A coefficients in Q12 */
	...
	for (j = 1; j < COS_GRID_POINTS && foundFreqs < 10; j++) {
		...
		if (WEBRTC_SPL_MUL_16_16(ylow, yhigh) <= 0) {
			...
			for (i = 0; i < 4; i++) {
				/* xmid =(xlow + xhigh)/2 */
				xmid =
				    WEBRTC_SPL_RSHIFT_W16(xlow,
							  1) +
				    WEBRTC_SPL_RSHIFT_W16(xhigh, 1);
				ymid =
				    WebRtcIlbcfix_Chebyshev(xmid, f[fi_select]);
			...
			if (foundFreqs < 10) {
				...
				ylow =
				    WebRtcIlbcfix_Chebyshev(xlow, f[fi_select]);
			}
		}
	}
