    ...
    )
{
	...
	WebRtcSpl_MemSetW16(&buf[-LPC_FILTERORDER], 0,
			    noOfZeros + LPC_FILTERORDER);
	WebRtcSpl_FilterARFastQ12(decResidual + noOfZeros, buf + noOfZeros,
				  weightDenum, LPC_FILTERORDER + 1,
				  WebRtcIlbcfix_kFilterRange[block]);

	/* weighting of the target vector */
	WEBRTC_SPL_MEMCPY_W16(&target[-LPC_FILTERORDER],
			      buf + noOfZeros +
			      WebRtcIlbcfix_kFilterRange[block] -
			      LPC_FILTERORDER, LPC_FILTERORDER);
	WebRtcSpl_FilterARFastQ12(intarget, target, weightDenum,
				  LPC_FILTERORDER + 1, lTarget);
	...
	if ((temp1 > 0) && (temp2 > 0)) {
		...
		scale =
		    WebRtcSpl_GetSizeInBits(WEBRTC_SPL_MUL_16_16(temp1, temp1));
	} else {
	...
	targetEner =
	    WebRtcSpl_DotProductWithScale(target, target, lTarget, scale);

	/* Prepare search over one more codebook section. This section
	   is created by filtering the original buffer with a filter. */
	WebRtcIlbcfix_FilteredCbVecs(cbvectors, buf, lMem,
				     WebRtcIlbcfix_kFilterRange[block]);
	...
	if (lTarget == SUBL) {
		...
		WebRtcIlbcfix_InterpolateSamples(interpSamplesFilt, cbvectors,
						 lMem);
		...
		WebRtcIlbcfix_CbMemEnergy(range, buf, cbvectors, lMem,
					  lTarget, energyW16 + 20,
					  energyShifts + 20, scale, base_size);
		 ...
		 * energy values are used in all three stages. */
		WebRtcIlbcfix_CbMemEnergy(range, buf, cbvectors, lMem,
					  lTarget, energyW16, energyShifts,
					  scale, base_size);
		...
		WebRtcSpl_MemSetW16(energyW16 + range + base_size, 0,
				    (base_size - range));
	}
	...
	for (stage = 0; stage < CB_NSTAGES; stage++) {
		...
		WebRtcIlbcfix_CbSearchCore(cDot, range, stage, inverseEnergy,
		...
		if (lTarget == SUBL) {
			i = sInd;
			if (sInd < 20) {
				WebRtcIlbcfix_AugmentedCbCorr(target,
							      cbvectors + lMem,
							      interpSamplesFilt,
							      cDot,
							      (int16_t)
	...
	codedEner =
	    WebRtcSpl_DotProductWithScale(codedVec, codedVec, lTarget, scale);
