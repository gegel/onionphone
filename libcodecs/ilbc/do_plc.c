...
void WebRtcIlbcfix_DoThePlc(int16_t * PLCresidual,	/* (o) concealed residual */
	...
	if (PLI == 1) {
		...
		if (iLBCdec_inst->prevPLI != 1) {
			...
			if ((tmp2W32 > 0) && (ener_comp > 0)) {
				...
				scale1 =
				    (int16_t) WebRtcSpl_NormW32(tmp2W32) -
				    16;
				tmp1 =
				    (int16_t)
				...
				scale2 =
				    (int16_t) WebRtcSpl_NormW32(ener) -
				    16;
				tmp2 =
				    (int16_t) WEBRTC_SPL_SHIFT_W32(ener,
									 scale2);
				...
				tmp1 =
				    (int16_t) WEBRTC_SPL_SHIFT_W32(cross,
									 (totscale
									  >>
									  1));
				tmp2 =
				...
				max_perSquare =
				    (int16_t) WebRtcSpl_DivW32W16(nom,
									denom);
		...
		for (i = 0; i < iLBCdec_inst->blockl; i++) {
			...
			iLBCdec_inst->seed =
			    (int16_t) (WEBRTC_SPL_MUL_16_16
					     (iLBCdec_inst->seed,
					      31821) + (int32_t) 13849);
			randlag =
			    53 + (int16_t) (iLBCdec_inst->seed & 63);
			...
			PLCresidual[i] =
			    (int16_t) WEBRTC_SPL_MUL_16_16_RSFT(tot_gain,
								      (int16_t)
		...
		if (energy <
		    (WEBRTC_SPL_SHIFT_W32
		     (((int32_t) iLBCdec_inst->blockl * 900),
