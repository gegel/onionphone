...
void WebRtcIlbcfix_DoThePlc(WebRtc_Word16 * PLCresidual,	/* (o) concealed residual */
    ...
    )
{
	...
	if (PLI == 1) {
		...
		if (iLBCdec_inst->prevPLI != 1) {
			...
			max =
			    WebRtcSpl_MaxAbsValueW16((*iLBCdec_inst).
						     prevResidual,
						     (WebRtc_Word16)
						     iLBCdec_inst->blockl);
			...
			corrLen =
			    WEBRTC_SPL_MIN(60,
					   iLBCdec_inst->blockl - (inlag + 3));

			WebRtcIlbcfix_CompCorr(&cross, &ener,
					       iLBCdec_inst->prevResidual, lag,
					       iLBCdec_inst->blockl, corrLen,
					       scale3);

			/* Normalize and store cross^2 and the number of shifts */
			shiftMax =
			    WebRtcSpl_GetSizeInBits(WEBRTC_SPL_ABS_W32(cross)) -
			    15;
			crossSquareMax =
			    (WebRtc_Word16)
			...
			for (j = inlag - 2; j <= inlag + 3; j++) {
				...
				shift1 =
				    WebRtcSpl_GetSizeInBits(WEBRTC_SPL_ABS_W32
							    (cross_comp)) - 15;
				crossSquare =
				    (WebRtc_Word16)
				    WEBRTC_SPL_MUL_16_16_RSFT
				    (WEBRTC_SPL_SHIFT_W32(cross_comp, -shift1),
				     WEBRTC_SPL_SHIFT_W32(cross_comp, -shift1),
				     15);

				shift2 = WebRtcSpl_GetSizeInBits(ener) - 15;
				measure =
				    WEBRTC_SPL_MUL_16_16(WEBRTC_SPL_SHIFT_W32
							 (ener, -shift2),
							 crossSquare);

				shift3 =
				    WebRtcSpl_GetSizeInBits(ener_comp) - 15;
				maxMeasure =
				    WEBRTC_SPL_MUL_16_16(WEBRTC_SPL_SHIFT_W32
							 (ener_comp, -shift3),
							 crossSquareMax);
			 ...
			 */
			tmp2W32 =
			    WebRtcSpl_DotProductWithScale(&iLBCdec_inst->
							  prevResidual
							  [iLBCdec_inst->
							   blockl - corrLen],
							  &iLBCdec_inst->
			...
			if ((tmp2W32 > 0) && (ener_comp > 0)) {
				...
				scale1 =
				    (WebRtc_Word16) WebRtcSpl_NormW32(tmp2W32) -
				    16;
				tmp1 =
				    (WebRtc_Word16)
				    WEBRTC_SPL_SHIFT_W32(tmp2W32, scale1);

				scale2 =
				    (WebRtc_Word16) WebRtcSpl_NormW32(ener) -
				    16;
				tmp2 =
				    (WebRtc_Word16) WEBRTC_SPL_SHIFT_W32(ener,
									 scale2);
				...
				tmp1 =
				    (WebRtc_Word16) WEBRTC_SPL_SHIFT_W32(cross,
									 (totscale
									  >>
									  1));
				tmp2 =
				...
				max_perSquare =
				    (WebRtc_Word16) WebRtcSpl_DivW32W16(nom,
									denom);
		...
		} else if (iLBCdec_inst->consPLICount * iLBCdec_inst->blockl >
			   640) {
		...
		if (max_perSquare > 7868) {	/* periodicity > 0.7  (0.7^4=0.2401 in Q15) */
			...
			ind = 5;
			while ((max_perSquare < WebRtcIlbcfix_kPlcPerSqr[ind])
			       && (ind > 0)) {
				ind--;
			}
			/* pitch fact is approximated by first order */
			tmpW32 =
			    (WebRtc_Word32) WebRtcIlbcfix_kPlcPitchFact[ind] +
			    WEBRTC_SPL_MUL_16_16_RSFT(WebRtcIlbcfix_kPlcPfSlope
						      [ind],
						      (max_perSquare -
		...
		for (i = 0; i < iLBCdec_inst->blockl; i++) {
			...
			iLBCdec_inst->seed =
			    (WebRtc_Word16) (WEBRTC_SPL_MUL_16_16
					     (iLBCdec_inst->seed,
					      31821) + (WebRtc_Word32) 13849);
			randlag =
			    53 + (WebRtc_Word16) (iLBCdec_inst->seed & 63);
			...
			PLCresidual[i] =
			    (WebRtc_Word16) WEBRTC_SPL_MUL_16_16_RSFT(tot_gain,
								      (WebRtc_Word16)
								      WEBRTC_SPL_RSHIFT_W32
								      ((WEBRTC_SPL_MUL_16_16(pitchfact, PLCresidual[i]) + WEBRTC_SPL_MUL_16_16((32767 - pitchfact), randvec[i]) + 16384), 15), 15);
		...
		if (energy <
		    (WEBRTC_SPL_SHIFT_W32
		     (((WebRtc_Word32) iLBCdec_inst->blockl * 900),
		...
		/* use the old LPC */
		WEBRTC_SPL_MEMCPY_W16(PLClpc, (*iLBCdec_inst).prevLpc,
				      LPC_FILTERORDER + 1);
	...
	/* no packet loss, copy input */
		...
		WEBRTC_SPL_MEMCPY_W16(PLCresidual, decresidual,
				      iLBCdec_inst->blockl);
	...
	WEBRTC_SPL_MEMCPY_W16(iLBCdec_inst->prevLpc, PLClpc,
			      (LPC_FILTERORDER + 1));
	WEBRTC_SPL_MEMCPY_W16(iLBCdec_inst->prevResidual, PLCresidual,
			      iLBCdec_inst->blockl);
