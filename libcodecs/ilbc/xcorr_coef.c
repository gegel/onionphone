...
int WebRtcIlbcfix_XcorrCoef(WebRtc_Word16 * target,	/* (i) first array */
    ...
    )
{
	...
	if (step == 1) {
		max =
		    WebRtcSpl_MaxAbsValueW16(regressor,
					     (WebRtc_Word16) (subl + searchLen -
							      1));
		rp_beg = regressor;
		rp_end = &regressor[subl];
	} else {		/* step==-1 */
		max =
		    WebRtcSpl_MaxAbsValueW16(&regressor[-searchLen],
					     (WebRtc_Word16) (subl + searchLen -
							      1));
		rp_beg = &regressor[-1];
	...
	for (k = 0; k < searchLen; k++) {
		...
		if ((Energy > 0) && (crossCorr > 0)) {
			...
			crossCorrScale =
			    (WebRtc_Word16) WebRtcSpl_NormW32(crossCorr) - 16;
			crossCorrmod =
			    (WebRtc_Word16) WEBRTC_SPL_SHIFT_W32(crossCorr,
								 crossCorrScale);
			Energyscale =
			    (WebRtc_Word16) WebRtcSpl_NormW32(Energy) - 16;
			EnergyMod =
			    (WebRtc_Word16) WEBRTC_SPL_SHIFT_W32(Energy,
								 Energyscale);
			...
			if (scalediff < 0) {
				newCrit =
				    ((WebRtc_Word32) crossCorrSqMod *
				     EnergyMod_Max) >> (-scalediff);
				maxCrit =
				    ((WebRtc_Word32) crossCorrSqMod_Max *
				     EnergyMod);
			} else {
				newCrit =
				    ((WebRtc_Word32) crossCorrSqMod *
		...
		Energy += step * (WEBRTC_SPL_RSHIFT_W32(((WebRtc_Word32)
							 (*rp_end) *
							 (*rp_end)) -
							((WebRtc_Word32)
							 (*rp_beg) * (*rp_beg)),
