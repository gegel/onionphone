    ...
    )
{
	...
	WebRtcSpl_DownsampleFast(enh_buf + ENH_BUFL - inLen,	/* Input samples */
				 (WebRtc_Word16) (inLen + ENH_BUFL_FILTEROVERHEAD), downsampled, (WebRtc_Word16) WEBRTC_SPL_RSHIFT_W16(inLen, 1), (WebRtc_Word16 *) WebRtcIlbcfix_kLpFiltCoefs,	/* Coefficients in Q12 */
	...
	for (iblock = 0; iblock < new_blocks; iblock++) {
		...
		max16 = WebRtcSpl_MaxAbsValueW16(&regressor[-50],
						 (WebRtc_Word16)
						 (ENH_BLOCKL_HALF + 50 - 1));
		shifts =
		    WebRtcSpl_GetSizeInBits(WEBRTC_SPL_MUL_16_16(max16, max16))
		    - 25;
		...
		for (i = 0; i < 3; i++) {
			...
			ener =
			    WebRtcSpl_DotProductWithScale(&regressor
							  [-lagmax[i]],
							  &regressor[-lagmax
								     [i]],
							  ENH_BLOCKL_HALF,
			...
			corr16[i] =
			    (WebRtc_Word16) WEBRTC_SPL_SHIFT_W32(corrmax[i],
								 corrSh);
			corr16[i] =
			    (WebRtc_Word16) WEBRTC_SPL_MUL_16_16_RSFT(corr16[i],
								      corr16[i],
								      16);
			en16[i] =
			    (WebRtc_Word16) WEBRTC_SPL_SHIFT_W32(ener, enerSh);
		...
		for (i = 1; i < 3; i++) {
			if (totsh[ind] > totsh[i]) {
				...
				if (WEBRTC_SPL_MUL_16_16(corr16[ind], en16[i]) <
				    WEBRTC_SPL_MUL_16_16_RSFT(corr16[i],
							      en16[ind], sh)) {
					ind = i;
				}
			} else {
				sh = WEBRTC_SPL_MIN(31, totsh[i] - totsh[ind]);
				if (WEBRTC_SPL_MUL_16_16_RSFT
				    (corr16[ind], en16[i],
				     sh) < WEBRTC_SPL_MUL_16_16(corr16[i],
								en16[ind])) {
					ind = i;
				}
			}
	...
	if ((iLBCdec_inst->prev_enh_pl == 1)
		...
		if (iLBCdec_inst->prev_enh_pl == 1) {
			...
			while (lag < pos) {
				WEBRTC_SPL_MEMCPY_W16(&plc_pred[pos - lag], in,
						      lag);
				pos = pos - lag;
			}
		...
		if (iLBCdec_inst->prev_enh_pl == 1) {
			...
			max =
			    WebRtcSpl_MaxAbsValueW16(&enh_buf
						     [ENH_BUFL -
						      iLBCdec_inst->blockl -
			...
			tmp2 =
			    WebRtcSpl_DotProductWithScale(&enh_buf
							  [ENH_BUFL -
							   iLBCdec_inst->
							   blockl - plc_blockl],
							  &enh_buf[ENH_BUFL -
							  ...
							  plc_blockl, scale);
			tmp1 =
			    WebRtcSpl_DotProductWithScale(plc_pred, plc_pred,
							  plc_blockl, scale);
			...
			if ((tmp1 > 0) && ((tmp1 >> 2) > tmp2)) {
				...
				SqrtEnChange =
				    (WebRtc_Word16)
				    WebRtcSpl_SqrtFloor(WEBRTC_SPL_LSHIFT_W32
							((WebRtc_Word32)
							 EnChange, 14));

				/* Multiply first part of vector with 2*SqrtEnChange */
				WebRtcSpl_ScaleVector(plc_pred, plc_pred,
						      SqrtEnChange,
						      (WebRtc_Word16)
						      (plc_blockl - 16), 14);
				...
				inc =
				    (2048 -
				     WEBRTC_SPL_RSHIFT_W16(SqrtEnChange, 3));
				...
				for (i = 16; i > 0; i--) {
					(*tmpW16ptr) =
					    (WebRtc_Word16)
					    WEBRTC_SPL_MUL_16_16_RSFT((*tmpW16ptr), (SqrtEnChange + (win >> 1)), 14);
			...
			win = 0;
			enh_bufPtr1 =
			    &enh_buf[ENH_BUFL - 1 - iLBCdec_inst->blockl];
			for (i = 0; i < plc_blockl; i++) {
				win += inc;
				*enh_bufPtr1 =
				    (WebRtc_Word16)
				    WEBRTC_SPL_MUL_16_16_RSFT((*enh_bufPtr1),
							      win, 14);
				*enh_bufPtr1 +=
				    (WebRtc_Word16)
				    WEBRTC_SPL_MUL_16_16_RSFT((16384 - win),
							      plc_pred
							      [plc_blockl - 1 -
							       i], 14);
				enh_bufPtr1--;
			...
			enh_bufPtr1 =
			    &enh_buf[ENH_BUFL - iLBCdec_inst->blockl -
				     plc_blockl];
			WEBRTC_SPL_MEMCPY_W16(enh_bufPtr1, plc_pred,
					      plc_blockl);

			/* Clear fileter memory */
			WebRtcSpl_MemSetW16(iLBCdec_inst->syntMem, 0,
					    LPC_FILTERORDER);
			...
			WEBRTC_SPL_MEMCPY_W16(&synt[-LPC_FILTERORDER],
					      iLBCdec_inst->syntMem,
					      LPC_FILTERORDER);
			WebRtcSpl_FilterARFastQ12(enh_bufPtr1, synt,
						  &iLBCdec_inst->
						  old_syntdenum[(iLBCdec_inst->
								 nsub -
								 1) *
								(LPC_FILTERORDER
								 + 1)],
			...
			WEBRTC_SPL_MEMCPY_W16(&synt[-LPC_FILTERORDER],
					      &synt[lag - LPC_FILTERORDER],
					      LPC_FILTERORDER);
			WebRtcIlbcfix_HpOutput(synt,
					       (WebRtc_Word16 *)
					       WebRtcIlbcfix_kHpOutCoefs,
					       iLBCdec_inst->hpimemy,
					       iLBCdec_inst->hpimemx,
					       (WebRtc_Word16) lag);
			WebRtcSpl_FilterARFastQ12(enh_bufPtr1, synt,
						  &iLBCdec_inst->
						  old_syntdenum[(iLBCdec_inst->
			...
			WEBRTC_SPL_MEMCPY_W16(iLBCdec_inst->syntMem,
					      &synt[lag - LPC_FILTERORDER],
					      LPC_FILTERORDER);
			WebRtcIlbcfix_HpOutput(synt,
					       (WebRtc_Word16 *)
	...
	for (iblock = 0; iblock < new_blocks; iblock++) {
		WebRtcIlbcfix_Enhancer(out +
				       WEBRTC_SPL_MUL_16_16(iblock, ENH_BLOCKL),
				       enh_buf, ENH_BUFL,
