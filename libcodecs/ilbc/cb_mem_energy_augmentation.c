...
void WebRtcIlbcfix_CbMemEnergyAugmentation(WebRtc_Word16 * interpSamples,	/* (i) The interpolated samples */
    ...
    )
{
	...
	nrjRecursive =
	    WebRtcSpl_DotProductWithScale(CBmemPtr - 19, CBmemPtr - 19, 15,
					  scale);
	ppe = CBmemPtr - 20;
	...
	for (lagcount = 20; lagcount <= 39; lagcount++) {
		...
		/* interpolation */
		energy +=
		    WebRtcSpl_DotProductWithScale(interpSamplesPtr,
						  interpSamplesPtr, 4, scale);
		interpSamplesPtr += 4;

		/* Compute energy for the remaining samples */
		pp = CBmemPtr - lagcount;
		energy +=
		    WebRtcSpl_DotProductWithScale(pp, pp, SUBL - lagcount,
						  scale);
