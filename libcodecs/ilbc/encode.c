    ...
    )
{
	...
	residual =
	    &iLBCenc_inst->lpc_buffer[LPC_LOOKBACK + BLOCKL_MAX -
				      iLBCenc_inst->blockl];
	...
	if (iLBCenc_inst->section != 0) {
		WEBRTC_SPL_MEMCPY_W16(weightdenum, weightdenumbuf,
				      SCRATCH_ENCODE_DATAVEC -
				      SCRATCH_ENCODE_WEIGHTDENUM);
		/* Un-Packetize the frame into parameters */
		last_bit =
		    WebRtcIlbcfix_UnpackBits(iLBCenc_inst->bytes, iLBCbits_inst,
					     iLBCenc_inst->mode);
		if (last_bit)
			return;
		/* adjust index */
	...
	WebRtcIlbcfix_LpcEncode(syntdenum, weightdenum, iLBCbits_inst->lsf,
				data, iLBCenc_inst);
	...
	for (n = 0; n < iLBCenc_inst->nsub; n++) {
		WebRtcSpl_FilterMAFastQ12(&data[n * SUBL], &residual[n * SUBL],
					  &syntdenum[n * (LPC_FILTERORDER + 1)],
					  LPC_FILTERORDER + 1, SUBL);
	}

	/* Copy the state for next frame */
	WEBRTC_SPL_MEMCPY_W16(iLBCenc_inst->anaMem,
			      &data[iLBCenc_inst->blockl - LPC_FILTERORDER],
			      LPC_FILTERORDER);

	/* find state location */

	iLBCbits_inst->startIdx =
	    WebRtcIlbcfix_FrameClassify(iLBCenc_inst, residual);
	...
	WebRtcIlbcfix_StateSearch(iLBCenc_inst, iLBCbits_inst,
				  &residual[start_pos],
				  &syntdenum[(iLBCbits_inst->startIdx -
					      1) * (LPC_FILTERORDER + 1)],
				  &weightdenum[(iLBCbits_inst->startIdx -
						1) * (LPC_FILTERORDER + 1)]);

	WebRtcIlbcfix_StateConstruct(iLBCbits_inst->idxForMax,
	...
	if (iLBCbits_inst->state_first) {	/* put adaptive part in the end */
		...
		WebRtcIlbcfix_CbSearch(iLBCenc_inst, iLBCbits_inst->cb_index,
				       iLBCbits_inst->gain_index,
				       &residual[start_pos +
						 iLBCenc_inst->state_short_len],
	...
	WebRtcIlbcfix_PackBits(iLBCenc_inst->bytes, iLBCbits_inst,
			       iLBCenc_inst->mode);
...
if (Nfor > 0) {
	...
	WEBRTC_SPL_MEMCPY_W16(mem + CB_MEML - STATE_LEN,
			      decresidual + (iLBCbits_inst->startIdx -
					     1) * SUBL, STATE_LEN);

#ifdef SPLIT_10MS
	if (iLBCenc_inst->Nfor_flag > 0) {
		for (subframe = 0; subframe < WEBRTC_SPL_MIN(Nfor, 2);
		     subframe++) {
			/* update memory */
			WEBRTC_SPL_MEMCPY_W16(mem, mem + SUBL,
					      (CB_MEML - SUBL));
			WEBRTC_SPL_MEMCPY_W16(mem + CB_MEML - SUBL,
					      &decresidual[(iLBCbits_inst->
							    startIdx + 1 +
							    subframe) * SUBL],
					      SUBL);
		}
	...
	for (subframe = start_count; subframe < end_count; subframe++) {
		...
		WebRtcIlbcfix_CbSearch(iLBCenc_inst,
				       iLBCbits_inst->cb_index +
				       subcount * CB_NSTAGES,
				       iLBCbits_inst->gain_index +
				       subcount * CB_NSTAGES,
		...
		WebRtcIlbcfix_CbConstruct(&decresidual
					  [(iLBCbits_inst->startIdx + 1 +
					    subframe) * SUBL],
					  iLBCbits_inst->cb_index +
					  subcount * CB_NSTAGES,
					  iLBCbits_inst->gain_index +
					  subcount * CB_NSTAGES, mem,
					  MEM_LF_TBL, SUBL);
...
if (Nback > 0) {
	...
	WebRtcSpl_MemCpyReversedOrder(&reverseResidual[Nback * SUBL - 1],
				      residual, Nback * SUBL);
	...
	if (iLBCenc_inst->Nback_flag > 0) {
		for (subframe = 0; subframe < WEBRTC_SPL_MAX(2 - Nfor, 0);
		     subframe++) {
			/* update memory */
			WEBRTC_SPL_MEMCPY_W16(mem, mem + SUBL,
					      (CB_MEML - SUBL));
	...
	for (subframe = start_count; subframe < end_count; subframe++) {
		...
		WebRtcIlbcfix_CbSearch(iLBCenc_inst,
				       ...
				       iLBCbits_inst->gain_index +
				       subcount * CB_NSTAGES,
				       &reverseResidual[subframe * SUBL], mem,
				       MEM_LF_TBL, SUBL,
				       &weightdenum[(iLBCbits_inst->startIdx -
						     2 -
						     subframe) *
