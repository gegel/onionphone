...
*/
  ...
  /*-----------------------------------------*/
	...
	swTemp1 = hr_round(pL_R[1]);
	swTemp2 = hr_round(pL_R[0]);
  ...
  /*-------------------*/
	...
	for (i = 0; i <= NP - 2; i++) {
		L_temp = hr_L_mpy_ls(pL_vjOld[i], swRc);
		L_sum = L_add(L_temp, pL_pjOld[i]);
		L_temp = hr_L_mpy_ls(pL_pjOld[i], swRcSq);
  ...
  /*-------------------*/
	...
	for (i = -NP + 2; i <= NP - 2; i++) {
		L_temp = hr_L_mpy_ls(pL_vjOld[-i - 1], swRcSq);
		L_sum = L_add(L_temp, pL_vjOld[i + 1]);
		L_temp =
		    hr_L_mpy_ls(pL_pjOld[(((i + 1) >= 0) ? i + 1 : -(i + 1))],
				swRc);
  ...
  /*-------------------------------------------------*/
    ...
    /*-----------------------------------------*/
		...
		swTemp1 = hr_round(L_shl(pL_vjOld[0], swTemp));	/* normalize num.  */
		swTemp2 = hr_round(L_shl(pL_pjOld[0], swTemp));	/* normalize den.  */
...
*/
	...
	for (siLoopCnt = 1; siLoopCnt <= NP; siLoopCnt++) {
		pL_CorrTemp[siLoopCnt] = hr_L_mpy_ls(pL_CorrTemp[siLoopCnt],
						     psrSST[siLoopCnt]);
	}
...
*/
  ...
  /*-----------------------------------------------*/
	...
	if (L_OutEnergy == 0) {
		...
		snsOutEnergy.man = hr_round(L_OutEnergy);
  ...
  /*-------------------------------------------------------------------*/
    ...
    /*---------------------------------------------------------------*/
      ...
      /*----------------------------------------*/
			...
			for (i = siSampsSoFar;
			     i < siSampsSoFar + siSampsThisPass; i++)
      ...
      /*--------------------------------------------*/
			...
			for (i = siSampsSoFar;
				...
				/* first tap with hr_rounding offset */
	/*--------------------------------*/
				L_Temp = L_mac((long)32768,
					       pswLTPState[i - swIntLag -
							   P_INT_MACS / 2],
...
*/
	...
	if (siSoftInterpolation) {
		...
		if (siStable) {
			...
			for (i = 0; i < NP; i++) {
				L_Temp1 =
				    L_mult(pswNewFrmPFNum[i],
					   psrNewCont[siSfrm]);
				ppswPFNumAs[siSfrm][i] =
				    mac_r(L_Temp1, pswPrevFrmPFNum[i],
					  psrOldCont[siSfrm]);
				L_Temp2 =
				    L_mult(pswNewFrmPFDenom[i],
					   psrNewCont[siSfrm]);
				ppswPFDenomAs[siSfrm][i] =
				    mac_r(L_Temp2, pswPrevFrmPFDenom[i],
					  psrOldCont[siSfrm]);
			}
		} else {
 ...
 *   PURPOSE:
 *
 *     This subroutine calculates IP, the single-resolution lag hr_rounded
 ...
 *
 *     *pswIp
 *                     fractional lag hr_rounded down to nearest integer, IP
 *
 *     *pswJj
 *                     the remainder JJ
 *
 ...
 *     jj = integer_hr_round[((lag/OS_FCTR)-ip)*(OS_FCTR)]
 *          if the hr_rounding caused an 'overflow'
...
*/
	...
	swTemp = hr_round(L_Temp);	/* hr_round and pick-off jj */
	if (sub(swTemp, OS_FCTR) == 0) {	/* if 'overflow ' */
 ...
 *     i=0, i < S_LEN
 *        out[i] = hr_rounded(state[i]*coef[0])
 *        j=1, j < NP
...
*/
	...
	/* ---------------------- */
	/* 0th coef, with hr_rounding */
	L_Sum = L_mac(LPC_ROUND, pswState[0], pswCoef[0]);

	for (siStage = 1; siStage < NP; siStage++) {	/* remaining coefs */
	...
	for (siSmp = 1; siSmp < S_LEN; siSmp++) {
		...
		/* remaining coefs */
		for (siStage = 1; ((0 < (siSmp - siStage)) && siStage < NP);
		     siStage++) {
			L_Sum =
			    L_mac(L_Sum, pswInput[siSmp - siStage - 1],
				  pswCoef[siStage]);
		}
		...
		for (siStage = siSmp; siStage < NP; siStage++) {
			L_Sum =
			    L_mac(L_Sum, pswState[siStage - siSmp],
				  pswCoef[siStage]);
		}
...
*/
	...
	for (siSmp = 1; siSmp < S_LEN; siSmp++) {
		...
		for (siStage = 1; ((0 < (siSmp - siStage)) && siStage < NP);
		     siStage++) {
			L_Sum =
			    L_msu(L_Sum, pswFiltOut[siSmp - siStage - 1],
				  pswCoef[siStage]);
		}
		...
		for (siStage = siSmp; siStage < NP; siStage++) {
			L_Sum =
			    L_msu(L_Sum, pswState[siStage - siSmp],
				  pswCoef[siStage]);
		}
...
*/
	...
	for (siSmp = 1; siSmp < S_LEN; siSmp++) {
		...
		for (siStage = 1; ((0 < (siSmp - siStage)) && siStage < NP);
		     siStage++) {
			L_Sum =
			    L_mac(L_Sum, pswCommonIO[siSmp - siStage - 1],
				  pswCoef[siStage]);
		}
 ...
 *
 *     The ex_p(n-L) sample is interpolated from the surhr_rounding samples,
  ...
  /*---------------------*/
    ...
    /*---------------------------------------*/
      ...
      /*--------------------------------------------------------*/
			...
			snsOrigEnergy.sh =
			    g_corr1s(pswExcite, swEnergy, &L_OrigEnergy);
			snsOrigEnergy.man = hr_round(L_OrigEnergy);
      ...
      /*------------------------------------------------------*/
			...
			for (i = 0; i < S_LEN; i++) {
				...
				L_OrigEnergy =
				    L_mac(L_OrigEnergy, pswExcite[i],
					  pswExcite[i]);
			}

			snsOrigEnergy.sh = norm_l(L_OrigEnergy);
			snsOrigEnergy.man =
			    hr_round(L_shl(L_OrigEnergy, snsOrigEnergy.sh));
		}
    ...
    /*-------------------------------------------------------------------*/
		...
		for (i = 0; i < P_INT_MACS; i++)
			pswInterpCoefs[i] =
			    mult_r(ppsrPVecIntFilt[i][swRemain], swScale);
...
*/
  ...
  /*-------------------------------------------------------------------*/
	...
	for (i = 0; i < NP; i++) {
		...
		for (j = 0; j <= i - 1; j++) {
			L_temp = hr_L_mpy_ls(pL_A[i - j - 1], pswRc[i]);
...
*/
  ...
  /*------------------------------------*/
    ...
    /*---------------------------------------------------------------------*/
		...
		L_temp = hr_L_mpy_ll(pL_A[0], pL_R[i]);
		L_sum = L_negate(L_temp);

		for (j = 1; j <= i; j++) {
			L_temp = hr_L_mpy_ll(pL_A[j], pL_R[i - j]);
...
*/
  ...
  /*----------------------------------------------------------------*/
    ...
    /*---------------------------------------------------------*/
		...
		/* Normalize partial product, hr_round */
    /*----------------------------------*/

		swShift = norm_s(extract_h(L_Product));
		swPartialProduct = hr_round(L_shl(L_Product, swShift));
  ...
  /*----------------------------------------------------------------*/
	...
	swSqrtResEng = hr_round(L_shl(L_SqrtResEng, swShift));
...
*/
    ...
    /*--------------------------------------------------------*/
		...
		swNormShift =
		    g_corr1s(pswExcitation, negate(swEnergy), &L_Temp);
	...
	/* ---------------------------- */
	swTemp2 = divide_s(swTemp, hr_round(L_Temp));
 ...
 *              gain's shift was > 0 or the hr_rounded vector gain otherwise.
 *
 *   DESCRIPTION:
 *
 *     If gain > 1.0 then
 *         (do not shift gain up yet)
 *         partially scale vector element THEN shift and hr_round save
 *      else
 *         shift gain correctly
 *         scale vector element and hr_round save
 *         update state array
 *
...
*/
	...
	/* ---------- */
		...
		/* the hr_rounded actual vector gain */
		/* ------------------------------ */
		swGain = hr_round(L_GainUs);

		/* now scale the vector (with hr_rounding) */
		/* ------------------------------------ */
...
*/
	...
	static int16_t
	    * pswLtpStateOut = &pswLtpStateBaseDec[LTP_LEN],
	    ...
	    pswPFDenomAsSpace[NP * N_SUB], *ppswSynthAs[N_SUB] = {
		&pswSythAsSpace[0],
		&pswSythAsSpace[10],
		&pswSythAsSpace[20],
		&pswSythAsSpace[30],
	...
	&pswPFNumAsSpace[0],
		    &pswPFNumAsSpace[10],
		    &pswPFNumAsSpace[20],
		    &pswPFNumAsSpace[30],}, *ppswPFDenomAs[N_SUB] = {
	&pswPFDenomAsSpace[0],
...
*/
	...
	int16_t pswFrmKs[NP],
	    ...
	    swR0Index, swLag, swSemiBeta, pswBitArray[MAXBITS];
...
*/
	...
	if (swDecoMode == SPEECH) {
		...
		for (i = 0; i < N_SUB; i++) {
			pL_RxGsHist[swRxGsHistPtr] =
			    ppLr_gsTable[swVoicingMode][pswParameters
							[(i * 3) + 8]];
			swRxGsHistPtr++;
    ...
    /*----------------------------- */
		...
		} else if (swDecoMode == CNICONT) {	/* SID frame detected, but *//* not the first SID       */
			swDtxBfiCnt = 0;
			swDtxMuting = 0;
		...
		} else if (swDecoMode == CNIBFI) {	/* bad frame received in *//* CNI mode              */
			if (swRxDTXState < (CNINTPER - 1))
				swRxDTXState = add(swRxDTXState, 1);
			swR0Index = swOldR0IndexDec;
	...
	if (!rcToADp(ASCALE, pswFrmKs, pswFrmAs)) {
		...
		for (i = 0; i < NP; i++) {
			pswFrmPFDenom[i] =
			    mult_r(pswFrmAs[i], psrSPFDenomWidenCf[i]);
		}
	...
	for (giSfrmCnt = 0; giSfrmCnt < 4; giSfrmCnt++) {
		...
		if (sub(swVoicingMode, 0) == 0) {	/* unvoiced */
		...
		if (swVoicingMode) {	/* voiced */
			...
			v_con(pppsrVcdCodeVec[0][0], ppswVselpEx[0],
		...
		if (!swVoicingMode) {
			rs_rrNs(ppswVselpEx[1], psnsSqrtRs[giSfrmCnt],
				&snsRs22);
		}
		...
		if (swVoicingMode) {	/* voiced */
			...
			swSemiBeta = scaleExcite(pswPVec,
						 pppsrGsp0[swVoicingMode]
						 [siGsp0Code][0], snsRs00,
						 pswPVec);
			...
			for (i = 0; i < S_LEN; i++) {
				pswExcite[i] =
				    add(ppswVselpEx[1][i], ppswVselpEx[0][i]);
			}
		}
		...
		signal_conceal_sub(pswPPFExcit, ppswSynthAs[giSfrmCnt],
				   pswSynthFiltState, &pswLtpStateOut[-S_LEN],
				   &pswPPreState[LTP_LEN - S_LEN], swLevelMean,
				   swLevelMax, pswParameters[19], swMuteFlagOld,
 ...
 *
 *      The algorithm is based ahr_round a six term Taylor expansion :
 *
 *        y^0.5 = (1+x)^0.5
...
*/
	...
	swTemp4 = hr_round(L_Temp0);	/* swTemp4 = (x/2)^4 */

	/* determine partial 6th term */
	/* -------------------------- */

	L_Temp0 = L_msu(0L, swTemp2, swTemp3);	/* L_Temp0 = (x/2)^5 */
	swTemp2 = hr_round(L_Temp0);	/* swTemp2 = (x/2)^5 */
