...
*/
  ...
  /*-----------------------------------------------------*/
	...
	swLTPEnergy = hr_round(L_Energy);

	if (sub(swLTPEnergy, 0x07ff) > 0) {	/* E = (0x7ff.. 0x7fff] */
    ...
    /*-------------------------------------------------------------*/
		...
		for (j = 1; j < S_LEN; j++)
			L_Energy =
			    L_mac(L_Energy, ppswTVect[i][j], ppswTVect[i][j]);
		...
		for (j = 1; j < S_LEN; j++)
			L_CrossCorr =
			    L_mac(L_CrossCorr, ppswTVect[i][j], pswPVect[j]);
	...
	for (i = 0; i < iNumLags; i++) {
		pswCCBuf[i] = hr_round(L_shl(pL_CCBuf[i], swCCShiftCnt));
		pswCGBuf[i] = hr_round(L_shl(pL_CGBuf[i], swCGShiftCnt));
	}

	/* Maximize C**2/G */
	/* --------------- */
	siLagOffset = maxCCOverGWithSign(pswCCBuf, pswCGBuf,
					 &swCCMax, &swGMax, iNumLags);
 ...
 */
  ...
  /*---------------------*/
	...
	for (iLoopCnt = 0; iLoopCnt < iNumVects; iLoopCnt++) {
		...
		swTemp = divide_s(hr_round(L_Temp1), swNorm_energy);
		...
		for (i = 0; i < S_LEN; i++) {
			L_Accum =
			    L_msu(0x00008000L, pswVects[i + iLoopCnt * S_LEN],
				  SW_MIN);
			pswVects[iLoopCnt * S_LEN + i] =
			    extract_h(L_mac
				      (L_Accum, swTemp,
...
*/
	...
	if (swUVCode == 0) {
		...
		ErrorTerm[0].man = hr_round(L_shl(L_Temp, swShift));
		...
		ErrorTerm[1].man = hr_round(L_shl(L_Temp, swShift));
		...
		ErrorTerm[2].man = hr_round(L_shl(L_Temp, swShift));
		...
		ErrorTerm[3].man = hr_round(L_shl(L_Temp, swShift));
		L_Temp = L_mult(ErrorTerm[3].man, snsRs11.man);
		swShift = norm_s(extract_h(L_Temp));
		ErrorTerm[3].sh = add(ErrorTerm[3].sh, swShift);
		ErrorTerm[3].man = hr_round(L_shl(L_Temp, swShift));
		...
		ErrorTerm[4].man = hr_round(L_shl(L_Temp, swShift));
		L_Temp = L_mult(ErrorTerm[4].man, snsRs22.man);
		swShift = norm_s(extract_h(L_Temp));
		ErrorTerm[4].sh = add(ErrorTerm[4].sh, swShift);
		ErrorTerm[4].man = hr_round(L_shl(L_Temp, swShift));
	...
	for (i = 0; i < GSP0_VECTOR_SIZE; i++) {
		...
		ErrorTerm[i].man = hr_round(L_Temp);
	}
	...
	for (i = 0; i < GSP0_NUM; i++) {
		...
		L_Temp =
		    L_mac(L_Temp, pppsrGsp0[swUVCode][i][1], ErrorTerm[1].man);
		L_Temp =
		    L_mac(L_Temp, pppsrGsp0[swUVCode][i][2], ErrorTerm[2].man);
		L_Temp =
		    L_mac(L_Temp, pppsrGsp0[swUVCode][i][3], ErrorTerm[3].man);
		L_Temp =
		    L_mac(L_Temp, pppsrGsp0[swUVCode][i][4], ErrorTerm[4].man);
...
*/
	...
	terms[0].man = hr_round(L_shl(L_Temp, swShift));
	...
	terms[1].man = hr_round(L_shl(L_Temp, swShift));
	...
	terms[2].man = hr_round(L_shl(L_Temp, swShift));
	...
	terms[3].man = hr_round(L_shl(L_Temp, swShift));
	...
	terms[4].man = hr_round(L_shl(L_Temp, swShift));
  ...
  /*----------------------------------------------------------------------*/
	...
	swShift = norm_s(extract_h(L_Temp));
	siNormShift = add(siNormShift, swShift);
	swGainTweak = hr_round(L_shl(L_Temp, swShift));
	...
	psErrorTerm[0].man = hr_round(L_shl(L_Temp, swShift));
	...
	psErrorTerm[1].man = hr_round(L_shl(L_Temp, swShift));
...
*/
    ...
    /*------------------------------------------------------------------*/
		...
		for (i = 0; i < iNumSamples; i++) {
			...
			/* get input with hr_rounding */
      /*-------------------------*/
			L_temp =
			    L_mac((long)16384, pswInSample[S_LEN - i - 1],
				  0x4000);
	...
	/*-----------------------------------------*/
				L_temp =
				    L_mac(L_temp,
					  pswInSample[iStatIndx - i + j],
					  pswInCoef[j]);

			pswOutSample[S_LEN - 1 - i] =
			    extract_h(L_shl(L_temp, 1));
		}
	} else {
		for (i = 0; i < iNumSamples; i++) {
	...
	/*----------------------------------------*/
				L_temp =
				    L_mac(L_temp, pswState[iStatIndx1 - i + j],
					  pswInCoef[j]);
...
*/
	...
	memzero(&snsRs00, sizeof(struct NormSw));

	pswLtpStateOut = pswLtpStateBase + LTP_LEN;

	if (swSP == 1) {	/* DTX mode *//* DTX mode */
    ...
    /*---------------------------------------------------------------------*/
		...
		swPnEnergy = hr_round(L_PnEnergy);

		if (sub(swPnEnergy, 0x7ff) <= 0) {	/* E = [0..0x7ff] */

			swPnShift = 0;

		} else {

			if (sub(swPnEnergy, 0x1fff) <= 0) {	/* E = (0x7ff.. 0x1fff] */
				swPnShift = 1;
    ...
    /*------------------------------------------*/
      ...
      /*-------------------------------------------------------------*/
			...
			hnwFilt(pswWSpeech, pswWSVec, &pswWSpeech[-1],
				pswHNWCoefs, siHnwOffset, 1, S_LEN);

			hnwFilt(pswTempVec, pswTempVec,
				&pswHNWState[HNW_BUFF_LEN - 1], pswHNWCoefs,
				siHnwOffset, 1, S_LEN);

			for (i = 0; i < S_LEN; i++) {

				pswWSVec[i] =
				    shr(sub(pswWSVec[i], pswTempVec[i]),
					swPnShift);
      ...
      /*------------------------------------------------------------*/
			...
			if (siHnwNum > 0) {
				hnwFilt(pswWPVec, pswWPVec, NULL, pswHNWCoefs,
    ...
    /*----------------------------------------------------------------*/
		...
		for (i = 0; i < siNumBasisVecs; i++) {
			...
			if (siHnwNum > 0) {
				...
				hnwFilt(&pswWBasisVecs[i * S_LEN],
					&pswWBasisVecs[i * S_LEN], NULL,
					pswHNWCoefs, siHnwOffset, 0, siHnwNum);
			}
		}
    ...
    /*--------------------------------------------------------------------*/
      ...
      /*-------------------------------------------------*/
			...
			v_con(pswWBasisVecs, ppswWVselpEx[0], pswBitArray,
			      siNumBasisVecs);
      ...
      /*---------------------------------------------------------*/
			...
			*psiVSCode2 =
			    v_srch(pswWSVec, pswWBasisVecs, siNumBasisVecs);
    ...
    /*----------------------------------------------------------------*/
		...
		if (siHnwNum > 0) {
			hnwFilt(ppswWVselpEx[0], ppswWVselpEx[0], NULL,
				pswHNWCoefs, siHnwOffset, 0, siHnwNum);
		}
    ...
    /*------------------------------------------------------------------*/
		...
		*psiGsp0Code = g_quant_vl(swVoicingMode, pswWSVec, swPnShift,
					  pswWPVec,
					  ppswWVselpEx[0], ppswWVselpEx[1],
					  snsRs00, snsRs11, snsRs22);
  ...
  /*-----------------------------------------------------------------*/
	...
	if (swVoicingMode == 0) {
		...
		for (i = 0; i < S_LEN; i++)
			pswTempVec[i] =
			    add(ppswVselpEx[0][i], ppswVselpEx[1][i]);
	}
...
*/
	...
	0x01, 0x02, 0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x05, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x06, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x05, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x07, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x05, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x06, 0x01, 0x02,
		    ...
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x05, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x00, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x05, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x06, 0x01, 0x02,
		    ...
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x05, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01,}, pswModNextBitV[C_BITS_V] = {
	1, 2, 3, 4, 5, 6, 7, 0, 1,}, pswUpdateIndexUn[UN_ARRAY_SIZE] = {
	0x00, 0x07, 0x2a, 0x0e, 0x00, 0x31, 0x2a, 0x15, 0x00, 0x07,
		    0x2a, 0x38, 0x00, 0x31, 0x2a, 0x1c, 0x00, 0x07,
		    0x2a, 0x0e, 0x00, 0x31, 0x2a, 0x3f, 0x00, 0x07,
		    0x2a, 0x38, 0x00, 0x31, 0x2a, 0x23, 0x00, 0x07,
		    0x2a, 0x0e, 0x00, 0x31, 0x2a, 0x15, 0x00, 0x07,
		    0x2a, 0x38, 0x00, 0x31, 0x2a, 0x46, 0x00, 0x07,
		    0x2a, 0x0e, 0x00, 0x31, 0x2a, 0x3f, 0x00, 0x07,
		    0x2a, 0x38, 0x00, 0x31, 0x2a,},
	...
	0x01, 0x02, 0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    ...
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x05, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01, 0x04, 0x01, 0x02,
		    0x01, 0x03, 0x01, 0x02, 0x01,}, pswModNextBitUV[C_BITS_UV] =
	{
	1, 2, 3, 4, 5, 0, 1,};
...
*/
	...
	for (siI = 0; siI < siNumBasis; siI++) {
		...
		for (siJ = 1; siJ < S_LEN; siJ++) {
			L_R =
			    L_mac(L_R, pswWBasisVecs[siJ + (siI * S_LEN)],
				  pswWInput[siJ]);
		}
		pL_R[siI] = L_R;
		L_R = L_abs(L_R);
	...
	for (siI = 0; siI < siNumBasis - 1; siI++) {
		...
		for (siJ = siI + 1; siJ < siNumBasis; siJ++) {
			...
			L_D =
			    L_mult(pswWBasisVecs[siI * S_LEN],
				   pswWBasisVecs[siJ * S_LEN]);

			for (siK = 1; siK < S_LEN; siK++) {
				L_D =
				    L_mac(L_D,
					  pswWBasisVecs[siK + (siI * S_LEN)],
	...
	for (siI = 0; siI < siNumBasis - 1; siI++) {
		...
		for (siJ = siI + 1; siJ < siNumBasis; siJ++) {
			...
			L_D =
			    L_add(L_D,
				  (LSP_MASK &
				   L_deposit_l(pppswDubD[siI][siJ][1])));
	...
	for (siI = 0; siI < siEBits; siI++) {
		pswCGUpdates[siI * (siEBits + 1)] = hr_round(pL_R[siI]);
	}
