...
*/
	...
	*pswMutePermit = 0;
	...
	/* --------------------------------------------------------- */
		 ...
		 * the last good frame.               */
		/* -------------------------------------------------------------- */
		if (pswLastGood[5] == 0) {	/* unvoiced good frame */
			if (pswSpeechPara[5] == 0) {	/* unvoiced bad frame */
				for (i = 0; i < 5; i++)
					pswSpeechPara[i] = pswLastGood[i];
				for (i = 0; i < 4; i++)
					pswSpeechPara[3 * i + 8] =
					    pswLastGood[17];
			} else {	/* voiced bad frame */
				for (i = 0; i < 18; i++)
					pswSpeechPara[i] = pswLastGood[i];
				for (i = 0; i < 3; i++)
					pswSpeechPara[3 * i + 8] =
		 ...
		 * frame.                              */
		/* ---------------------------------------------------------------- */
		else {		/* voiced good frame */
			swLastLag = pswLastGood[6];	/* frame lag */
			for (i = 0; i < 3; i++) {	/* each delta lag */
				swLag = sub(pswLastGood[3 * i + 9], 0x8);	/* biased ahr_round 0 */
				swLag = add(swLag, swLastLag);	/* reconstruct pitch */
				if (sub(swLag, 0x00ff) > 0) {	/* limit, as needed */
					swLastLag = 0x00ff;
				} else if (swLag < 0) {
					swLastLag = 0;
				} else
			...
			if (pswSpeechPara[5] != 0) {	/* voiced bad frame */
				for (i = 0; i < 6; i++)
					pswSpeechPara[i] = pswLastGood[i];
				for (i = 0; i < 4; i++)
					pswSpeechPara[3 * i + 6] =
...
*/
  ...
  /*-----------------------------------------------------------------------*/
	...
	if (swPermitMuteSub > 0) {
		...
		swMute =
		    sub(swLevelSub,
			add(swLevelMax, psrLevelMaxIncrease[swIndex]));
		if (swMute <= 0)
			swPermitMuteSub = 0;
	}
	...
	if (swPermitMuteSub > 0) {
		...
		for (i = 0; i < 10; ++i)
			pswSynthFiltState[i] =
			    mult_r(pswSynthFiltState[i],
				   psrConceal[swMute - 1]);
		for (i = 0; i < S_LEN; ++i)
			pswPPFExcit[i] =
			    mult_r(pswPPFExcit[i], psrConceal[swMute - 1]);
		...
		for (i = 0; i < S_LEN; ++i)
			pswPPreState[i] =
			    mult_r(pswPPreState[i], psrConceal[swMute - 1]);
	}
}
