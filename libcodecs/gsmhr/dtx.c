...
*/
	...
	if (swVadFrmCnt) {
		...
		if (swVadFrmCnt >= OVERHANG) {
			...
			lpcCorrQntz(pL_RefCorr, pswFinalRc, piRefVqCodewds);
...
*/
	...
	for (i = 1; i < N_SUB * (OVERHANG - 1); i++)
		L_avg =
		    L_add(L_shift_r(pL_GsHistory[i], -(OH_SHIFT + 2)), L_avg);

	/* avg number x/32 not x/28 */

	*pL_GsAvgd = L_add(L_avg, hr_L_mpy_ls(L_avg, 0x1249));	/* L_avg *= 32/28 */
...
*/
	...
	for (swGsIndex = 0; swGsIndex < 32; swGsIndex++) {
		L_diff =
		    L_abs(L_sub
			  (L_GsIn, ppLr_gsTable[swVoicingMode][swGsIndex]));
...
*/
	...
	*pL_AvgdR0 = L_avg;
	...
	for (j = 0; j < NP + 1; j++) {
		for (L_avg = 0, i = 0; i < OVERHANG; i++) {
			L_avg =
			    L_add(L_shift_r(ppL_CorrHistory[i][j], -OH_SHIFT),
				  L_avg);
		}
  ...
  /*--------------------------------------------------------------------*/
    ...
    /*---------------------------------*/
      ...
      /*---------------------------*/
			...
			quantList.iRCIndex =
			    bestPql[iVec].iRCIndex * psrQuantSz[iSeg - 1];
    ...
    /*--------------------------------------------------------------*/
		...
		if (iSeg < LPC_VQ_SEG)
			aflatNewBarRecursionL(&pswFinalRc
					      [psvqIndex[iSeg - 1].l - 1], iSeg,
					      pL_PBarFull, pL_VBarFull, pswPBar,
					      pswVBar);
...
*/
	...
	if (swDecoMode == CNIFIRSTSID) {
		...
		for (i = 0; i < NP + 1; i++) {
			pL_CorrSeq[i] =
			    linInterpSid(pL_NewCorrSeq[i], pL_OldCorrSeq[i],
					 swRxDTXState);
		}
...
*/
	...
	L_New = hr_L_mpy_ls(L_New, psrCNNewFactor[swDtxState]);
	L_Old = hr_L_mpy_ls(L_Old, swOldFactor);
...
*/
	...
	return (hr_round(L_add(L_New, L_Old)));
