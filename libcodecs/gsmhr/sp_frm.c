...
*/
  ...
  /*---------------------------------------------------------------------*/
	...
	*pswSP = swComfortNoise(*pswVadFlag,	/* DTX mode */
	...
	if (*pswSP == 0) {	/* DTX mode *//* SID frame generation *//* DTX mode */
	...
	/*---------------------------------------*/
				...
				quantList.pswPredErr[iCnt] =
				    aflatRecursion(&pswRc
						   [psvqIndex[iSeg - 1].l],
						   pswPBar, pswVBar, ppswPAddrs,
						   ppswVAddrs,
	...
	/*---------------------------*/
				...
				quantList.iRCIndex =
				    bestPql[iVec].iRCIndex * psrQuantSz[iSeg -
									1];
	...
	/*----------------------------------------------------------*/
      ...
      /*--------------------------------------------------------------*/
			...
			if (iSeg < LPC_VQ_SEG) {
				...
				aflatNewBarRecursionL(&pswFinalRc
						      [psvqIndex[iSeg - 1].l -
						       1], iSeg, pL_PBarFull,
						      pL_VBarFull, pswPBar,
						      pswVBar);
...
*/
  ...
  /*-------------------------------------------------------------------*/
    ...
    /*-------------------*/
		...
		L_temp = hr_L_mpy_ls(pL_VOld[0], pswQntRc[j]);
		L_sum = L_add(L_temp, pL_POld[0]);
		L_temp = hr_L_mpy_ls(pL_POld[0], swQntRcSq);
    ...
    /*---------------------------------------------------*/
		...
		for (i = 1; i <= bound; i++) {
			L_temp = hr_L_mpy_ls(pL_VOld[i], pswQntRc[j]);
			L_sum = L_add(L_temp, pL_POld[i]);
			L_temp = hr_L_mpy_ls(pL_POld[i], swQntRcSq);
			L_sum = L_add(L_temp, L_sum);
			L_temp = hr_L_mpy_ls(pL_VOld[-i], pswQntRc[j]);
			L_temp = L_add(L_sum, L_temp);
    ...
    /*-------------------*/
		...
		for (i = -bound; i < 0; i++) {
			L_temp = hr_L_mpy_ls(pL_VOld[-i - 1], swQntRcSq);
			L_sum = L_add(L_temp, pL_VOld[i + 1]);
			L_temp = hr_L_mpy_ls(pL_POld[-i - 1], pswQntRc[j]);
  ...
  /*----------------------------------------------------------------------*/
	...
	for (i = 0; i <= bound; i++) {
		pswPBar[i] = hr_round(pL_PBar[i]);
		pswVBar[i] = hr_round(pL_VBar[i]);
	}
	for (i = -bound; i < 0; i++) {
		pswVBar[i] = hr_round(pL_VBar[i]);
	}
...
*/
	...
	Shortword pswCBuf[DELTA_LEVELS + CG_INT_MACS + 2],
	    ...
	    *psw1, *psw2, swCmaxSqr, swGmax, swPeak;
	short int siIPLo, siRemLo, siIPHi, siRemHi, siLoLag, siHiLag, siI;
...
*/
    ...
    /*--------------------------*/
		...
		for (i = 1; i <= CG_INT_MACS - 1; i++) {
			L_Temp1 =
			    L_mac(L_Temp1, pswCIn[i + swLagInt - swLoIntLag],
				  ppsrCGIntFilt[i][swLagRem]);
			L_Temp2 =
			    L_mac(L_Temp2, pswGIn[i + swLagInt - swLoIntLag],
				  ppsrCGIntFilt[i][swLagRem]);
...
*/
	...
	CGInterp(pswLOut, siNum, pswCIn, pswGIn, LSMIN, pswCOut, pswGOut);
...
*/
  ...
  /*----------------------------------------------------------------*/
	...
	for (i = 0; i < N_SUB; i++) {
		...
		if (sub(psnsSqrtRs[i].sh, swMinShift) < 0
		    && psnsSqrtRs[i].man > 0)
	...
	for (i = 0; i < N_SUB; i++) {
		...
		if (siOverflowPossible) {
			...
			for (j = 0; j < S_LEN; j++) {
				...
				swSample =
				    mult_r(swShiftFactor, pswResidual[j]);
				L_ResidualEng =
				    L_mac(L_ResidualEng, swSample, swSample);
			}
		}
			...
			for (j = 0; j < S_LEN; j++) {
				...
				L_ResidualEng =
				    L_mac(L_ResidualEng, pswResidual[j],
					  pswResidual[j]);
			}
		}
	}
		...
		if (siOverflowPossible) {
			...
			for (j = 0; j < S_LEN; j++) {
				...
				swSample =
				    mult_r(swShiftFactor, pswResidual[j]);
				L_ResidualEng =
				    L_msu(L_ResidualEng, swSample, swSample);
			}
		}
			...
			for (j = 0; j < S_LEN; j++) {
				...
				L_ResidualEng =
				    L_msu(L_ResidualEng, pswResidual[j],
					  pswResidual[j]);
			}
		}
	}
...
*/
  ...
  /*-----------------------------------------------------------------*/
    ...
    /*------------------------------*/
		...
		if (k < 9) {
			...
			for (kk = 0; kk < k; kk++) {
				pL_VadAcf[k] =
				    L_msu(pL_VadAcf[k], pswInScale[NP + kk],
					  pswInScale[NP + kk - k]);
			}
		}

		L_temp = L_shl(L_Pwr, swNorm);	/* Normalize the result */
		L_temp = hr_L_mpy_ll(L_temp, pL_rFlatSstCoefs[k - 1]);	/* Apply SST */
  ...
  /*-------------------------------*/
	...
	L_temp = L_shl(L_Pwr, swNorm);	/* Normalize the result */
	L_temp = hr_L_mpy_ll(L_temp, pL_rFlatSstCoefs[NP - 1]);	/* Apply SST */
...
*/
  ...
  /*-------------------------------*/
	...
	iir_d(&pswCoeff[0], pswIn, &pswXstate[0], &pswYstate[0], npts, shifts,
	      1, 0);

	/* Do second second order section */
  /*--------------------------------*/

	iir_d(&pswCoeff[5], pswIn, &pswXstate[2], &pswYstate[4], npts, shifts,
	      0, 1);
...
*/
	...
	for (quantIndex = 0; quantIndex < psqlInList.iNum; quantIndex++) {
		...
		if (bstIndex < iNumVectOut) {	/* a new value is found */
 ...
 *     the search for a peak is done on the surhr_rounding
 *     integer lags.
 *
 *   INPUTS:
 *
...
*/
	...
	/* ---------------------------- */
		...
		if ((sub(siPeak, siLowerBound) == 0) ||
			...
			swFullResPeak =
			    shr(extract_l(L_mult(siPeak, OS_FCTR)), 1);
		}
	}
...
*/
	...
	Longword
	    ...
	    pppL_C[NP][NP][2],
	    L_Num,
	    L_TmpA, L_TmpB, L_temp, L_sum, L_R0, L_Fik, L_Bik, L_Cik, L_Cki;
	short int i, j, k, l, j_0, j_1;
  ...
  /*---------------------------------------------*/
    ...
    /*-----------------------------------------------------------------*/
		...
		L_Num = L_add(L_shl(pppL_C[0][0][j_1], swShift),
			      L_shl(pppL_C[NP - j - 1][NP - j - 1][j_1],
				    swShift));
    ...
    /*-------------------------------------------------------------------*/
		...
		L_TmpA =
		    L_add(L_shl(pppL_F[NP - j - 1][NP - j - 1][j_1], swShift),
    ...
    /*---------------------------------------------------*/
		...
		swNum = hr_round(L_Num);
		swDen = hr_round(L_sum);
    ...
    /*-------------------------------------------------------------*/
      ...
      /*---------------------------------------------------------------*/
			...
			L_TmpA = hr_L_mpy_ls(L_TmpA, pswRc[j]);
      ...
      /*--------------------*/
			...
			L_temp = hr_L_mpy_ls(L_Bik, swRcSq);
	...
	/*---------------------------------------------------------------*/
				...
				L_TmpB = hr_L_mpy_ls(L_TmpB, pswRc[j]);
	...
	/*---------------------------------*/
				...
				L_temp = hr_L_mpy_ls(L_Cki, swRcSq);
	...
	/*-----------------------------------*/
				...
				L_temp = hr_L_mpy_ls(L_Fik, swRcSq);
				L_temp = L_add(L_temp, L_Bik);
				pppL_B[i - 1][k - 1][j_0] =
				    L_add(L_temp, L_TmpA);

				L_temp = hr_L_mpy_ls(L_Cik, swRcSq);
				L_temp = L_add(L_temp, L_Cik);
				pppL_C[i][k - 1][j_0] = L_add(L_temp, L_TmpB);
	  ...
	  /*---------------------------------------------------------------*/
					...
					L_Cik =
					    L_shl(pppL_C[i][k][j_1], swShift);
					L_Cki =
					    L_shl(pppL_C[k][i][j_1], swShift);

					L_TmpA = L_add(L_Cik, L_Cki);
					L_TmpA = hr_L_mpy_ls(L_TmpA, pswRc[j]);

					L_Bik =
					    L_shl(pppL_B[i][k][j_1], swShift);
					L_Fik =
					    L_shl(pppL_F[i][k][j_1], swShift);
...
*/
	...
	for (siLoopCnt = 0; siLoopCnt < siNumPairs; siLoopCnt++) {
		...
		if ((pswGframe[siLoopCnt] > 0) && (pswCframe[siLoopCnt] > 0)) {
			...
			swTemp =
			    mult_r(pswCframe[siLoopCnt], pswCframe[siLoopCnt]);
			...
			L_Temp2 =
			    L_msu(L_Temp2, swCmaxSqr, pswGframe[siLoopCnt]);
...
*/
	...
	for (k = 1; k <= NP; k++) {
		...
		for (siNum = S_LEN - k, iLoopCnt2 = 1; iLoopCnt2 < siNum;
		     iLoopCnt2++) {
			L_Temp =
			    L_mac(L_Temp, pswVecTmp[iLoopCnt2],
...
*/
  ...
  /*------------------------------------------------------------------*/
	...
	siStable = interpolateCheck(pswPrevFrmKs, pswPrevFrmAs,
				    ...
				    swPrevR0,
				    &psnsSqrtRs[siSfrm], ppswSynthAs[siSfrm]);
	if (siStable) {
		for (i = 0; i < NP; i++) {
			L_Temp =
			    L_mult(pswNewFrmSNWCoef[i], psrNewCont[siSfrm]);
			ppswSNWCoefAs[siSfrm][i] =
			    mac_r(L_Temp, pswPrevFrmSNWCoef[i],
				  psrOldCont[siSfrm]);
		}
	} else {
  ...
  /*-----------------------------------*/
	...
	for (siSfrm = 1; siSfrm < N_SUB - 1; siSfrm++) {
		siStable = interpolateCheck(pswNewFrmKs, pswNewFrmAs,
					    pswPrevFrmAs, pswNewFrmAs,
					    psrOldCont[siSfrm],
					    psrNewCont[siSfrm], swNewR0,
					    &psnsSqrtRs[siSfrm],
					    ppswSynthAs[siSfrm]);
		if (siStable) {
			for (i = 0; i < NP; i++) {
				L_Temp =
				    L_mult(pswNewFrmSNWCoef[i],
					   psrNewCont[siSfrm]);
				ppswSNWCoefAs[siSfrm][i] =
				    mac_r(L_Temp, pswPrevFrmSNWCoef[i],
					  psrOldCont[siSfrm]);
...
*/
  ...
  /*------------------------------------------*/
    ...
    /*---------------------------------------------------------------*/
		...
		pswIn[loop_cnt] = hr_round(L_shl(L_sumA, swFinalUpShift));
    ...
    /*---------------------------------------------------------------*/
		...
		pswIn[loop_cnt + 1] = hr_round(L_shl(L_sumA, swFinalUpShift));
...
*/
  ...
  /*--------------------------------------------------------------------*/
	...
	for (i = 0; i <= bound; i++) {
		pswPBar[i] = hr_round(pL_PBarFull[i]);
	}
	for (i = -bound; i < 0; i++) {
	...
	pswVBar[bound] = hr_round(pL_PBarFull[bound + 1]);
...
*/
  ...
  /*-------------------------------------------------------------*/
	...
	for (i = 0; i < W_F_BUFF_LEN; i++)
		pswScaledWSpeech[i - LSMAX] =
		    shl(pswWSpeech[i - LSMAX], siShift);
  ...
  /*--------------------------------------------------------------------*/
	...
	for (i = F_LEN - S_LEN; i < F_LEN; i++)
		L_WSfrmEng =
		    L_mac(L_WSfrmEng, pswScaledWSpeech[i], pswScaledWSpeech[i]);
	...
	for (i = 0; i < N_SUB; i++) {
		...
		for (j = LSMIN; j <= LSMAX; j++) {
			...
			for (k = 0; k < S_LEN; k++) {
				...
				L_C =
				    L_mac(L_C, pswScaledWSpeech[i * S_LEN + k],
  ...
  /*-------------------------------------------------------------------*/
    ...
    /*----------------------------------------------*/
      ...
      /*-------------------------------------------------*/
			...
			pswCCThresh[i] =
			    getCCThreshold(pswSfrmEng[i], swCCMax, swGMax);
		} else
			pswCCThresh[i] = 0;
	}
  ...
  /*-------------------------------*/
	...
	/*--------------------------------------------------------*/
				...
				ppswTrajLBuf[siTrajIndex][siAnchorIndex] =
				    pswLPeaks[siPeakIndex];
				ppswTrajGBuf[siTrajIndex][siAnchorIndex] =
				    pswGPeaks[siPeakIndex];
				ppswTrajCCBuf[siTrajIndex][siAnchorIndex] =
				    mult_r(pswCPeaks[siPeakIndex],
	...
	/*----------------------------------------------------------*/
				...
				for (siFIndex = siAnchorIndex + 1;
				     siFIndex < N_SUB; siFIndex++) {
	  ...
	  /*--------------------------------------------*/
					...
					quantLag(ppswTrajLBuf[siTrajIndex]
						 [siFIndex - 1], &si1);
					si2 =
					    add(si1,
						(DELTA_LEVELS / 2 - 1) -
	...
	/*-------------------------------------------------------*/
	  ...
	  /*----------------------------------------------------------*/
					...
					*psiUVCode = siLowestSoFar;
					...
					L_Voicing = L_deposit_h(swCC);
					L_Voicing =
					    L_mac(L_Voicing, swRG, UV_SCALE1);
      ...
      /*-----------------------------------------------------*/
			...
			for (i = 0; i < N_SUB; i++) {
				...
				quantLag(ppswTrajLBuf[siBestTrajIndex][i],
					 &si1);
				...
				for (j = siLagsSoFar;
				     j < siLagsSoFar + pswNumLagList[i]; j++)
...
*/
  ...
  /*--------------------------------------------------*/
    ...
    /*------------------------------------------------------------*/
      ...
      /*-----------------------------------------------------------*/
			...
			sw2 = 0x4000;
			siPeakIndex =
			    fnBest_CG(pswCBuf, pswGBuf, &sw1, &sw2, siNum);
			if (siPeakIndex != -1) {
      ...
      /*----------------------*/
			swSubMult =
			    mult_r(pswLPeaks[0], psrSubMultFactor[siSMFIndex]);
	...
	/* lags ahr_round which to search for peak.  Note that these lags are   */
  ...
  /*------------------------------------------------------*/
	...
	siNum = sub(siUpperBound, siLowerBound);
	siNum = add(siNum, 1);
	CGInterp(pswLBuf, siNum, pswIntCs, pswIntGs, LSMIN, pswCBuf, pswGBuf);
	sw1 = 0;
	sw2 = 0x003f;
	siPeakIndex = fnBest_CG(pswCBuf, pswGBuf, &sw1, &sw2, siNum);
	if (siPeakIndex == -1) {
  ...
  /*----------------------------------------------*/
	...
	*psiNumSorted = 0;
	...
	for (i = 0; i < siNumPeaks; i++) {
		...
		sw2 = 0x003f;
		siPeakIndex =
		    fnBest_CG(pswCPeaks, pswGPeaks, &sw1, &sw2, siNumPeaks);
		...
		for (j = 0; j < *psiNumSorted; j++) {
			...
			if (sub
			    (swWorkingLag,
			     pswLPeaksSorted[j + siLPeaksSortedInit]) == 0)
				siRepeat = 1;
		}
...
*/
	...
	swUnqntzdR0 = hr_round(L_UnqntzdR0);
