...
*/
	...
	energy_computation
	    (pL_acf, swScaleAcf,
	     pswRvad, swNormRvad, &swM_pvad, &swE_pvad, &swM_acf0, &swE_acf0);

	average_acf(pL_acf, swScaleAcf, pL_av0, pL_av1);

	predictor_values(pL_av1, pswRav1, &swNormRav1);

	spectral_comparison(pswRav1, swNormRav1, pL_av0, &swStat);

	tone_detection(pswRc, &swTone);

	threshold_adaptation
	    (swStat, swPtch, swTone,
	     pswRav1, swNormRav1,
	     swM_pvad, swE_pvad,
	     swM_acf0, swE_acf0, pswRvad, &swNormRvad, &swM_thvad, &swE_thvad);

	vad_decision(swM_pvad, swE_pvad, swM_thvad, swE_thvad, &swVvad);

	vad_hangover(swVvad, pswVadFlag);
...
*/
  ...
  /*** Compute Parcor coefficients: ***/
    ...
    /*** Schur recursion: ***/
		...
		for (m = 1; m <= (7 - n); m++) {
			pswPp[m] =
			    add(pswPp[1 + m], mult_r(pswKk[9 - m], pswVpar[n]));
			pswKk[9 - m] =
			    add(pswKk[9 - m], mult_r(pswPp[1 + m], pswVpar[n]));
		}
	}
...
*/
  ...
  /*** Computation of the rav1[0..8] ***/
	...
	for (i = 0; i <= 8; i++) {
		...
		for (k = 0; k <= 8 - i; k++)
			pL_work[i] =
			    L_mac(pL_work[i], pswAav1[k], pswAav1[k + i]);
	}
  ...
  /*** compute pvad + margin ***/
	...
	if (swE_pvad == E_MARGIN) {
		...
		if (swE_pvad > E_MARGIN) {
			...
			L_temp =
			    L_add(L_deposit_l(swM_pvad), L_deposit_l(swTemp));
			...
			L_temp =
			    L_add(L_deposit_l(M_MARGIN), L_deposit_l(swTemp));
