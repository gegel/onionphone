    ...
    )
{
	...
	if (dispState - prevDispState > 1 && !onset)
		dispState -= 1;
	...
	if (dispState == 0) {
		for (nPulse = 0; nPulse < nze; nPulse++) {
			...
			for (i1 = ppos; i1 < L_SUBFR; i1++)
				inno[i1] +=
				    inno_sav[ppos] * ph_imp_low[i1 - ppos];
			for (i2 = 0; i2 < ppos; i2++)
				inno[i2] +=
				    inno_sav[ppos] * ph_imp_low[L_SUBFR - ppos +
								i2];
		}
	}
	...
	if (dispState == 1) {
		for (nPulse = 0; nPulse < nze; nPulse++) {
			...
			for (i1 = ppos; i1 < L_SUBFR; i1++)
				inno[i1] +=
				    inno_sav[ppos] * ph_imp_mid[i1 - ppos];
			for (i2 = 0; i2 < ppos; i2++)
				inno[i2] +=
				    inno_sav[ppos] * ph_imp_mid[L_SUBFR - ppos +
								i2];
		}
	}
	...
	if (dispState == 2) {
		for (nPulse = 0; nPulse < nze; nPulse++) {
			...
			for (i1 = ppos; i1 < L_SUBFR; i1++)
				inno[i1] +=
				    inno_sav[ppos] * ph_imp_high[i1 - ppos];
			for (i2 = 0; i2 < ppos; i2++)
				inno[i2] +=
				    inno_sav[ppos] * ph_imp_high[L_SUBFR -
								 ppos + i2];
		}
	}
