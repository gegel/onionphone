	...
	if (tameflag == 1) {
		for (i = 0; i < NCAN1_6K; i++) {
			for (j = 0; j < NCAN2_6K; j++) {
				g_pitch =
				    gbk1_6k[cand1 + i][0] + gbk2_6k[cand2 +
								    j][0];
				if (g_pitch < GP0999) {
					g_code =
					    gcode0 * (gbk1_6k[cand1 + i][1] +
						      gbk2_6k[cand2 + j][1]);
    ...
    )
{
	...
	x = (best_gain[1] -
	     (coef_6k[0][0] * best_gain[0] +
	      coef_6k[1][1]) * gcode0) * INV_COEF_6K;
    ...
    *----------------------------------------------*/
	...
	if (tameflag == 1) {
		for (i = 0; i < NCAN1; i++) {
			for (j = 0; j < NCAN2; j++) {
				g_pitch =
				    gbk1[cand1 + i][0] + gbk2[cand2 + j][0];
				if (g_pitch < GP0999) {
					g_code =
					    gcode0 * (gbk1[cand1 + i][1] +
						      gbk2[cand2 + j][1]);
					dist = g_pitch * g_pitch * g_coeff[0]
					    + g_pitch * g_coeff[1]
					    + g_code * g_code * g_coeff[2]
