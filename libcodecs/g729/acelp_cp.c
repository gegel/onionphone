    ...
    )
{
	...
	*sign = signIndex[0] + 2 * signIndex[1];
	...
	if (signIndex[1] == 1) {
		for (i = m1_bestPos, j = 0; i < L_SUBFR; i++, j++)
			y[i] = y[i] + h[j];
	} else {
		for (i = m1_bestPos, j = 0; i < L_SUBFR; i++, j++)
			y[i] = y[i] - h[j];
	...
	/*--------------------------------------------------*
        ...
        * in vector vec[] for the "cor_h_vec()" function.  *
	    ...
	    /*--------------------------------------------------*
            ...
            *--------------------------------------------------*/
			search_ixiy((int)ipos[k + j], (int)ipos[k + j + 1], &ps,
				    &alp, &ix, &iy, dn, cor_x, cor_y, rrixiy);
	...
	/*--------------------------------------------------*
        ...
        * in vector vec[] for the "cor_h_vec()" function.  *
	    ...
	    /*--------------------------------------------------*
            * Fix 2 pulses, try 8x8 = 64 positions.            *
            *--------------------------------------------------*/
			search_ixiy(ipos[k + j], ipos[k + j + 1], &ps, &alp,
				    &ix, &iy, dn, cor_x, cor_y, rrixiy);
			ip[j] = ix;
			ip[j + 1] = iy;
		}
	...
	for (k = 0; k < nb_of_pulse; k++) {
		...
		if (indx[track] < 0) {
			...
			if (((index ^ indx[track]) & 8) == 0) {
				...
				if (indx[track] <= index) {
					indx[track] =
					    (indx[track] << (int)4) + index;
					indx[track] |= (int)256;
				} else {
					indx[track] =
					    (index << (int)4) + indx[track];
					indx[track] |= (int)256;
				}
			} else {
				/* sign of 1st pulse != sign of 2th pulse */
