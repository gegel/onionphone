			...
			if ((prm[1]) == 0) {	/* mode parameter */
				...
				*ptr_bits++ = BIT_0;	/* Parity bit */
				for (i = 0; i < PRM_SIZE_E_fwd - 1; i++) {
					int2bin(prm[i + 2], bitsno_E_fwd[i],
						ptr_bits);
				...
				*ptr_bits++ = BIT_1;	/* Parity bit */
				for (i = 0; i < PRM_SIZE_E_bwd - 1; i++) {
					int2bin(prm[i + 2], bitsno_E_bwd[i],
						ptr_bits);
    ...
    )
{
	...
	case RATE_11800:{	/* G729 Annex E: 118 bits */
			...
			if (mode == 0) {
				for (i = 0; i < PRM_SIZE_E_fwd - 1; i++) {
					prm[i + 3] =
					    bin2int(bitsno_E_fwd[i], bits);
					bits += bitsno_E_fwd[i];
				}
			} else {
				for (i = 0; i < PRM_SIZE_E_bwd - 1; i++) {
					prm[i + 3] =
					    bin2int(bitsno_E_bwd[i], bits);
					bits += bitsno_E_bwd[i];
				}
			}
			break;
	...
	default:{
			printf
			    ("Unrecognized frame type: uncorrect value of serial_size\n");
			exit(-1);
		}
