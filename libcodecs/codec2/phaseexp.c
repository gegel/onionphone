...
void phase_experiment_destroy(struct PEXP *pexp)
{
	...
	if (pexp->var != 0.0)
		printf
		    ("var...: %4.3f  std dev...: %4.3f (%d non zero phases)\n",
		     pexp->var / pexp->var_n, sqrt(pexp->var / pexp->var_n),
		     pexp->var_n);
	if (pexp->vq_var != 0.0)
		printf
		    ("vq var: %4.3f  vq std dev: %4.3f (%d non zero phases)\n",
		     pexp->vq_var / pexp->vq_var_n,
		     sqrt(pexp->vq_var / pexp->vq_var_n), pexp->vq_var_n);
	free(pexp);
}
...
static void predict_phases2(struct PEXP *pexp, MODEL * model, int start,
			    int end)
{
	int i;
	float pred, str, diff;
...
#ifdef REAS_CAND3
static void fixed_bits_per_frame(struct PEXP *pexp, MODEL * model, int m,
				 int budget)
{
	int res, finished;
...
#if defined(REAS_CAND1) || defined(REAS_CAND2)
static void top_amp(struct PEXP *pexp, MODEL * model, int start, int end,
		    int n_harm, int pred)
{
#define ALTTOP
#ifdef ALTTOP
...
static void update_variance_calc(struct PEXP *pexp, MODEL * model,
				 float before[])
{
	int m;
	float diff;
	...
	for (j = 0; j < e; j++) {
		...
		for (i = 0; i < d; i++) {
			...
			if (!ignore) {
				...
				error += diffp * diffp;
				metric +=
				    weights[i] * weights[i] * diffp * diffp;
...
static void split_vq(COMP sparse_pe_out[], struct PEXP *pexp,
		     struct codebook *vq, float weights[], COMP sparse_pe_in[])
{
	int i, j, non_zero, vq_ind;

	//printf("\n offset %d k %d m %d  j: ", vq->offset, vq->k, vq->m);
	vq_ind =
	    vq_phase(vq->cb, &sparse_pe_in[vq->offset], &weights[vq->offset],
		     vq->k, vq->m, &pexp->vq_var);

	non_zero = 0;
	for (i = 0, j = vq->offset; (unsigned int)i < vq->k; i++, j++) {
		//printf("%f ", atan2(sparse_pe[i].imag, sparse_pe[i].real));
		if ((sparse_pe_in[j].real != 0.0)
		    && (sparse_pe_in[j].imag != 0.0)) {
			//printf("%d ", j);
...
void smooth_phase(struct PEXP *pexp, MODEL * model, int mode)
{
	...
	if (mode == 2) {
		...
		split_vq(smoothed_out, pexp, pexp->vq1, smoothed_weights,
			 smoothed_in);
		for (i = 0; i < MAX_AMP; i++)
	...
	for (i = st, v = 0; i < MAX_AMP; i += step, v++) {
		...
		if (mode == 1)
			printf("%f ",
			       atan2(smoothed[v].imag, smoothed[v].real));
	}
	if (mode == 1)
		printf("\n");
	...
	for (m = 1; m <= model->L; m++) {
		...
		pred = pexp->phi_prev[m] + N * m * best_Wo;
		err =
		    atan2(sparse_pe_out[index].imag, sparse_pe_out[index].real);
...
void smooth_phase3(struct PEXP *pexp, MODEL * model)
{
	...
	for (m = 1; m <= model->L; m++) {
		...
		if (f > bins[0]) {
			...
			model->phi[m] = pred + err;
			model->phi[m] =
			    atan2(sin(model->phi[m]), cos(model->phi[m]));
			printf("%4.3f\n", model->phi[m]);
		}
	}
	printf("\n");
...
void cb_phase2(struct PEXP *pexp, MODEL * model)
{
	...
	step = 3;
	model->phi[1] =
	    pexp->phi_prev[1] + (pexp->Wo_prev + model->Wo) * N / 2.0;
...
static void smooth_phase4(MODEL * model)
{
	...
	if (model->L > 25) {
		...
		for (m = model->L / 2; m <= model->L; m += 2) {
			if ((m + 1) <= model->L) {
				phi_m =
				    (model->phi[m] - model->phi[m + 1]) / 2.0;
				phi_m_1 =
				    (model->phi[m + 1] - model->phi[m]) / 2.0;
				model->phi[m] = phi_m;
