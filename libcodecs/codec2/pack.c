...
void pack_natural_or_gray(unsigned char *bitArray,	/* The output bit string. */
			  unsigned int *bitIndex,	/* Index into the string in BITS, not bytes. */
			  int field,	/* The bit field to be packed. */
			  unsigned int fieldWidth,	/* Width of the field in BITS, not bytes. */
 ...
 *
 */
int unpack_natural_or_gray(const unsigned char *bitArray,	/* The input bit string. */
			   unsigned int *bitIndex,	/* Index into the string in BITS, not bytes. */
			   unsigned int fieldWidth,	/* Width of the field in BITS, not bytes. */
			   unsigned int gray	/* non-zero for Gray coding */
    )
{
	...
	do {
		...
		field |=
		    (((bitArray[bI >> ShiftRight] >> (bitsLeft - sliceWidth)) &
		      ((1 << sliceWidth) - 1)) << (fieldWidth - sliceWidth));
