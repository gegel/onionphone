...
struct FDMDV *CODEC2_WIN32SUPPORT fdmdv_create(int Nc)
{
	...
	assert((sizeof(test_bits) / sizeof(int)) >=
	       (unsigned int)f->ntest_bits);
	...
	for (c = 0; c < Nc; c++) {
		...
		if ((msb == 1) && (lsb == 0)) {
			if (old_qpsk_mapping)
				...
				tx_symbols[c] =
				    cmult(cneg(j), prev_tx_symbols[c]);
		}
		if ((msb == 1) && (lsb == 1)) {
			if (old_qpsk_mapping)
				tx_symbols[c] =
...
void tx_filter(COMP tx_baseband[NC + 1][M], int Nc, COMP tx_symbols[],
...
void fdm_upconvert(COMP tx_fdm[], int Nc, COMP tx_baseband[NC + 1][M],
		   COMP phase_tx[], COMP freq[])
{
	...
	for (c = 0; c < Nc / 2; c++)
		for (i = 0; i < M; i++) {
			...
			tx_fdm[i] =
			    cadd(tx_fdm[i],
				 cmult(tx_baseband[c][i], phase_tx[c]));
		}
				   ...
				   int tx_bits[], int *sync_bit)
{
	...
	bits_to_dqpsk_symbols(tx_symbols, fdmdv->Nc, fdmdv->prev_tx_symbols,
			      tx_bits, &fdmdv->tx_pilot_bit,
			      fdmdv->old_qpsk_mapping);
	memcpy(fdmdv->prev_tx_symbols, tx_symbols,
	       sizeof(COMP) * (fdmdv->Nc + 1));
...
void lpf_peak_pick(float *foff, float *max, COMP pilot_baseband[],
		   COMP pilot_lpf[], kiss_fft_cfg fft_pilot_cfg, COMP S[],
		   int nin)
{
	int i, j, k;
	int mpilot;
	...
	for (i = NPILOTLPF - nin, j = 0; i < NPILOTLPF; i++, j++) {
		...
		for (k = 0; k < NPILOTCOEFF; k++)
			pilot_lpf[i] =
			    cadd(pilot_lpf[i],
				 fcmult(pilot_coeff[k], pilot_baseband[j + k]));
	}
		     ...
		     COMP phase_rx[], COMP freq[], int nin)
{
	...
	for (c = 0; c < Nc / 2; c++)
		for (i = 0; i < nin; i++) {
			...
			rx_baseband[c][i] =
			    cmult(rx_fdm[i], cconj(phase_rx[c]));
		}
...
void rx_filter(COMP rx_filt[NC + 1][P + 1], int Nc,
...
float rx_est_timing(COMP rx_symbols[],
		    ...
		    float env[],
		    COMP rx_baseband_mem_timing[NC + 1][NFILTERTIMING], int nin)
{
	int c, i, j, k;
	int adjust, s;
	COMP x, phase, freq;
	...
	for (c = 0; c < Nc + 1; c++)
		for (i = 0, j = nin; i < NFILTERTIMING - nin; i++, j++)
			rx_baseband_mem_timing[c][i] =
			    rx_baseband_mem_timing[c][j];
	for (c = 0; c < Nc + 1; c++)
		for (i = NFILTERTIMING - nin, j = 0; i < NFILTERTIMING;
		     i++, j++)
		...
		COMP phase_difference[])
{
	...
	for (c = 0; c < Nc + 1; c++)
		noise_est[c] =
		    SNR_COEFF * noise_est[c] + (1 - SNR_COEFF) * n[c];
}
					     ...
					     int rx_bits[])
{
	...
	for (i = 0, j = bits_per_frame; i < f->ntest_bits - bits_per_frame;
	     i++, j++)
		f->rx_test_bits_mem[i] = f->rx_test_bits_mem[j];
	for (i = f->ntest_bits - bits_per_frame, j = 0; i < f->ntest_bits;
	     i++, j++)
...
void CODEC2_WIN32SUPPORT fdmdv_demod(struct FDMDV *fdmdv, int rx_bits[],
				     int *reliable_sync_bit, COMP rx_fdm[],
				     int *nin)
{
	...
	fdm_downconvert(rx_baseband, fdmdv->Nc, rx_fdm_fcorr, fdmdv->phase_rx,
			fdmdv->freq, *nin);
	rx_filter(rx_filt, fdmdv->Nc, rx_baseband, fdmdv->rx_filter_memory,
		  *nin);
	fdmdv->rx_timing =
	    rx_est_timing(rx_symbols, fdmdv->Nc, rx_filt, rx_baseband,
			  fdmdv->rx_filter_mem_timing, env,
			  fdmdv->rx_baseband_mem_timing, *nin);
	...
	*nin = M;
	...
	foff_fine =
	    qpsk_to_bits(rx_bits, &sync_bit, fdmdv->Nc, fdmdv->phase_difference,
			 fdmdv->prev_rx_symbols, rx_symbols,
			 fdmdv->old_qpsk_mapping);
	memcpy(fdmdv->prev_rx_symbols, rx_symbols,
	       sizeof(COMP) * (fdmdv->Nc + 1));
	...
	fdmdv->sync =
	    freq_state(reliable_sync_bit, sync_bit, &fdmdv->fest_state,
					       ...
					       struct FDMDV_STATS *fdmdv_stats)
{
	...
	fdmdv_stats->snr_est =
	    calc_snr(fdmdv->Nc, fdmdv->sig_est, fdmdv->noise_est);
...
void CODEC2_WIN32SUPPORT fdmdv_8_to_48(float out48k[], float in8k[], int n)
{
	...
	for (i = 0; i < n; i++) {
		for (j = 0; j < FDMDV_OS; j++) {
			...
			for (k = 0, l = 0; k < FDMDV_OS_TAPS;
			     k += FDMDV_OS, l++)
				out48k[i * FDMDV_OS + j] +=
				    fdmdv_os_filter[k + j] * in8k[i - l];
...
void CODEC2_WIN32SUPPORT fdmdv_48_to_8(float out8k[], float in48k[], int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		for (j = 0; j < FDMDV_OS_TAPS; j++)
			out8k[i] +=
			    fdmdv_os_filter[j] * in48k[i * FDMDV_OS - j];
	}
					       ...
					       COMP rx_fdm[], int nin)
{
	...
	for (i = 0; i < FDMDV_NSPEC; i++) {
		mag_spec_dB[i] =
		    10.0 * log10(fft_out[i].real * fft_out[i].real +
