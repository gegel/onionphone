...
float nlp(void *nlp_state, float Sn[],	/* input speech vector */
				 ...
				 float *prev_Wo)
{
	...
	cmax_bin = gmax_bin;
	prev_f0_bin =
	    *prev_Wo * (4000.0 / PI) * (PE_FFT_SIZE * DEC) / SAMPLE_RATE;
	...
	for (i = PE_FFT_SIZE * DEC / pmax; i <= PE_FFT_SIZE * DEC / pmin; i++) {
		if ((Fw[i].real > Fw[i - 1].real)
			...
			if (Fw[i].real > T * gmax) {
				...
				candidate_f0 =
				    (float)i *SAMPLE_RATE / (PE_FFT_SIZE * DEC);
...
float test_candidate_mbe(COMP Sw[], COMP W[], float f0)
{
	...
	for (l = 1; l < L / 4; l++) {
		...
		for (m = al; m < bl; m++) {
			offset =
			    FFT_ENC / 2 + m - l * Wo * FFT_ENC / TWO_PI + 0.5;
			Am.real +=
			    Sw[m].real * W[offset].real +
			    Sw[m].imag * W[offset].imag;
			Am.imag +=
			    Sw[m].imag * W[offset].real -
			    Sw[m].real * W[offset].imag;
			den +=
			    W[offset].real * W[offset].real +
		...
		for (m = al; m < bl; m++) {
			offset =
			    FFT_ENC / 2 + m - l * Wo * FFT_ENC / TWO_PI + 0.5;
			Sw_[m].real =
			    Am.real * W[offset].real - Am.imag * W[offset].imag;
			Sw_[m].imag =
			    Am.real * W[offset].imag + Am.imag * W[offset].real;
			error +=
			    (Sw[m].real - Sw_[m].real) * (Sw[m].real -
							  Sw_[m].real);
			error +=
			    (Sw[m].imag - Sw_[m].imag) * (Sw[m].imag -
							  Sw_[m].imag);
		}
	}
