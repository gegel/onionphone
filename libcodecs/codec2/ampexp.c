...
static float get_float(FILE * in, const char *name, char * *cursor,
...
void amp_experiment_destroy(struct AEXP *aexp)
{
	...
	if (aexp->var != 0.0)
		printf
		    ("var...: %4.3f  std dev...: %4.3f (%d amplitude samples)\n",
		     aexp->var / aexp->var_n, sqrt(aexp->var / aexp->var_n),
		     aexp->var_n);
	if (aexp->vq_var != 0.0)
		printf
		    ("vq var: %4.3f  std dev...: %4.3f (%d amplitude samples)\n",
		     aexp->vq_var / aexp->vq_var_n,
		     sqrt(aexp->vq_var / aexp->vq_var_n), aexp->vq_var_n);
	free(aexp);
}
...
static void add_quant_noise(struct AEXP *aexp, MODEL * model, int start,
			    int end, float sigma_dB)
{
	int m;
	float x_dB;
	float noise_sam_dB;
...
static void print_sparse_amp_error(struct AEXP *aexp, MODEL * model,
				   float edB_thresh)
{
	int m, index;
...
static int split_vq(float sparse_pe_out[], struct AEXP *aexp,
		    struct codebook *vq, float weights[], float sparse_pe_in[])
{
...
#define SPLIT
	...
	for (m = 0; m < MAX_AMP; m++) {
		if (sparse_pe_in[m] != 0.0)
			aexp->vq_var +=
			    pow(sparse_pe_out[m] - sparse_pe_in[m], 2.0);
	}
...
static void gain_shape_split_vq(float sparse_pe_out[], struct AEXP *aexp,
				struct codebook *vq, float weights[],
				float sparse_pe_in[], float *best_gain)
{
	int i, j, non_zero, vq_ind;
	float se = 0;

	vq_ind =
	    gain_shape_vq_amp(vq->cb, &sparse_pe_in[vq->offset],
			      &weights[vq->offset], vq->k, vq->m, &se,
			      best_gain);
...
static void gain_shape_sparse_vq_amp(struct AEXP *aexp, MODEL * model)
{
	...
	gain_shape_split_vq(sparse_pe_out, aexp, aexp->vq1, weights,
			    sparse_pe_in, &best_gain);
	gain_shape_split_vq(sparse_pe_out, aexp, aexp->vq2, weights,
			    sparse_pe_in, &best_gain);
	gain_shape_split_vq(sparse_pe_out, aexp, aexp->vq3, weights,
			    sparse_pe_in, &best_gain);
	gain_shape_split_vq(sparse_pe_out, aexp, aexp->vq4, weights,
			    sparse_pe_in, &best_gain);
...
static void interp_split_vq(float sparse_pe_out[], struct AEXP *aexp,
			    struct codebook *vq, float sparse_pe_in[], int ind)
{
	int i, j;
	float amp_dB;
	...
	for (i = 0, j = vq->offset; (unsigned int)i < vq->k; i++, j++) {
		if (sparse_pe_in[j] != 0.0) {
			amp_dB =
			    0.5 * (aexp->mag[0] +
				   vq->cb[vq->k * aexp->indexes[ind][0] + i]);
			amp_dB +=
			    0.5 * (aexp->mag[2] +
				   vq->cb[vq->k * aexp->indexes[ind][2] + i]);
...
static void vq_interp(struct AEXP *aexp, MODEL * model, int on)
{
	...
	if (on && ((aexp->frames % 2) == 0)) {
		...
		interp_split_vq(sparse_pe_out, aexp, aexp->vq1, sparse_pe_in,
				0);
		interp_split_vq(sparse_pe_out, aexp, aexp->vq2, sparse_pe_in,
				1);
		interp_split_vq(sparse_pe_out, aexp, aexp->vq3, sparse_pe_in,
				2);
		interp_split_vq(sparse_pe_out, aexp, aexp->vq4, sparse_pe_in,
				3);
		interp_split_vq(sparse_pe_out, aexp, aexp->vq5, sparse_pe_in,
				4);
		...
		for (m = 1; m <= model->L; m++) {
			...
			if (index < vq->k) {
				amp_dB =
				    0.5 * (aexp->mag[0] +
					   vq->cb[vq->k * aexp->indexes[0] +
						  index]);
				amp_dB +=
				    0.5 * (aexp->mag[2] +
					   vq->cb[vq->k * aexp->indexes[2] +
						  index]);
				//printf("  %4.2f", 10.0*log10(model->A[m]));
				//amp_dB = 10;
				model->A[m] = pow(10.0, amp_dB / 20.0);
