		     ...
		     float * lsp1_q, int16_t * indice, int32_t * pred_init_i)
{
	...
	if (mode != MRDTX) {
		...
		*pred_init_i = 0;
		...
		for (j = 0; j < PAST_RQ_INIT_SIZE; j++) {
			...
			for (i = 0; i < M; i++) {
				temp_p[i] =
				    mean_lsf_3[i] + past_rq_init[j * M + i];
	...
	if ((mode == MR475) || (mode == MR515)) {
		indice[0] =
		    Vq_subvec3(&lsf_r1[0], dico1_lsf_3, &wf1[0], DICO1_SIZE_3,
			       0);
		indice[1] =
		    Vq_subvec3(&lsf_r1[3], dico2_lsf_3, &wf1[3],
			       DICO2_SIZE_3 / 2, 1);
		indice[2] =
		    Vq_subvec4(&lsf_r1[6], mr515_3_lsf, &wf1[6], MR515_3_SIZE);
	}

	/* MR795 */
	else if (mode == MR795) {
		indice[0] =
		    Vq_subvec3(&lsf_r1[0], mr795_1_lsf, &wf1[0], MR795_1_SIZE,
			       0);
		indice[1] =
		    Vq_subvec3(&lsf_r1[3], dico2_lsf_3, &wf1[3], DICO2_SIZE_3,
			       0);
		indice[2] =
		    Vq_subvec4(&lsf_r1[6], dico3_lsf_3, &wf1[6], DICO3_SIZE_3);
	}
		     ...
		     float * lsp1_q, float * lsp2_q, int16_t * indice)
{
	...
	indice[0] =
	    Vq_subvec(&lsf_r1[0], &lsf_r2[0], dico1_lsf_5, &wf1[0], &wf2[0],
		      DICO1_SIZE_5);
	indice[1] =
	    Vq_subvec(&lsf_r1[2], &lsf_r2[2], dico2_lsf_5, &wf1[2], &wf2[2],
		      DICO2_SIZE_5);
	indice[2] =
	    Vq_subvec_s(&lsf_r1[4], &lsf_r2[4], dico3_lsf_5, &wf1[4], &wf2[4],
			DICO3_SIZE_5);
	indice[3] =
	    Vq_subvec(&lsf_r1[6], &lsf_r2[6], dico4_lsf_5, &wf1[6], &wf2[6],
		      DICO4_SIZE_5);
	indice[4] =
	    Vq_subvec(&lsf_r1[8], &lsf_r2[8], dico5_lsf_5, &wf1[8], &wf2[8],
		      DICO5_SIZE_5);
		       ...
		       int32_t dtx, int16_t idx)
{
	...
	j = pit_min << 2;
	p_max1 =
	    Lag_max(corr_ptr, signal, L_frame, pit_max, j, &max1, dtx, &rmax1,
		    &r01);
	...
	j = pit_min << 1;
	p_max2 =
	    Lag_max(corr_ptr, signal, L_frame, i, j, &max2, dtx, &rmax2, &r02);

	/* Maxima 3  */
	i = j - 1;
	p_max3 =
	    Lag_max(corr_ptr, signal, L_frame, i, pit_min, &max3, dtx, &rmax3,
		    &r03);
#else
	/* Find a maximum for each section.  */
	/* Maxima 1  */
			   ...
			   int32_t dtx)
{
	...
	if (dtx) {
		if (idx == 1) {
			/* calculate max high-passed filtered correlation of all lags */
			hp_max(corrPtr, signal, L_FRAME_BY2, PIT_MAX, PIT_MIN,
			       &corr_hp_max);
		...
		for (i = 0; i < 4; i++) {
			...
			for (i0 = ipos[0]; i0 < L_CODE; i0 += STEP) {
				if (dn2[i0] >= 0) {
					...
					for (i2 = ipos[2]; i2 < L_CODE;
					     i2 += STEP) {
						ps1 = ps0 + dn[i2];

						/* alp1 = alp0 + rr[i0][i2] + rr[i1][i2] + 1/2*rr[i2][i2]; */
						alp1 =
						    alp0 + rr[i2][i2] * 0.0625F;
			...
			int32_t pos_max[], int32_t codvec[])
{
	...
	for (i = 1; i < 5; i++) {
		...
		*(p_rrv + 12) =
		    *(p_r3 + 492) + 2.0F * (*(p_r0 + 12) + *(p_r1 + 12));
		*(p_rrv + 16) =
		    *(p_r3 + 656) + 2.0F * (*(p_r0 + 16) + *(p_r1 + 16));
		*(p_rrv + 20) =
		    *(p_r3 + 820) + 2.0F * (*(p_r0 + 20) + *(p_r1 + 20));
		*(p_rrv + 24) =
		    *(p_r3 + 984) + 2.0F * (*(p_r0 + 24) + *(p_r1 + 24));
		*(p_rrv + 28) =
		    *(p_r3 + 1148) + 2.0F * (*(p_r0 + 28) + *(p_r1 + 28));
		*(p_rrv + 32) =
		    *(p_r3 + 1312) + 2.0F * (*(p_r0 + 32) + *(p_r1 + 32));
		*(p_rrv + 36) =
		    *(p_r3 + 1476) + 2.0F * (*(p_r0 + 36) + *(p_r1 + 36));
		sq = -1;
		alp = 1;
		ps = 0;
		ia = i2;
		...
		*(p_rrv + 8) = *(p_r5 + 328) + 2.0F * (*(p_r0 + 8) + *(p_r1 + 8)
						       + *(p_r2 + 8) + *(p_r3 +
									 8));
		*(p_rrv + 12) =
		    *(p_r5 + 492) + 2.0F * (*(p_r0 + 12) + *(p_r1 + 12) +
					    *(p_r2 + 12) + *(p_r3 + 12));
		*(p_rrv + 16) =
		    *(p_r5 + 656) + 2.0F * (*(p_r0 + 16) + *(p_r1 + 16) +
					    *(p_r2 + 16) + *(p_r3 + 16));
		*(p_rrv + 20) =
		    *(p_r5 + 820) + 2.0F * (*(p_r0 + 20) + *(p_r1 + 20) +
					    *(p_r2 + 20) + *(p_r3 + 20));
		*(p_rrv + 24) =
		    *(p_r5 + 984) + 2.0F * (*(p_r0 + 24) + *(p_r1 + 24) +
					    *(p_r2 + 24) + *(p_r3 + 24));
		*(p_rrv + 28) =
		    *(p_r5 + 1148) + 2.0F * (*(p_r0 + 28) + *(p_r1 + 28) +
					     *(p_r2 + 28) + *(p_r3 + 28));
		*(p_rrv + 32) =
		    *(p_r5 + 1312) + 2.0F * (*(p_r0 + 32) + *(p_r1 + 32) +
					     *(p_r2 + 32) + *(p_r3 + 32));
		*(p_rrv + 36) =
		    *(p_r5 + 1476) + 2.0F * (*(p_r0 + 36) + *(p_r1 + 36) +
					     *(p_r2 + 36) + *(p_r3 + 36));
		...
		p_r7 = &rr[i7][i7];
		*p_rrv =
		    *p_r7 + 2.0F * (*p_r0 + *p_r1 + *p_r2 + *p_r3 + *p_r4 +
				    *p_r5);
		...
		*(p_rrv + 8) = *(p_r7 + 328) + 2.0F * (*(p_r0 + 8) + *(p_r1 + 8)
						       ...
						       *(p_r4 + 8) + *(p_r5 +
								       8));
		*(p_rrv + 12) =
		    *(p_r7 + 492) + 2.0F * (*(p_r0 + 12) + *(p_r1 + 12) +
					    *(p_r2 + 12) + *(p_r3 + 12) +
					    *(p_r4 + 12) + *(p_r5 + 12));
		*(p_rrv + 16) =
		    *(p_r7 + 656) + 2.0F * (*(p_r0 + 16) + *(p_r1 + 16) +
					    *(p_r2 + 16) + *(p_r3 + 16) +
					    *(p_r4 + 16) + *(p_r5 + 16));
		*(p_rrv + 20) =
		    *(p_r7 + 820) + 2.0F * (*(p_r0 + 20) + *(p_r1 + 20) +
					    *(p_r2 + 20) + *(p_r3 + 20) +
					    *(p_r4 + 20) + *(p_r5 + 20));
		*(p_rrv + 24) =
		    *(p_r7 + 984) + 2.0F * (*(p_r0 + 24) + *(p_r1 + 24) +
					    *(p_r2 + 24) + *(p_r3 + 24) +
					    *(p_r4 + 24) + *(p_r5 + 24));
		*(p_rrv + 28) =
		    *(p_r7 + 1148) + 2.0F * (*(p_r0 + 28) + *(p_r1 + 28) +
					     *(p_r2 + 28) + *(p_r3 + 28) +
					     *(p_r4 + 28) + *(p_r5 + 28));
		*(p_rrv + 32) =
		    *(p_r7 + 1312) + 2.0F * (*(p_r0 + 32) + *(p_r1 + 32) +
					     *(p_r2 + 32) + *(p_r3 + 32) +
					     *(p_r4 + 32) + *(p_r5 + 32));
		*(p_rrv + 36) =
		    *(p_r7 + 1476) + 2.0F * (*(p_r0 + 36) + *(p_r1 + 36) +
					     *(p_r2 + 36) + *(p_r3 + 36) +
					     *(p_r4 + 36) + *(p_r5 + 36));
    ...
    )
{
	 ...
	 */
	indx[NB_TRACK_MR102] =
	    compress10(pos_indx[0], pos_indx[4], pos_indx[1]);
	...
	indx[NB_TRACK_MR102 + 1] =
	    compress10(pos_indx[2], pos_indx[6], pos_indx[5]
	    );
			 ...
			 int32_t pos_max[], int32_t codvec[])
{
	...
	for (i = 1; i < 5; i++) {
		...
		*(p_rrv + 10) =
		    *(p_r3 + 410) + 2.0F * (*(p_r0 + 10) + *(p_r1 + 10));
		*(p_rrv + 15) =
		    *(p_r3 + 615) + 2.0F * (*(p_r0 + 15) + *(p_r1 + 15));
		*(p_rrv + 20) =
		    *(p_r3 + 820) + 2.0F * (*(p_r0 + 20) + *(p_r1 + 20));
		*(p_rrv + 25) =
		    *(p_r3 + 1025) + 2.0F * (*(p_r0 + 25) + *(p_r1 + 25));
		*(p_rrv + 30) =
		    *(p_r3 + 1230) + 2.0F * (*(p_r0 + 30) + *(p_r1 + 30));
		*(p_rrv + 35) =
		    *(p_r3 + 1435) + 2.0F * (*(p_r0 + 35) + *(p_r1 + 35));
		sq = -1;
		alp = 1;
		ps = 0;
		ia = i2;
		...
		*(p_rrv + 5) = *(p_r5 + 205) + 2.0F * (*(p_r0 + 5) + *(p_r1 + 5)
						       + *(p_r2 + 5) + *(p_r3 +
									 5));
		*(p_rrv + 10) =
		    *(p_r5 + 410) + 2.0F * (*(p_r0 + 10) + *(p_r1 + 10) +
					    *(p_r2 + 10) + *(p_r3 + 10));
		*(p_rrv + 15) =
		    *(p_r5 + 615) + 2.0F * (*(p_r0 + 15) + *(p_r1 + 15) +
					    *(p_r2 + 15) + *(p_r3 + 15));
		*(p_rrv + 20) =
		    *(p_r5 + 820) + 2.0F * (*(p_r0 + 20) + *(p_r1 + 20) +
					    *(p_r2 + 20) + *(p_r3 + 20));
		*(p_rrv + 25) =
		    *(p_r5 + 1025) + 2.0F * (*(p_r0 + 25) + *(p_r1 + 25) +
					     *(p_r2 + 25) + *(p_r3 + 25));
		*(p_rrv + 30) =
		    *(p_r5 + 1230) + 2.0F * (*(p_r0 + 30) + *(p_r1 + 30) +
					     *(p_r2 + 30) + *(p_r3 + 30));
		*(p_rrv + 35) =
		    *(p_r5 + 1435) + 2.0F * (*(p_r0 + 35) + *(p_r1 + 35) +
					     *(p_r2 + 35) + *(p_r3 + 35));
		...
		*(p_rrv + 5) = *(p_r7 + 205) + 2.0F * (*(p_r0 + 5) + *(p_r1 + 5)
						       ...
						       *(p_r4 + 5) + *(p_r5 +
								       5));
		*(p_rrv + 10) =
		    *(p_r7 + 410) + 2.0F * (*(p_r0 + 10) + *(p_r1 + 10) +
					    *(p_r2 + 10) + *(p_r3 + 10) +
					    *(p_r4 + 10) + *(p_r5 + 10));
		*(p_rrv + 15) =
		    *(p_r7 + 615) + 2.0F * (*(p_r0 + 15) + *(p_r1 + 15) +
					    *(p_r2 + 15) + *(p_r3 + 15) +
					    *(p_r4 + 15) + *(p_r5 + 15));
		*(p_rrv + 20) =
		    *(p_r7 + 820) + 2.0F * (*(p_r0 + 20) + *(p_r1 + 20) +
					    ...
					    *(p_r4 + 20) + *(p_r5 + 20));
		*(p_rrv + 25) =
		    *(p_r7 + 1025) + 2.0F * (*(p_r0 + 25) + *(p_r1 + 25) +
					     *(p_r2 + 25) + *(p_r3 + 25) +
					     *(p_r4 + 25) + *(p_r5 + 25));
		*(p_rrv + 30) =
		    *(p_r7 + 1230) + 2.0F * (*(p_r0 + 30) + *(p_r1 + 30) +
					     *(p_r2 + 30) + *(p_r3 + 30) +
					     *(p_r4 + 30) + *(p_r5 + 30));
		*(p_rrv + 35) =
		    *(p_r7 + 1435) + 2.0F * (*(p_r0 + 35) + *(p_r1 + 35) +
					     *(p_r2 + 35) + *(p_r3 + 35) +
					     *(p_r4 + 35) + *(p_r5 + 35));
		...
		p_r9 = &rr[i9][i9];
		*p_rrv =
		    *p_r9 + 2.0F * (*p_r0 + *p_r1 + *p_r2 + *p_r3 + *p_r4 +
				    *p_r5 + *p_r6 + *p_r7);
		*(p_rrv + 5) = *(p_r9 + 205) + 2.0F * (*(p_r0 + 5) + *(p_r1 + 5)
						       ...
						       *(p_r6 + 5) + *(p_r7 +
								       5));
		*(p_rrv + 10) =
		    *(p_r9 + 410) + 2.0F * (*(p_r0 + 10) + *(p_r1 + 10) +
					    *(p_r2 + 10) + *(p_r3 + 10) +
					    *(p_r4 + 10) + *(p_r5 + 10) +
					    *(p_r6 + 10) + *(p_r7 + 10));
		*(p_rrv + 15) =
		    *(p_r9 + 615) + 2.0F * (*(p_r0 + 15) + *(p_r1 + 15) +
					    *(p_r2 + 15) + *(p_r3 + 15) +
					    *(p_r4 + 15) + *(p_r5 + 15) +
					    *(p_r6 + 15) + *(p_r7 + 15));
		*(p_rrv + 20) =
		    *(p_r9 + 820) + 2.0F * (*(p_r0 + 20) + *(p_r1 + 20) +
					    *(p_r2 + 20) + *(p_r3 + 20) +
					    *(p_r4 + 20) + *(p_r5 + 20) +
					    *(p_r6 + 20) + *(p_r7 + 20));
		*(p_rrv + 25) =
		    *(p_r9 + 1025) + 2.0F * (*(p_r0 + 25) + *(p_r1 + 25) +
					     *(p_r2 + 25) + *(p_r3 + 25) +
					     *(p_r4 + 25) + *(p_r5 + 25) +
					     *(p_r6 + 25) + *(p_r7 + 25));
		*(p_rrv + 30) =
		    *(p_r9 + 1230) + 2.0F * (*(p_r0 + 30) + *(p_r1 + 30) +
					     *(p_r2 + 30) + *(p_r3 + 30) +
					     *(p_r4 + 30) + *(p_r5 + 30) +
					     *(p_r6 + 30) + *(p_r7 + 30));
		*(p_rrv + 35) =
		    *(p_r9 + 1435) + 2.0F * (*(p_r0 + 35) + *(p_r1 + 35) +
					     *(p_r2 + 35) + *(p_r3 + 35) +
					     *(p_r4 + 35) + *(p_r5 + 35) +
					     *(p_r6 + 35) + *(p_r7 + 35));
			     ...
			     int32_t * qua_ener_index, int16_t ** anap)
{
	 ...
	 */
	MR795_gain_code_quant3(gcode0_exp, gcode0_fra, g_pitch_cand,
			       g_pitch_cind, coeff, gain_pit, &gain_pit_index,
			       gain_cod, &gain_cod_index, qua_ener_index);
	 ...
	 */
	gain_adapt(prev_gc, onset, ltpg_mem, prev_alpha, ltpg, *gain_cod,
		   &alpha);
	...
	if ((en[0] != 0) && (alpha > 0)) {
		...
		gain_cod_index =
		    MR795_gain_code_quant_mod(*gain_pit, gcode0_exp, gcode0_fra,
		      ...
		      float past_rq[], int32_t * past_qua_en, int16_t ** anap)
{
	...
	if ((compute_sid_flag != 0)) {
		...
		*log_en_index = (int16_t) ((log_en * 4) + 0.5F);	/* 6 bits */
		...
		Q_plsf_3(MRDTX, past_rq, lsp, lsp_q, lsp_index,
			 init_lsf_vq_index);
	}
...
int vad2(vadState * st, float * farray_ptr)
{
  ...
  /****** Executable code starts here ******/
	...
	for (i = LO_CHAN; i <= HI_CHAN; i++) {
		...
		for (j = j1; j <= j2; j++)
			enrg +=
			    square(data_buffer[2 * j]) +
			    square(data_buffer[2 * j + 1]);
		enrg /= (float)(j2 - j1 + 1);
		st->ch_enrg[i] = (1 - alpha) * st->ch_enrg[i] + alpha * enrg;
		if (st->ch_enrg[i] < MIN_CHAN_ENRG)
			st->ch_enrg[i] = MIN_CHAN_ENRG;
	}
	...
	if (st->Lframe_cnt == 1)
		...
		for (i = LO_CHAN; i <= HI_CHAN; i++)
			ch_enrg_dev +=
			    fabs(st->ch_enrg_long_db[i] - ch_enrg_db[i]);
	...
	for (i = LO_CHAN; i <= HI_CHAN; i++) {
		st->ch_enrg_long_db[i] =
		    alpha * st->ch_enrg_long_db[i] + (1. -
						      alpha) * ch_enrg_db[i];
	}
	...
	if (update_flag == TRUE) {
		for (i = LO_CHAN; i <= HI_CHAN; i++) {
			st->ch_noise[i] =
			    (1.0 - CNE_SM_FAC1) * st->ch_noise[i] +
			    CNE_SM_FAC1 * st->ch_enrg[i];
			if (st->ch_noise[i] < MIN_CHAN_ENRG)
				st->ch_noise[i] = MIN_CHAN_ENRG;
		}
	}
...
void real_fft(float *farray_ptr, int isign)
{
	...
	if (isign == 1) {
		...
		*(farray_ptr + 1) = ftmp1_real - ftmp2_real;
		...
		for (i = 2, j = SIZE - i; i <= SIZE_BY_TWO;
		     i = i + 2, j = SIZE - i) {
			...
			*(farray_ptr + i) =
			    (ftmp1_real + phs_tbl[i] * ftmp2_real -
			     phs_tbl[i + 1] * ftmp2_imag) / 2.0;
			*(farray_ptr + i + 1) =
			    (ftmp1_imag + phs_tbl[i] * ftmp2_imag +
			     phs_tbl[i + 1] * ftmp2_real) / 2.0;
			*(farray_ptr + j) =
			    (ftmp1_real + phs_tbl[j] * ftmp2_real +
			     phs_tbl[j + 1] * ftmp2_imag) / 2.0;
			*(farray_ptr + j + 1) =
			    (-ftmp1_imag - phs_tbl[j] * ftmp2_imag +
		...
		for (i = 0; i < NUM_STAGE; i++) {	/* i is stage counter */
			...
			for (j = 0; j < jj; j = j + 2) {	/* j is sample counter */
				...
				for (k = j; k < SIZE; k = k + kk) {	/* k is butterfly top */
					...
					ftmp_real =
					    *(farray_ptr + kj) * phs_tbl[ji] -
					    *(farray_ptr + kj +
					      1) * phs_tbl[ji + 1];

					ftmp_imag =
					    *(farray_ptr + kj +
					      1) * phs_tbl[ji] + *(farray_ptr +
								   kj) *
					    phs_tbl[ji + 1];
		    ...
		    int16_t ana[], enum Mode *used_mode, float synth[])
{
	...
	if (*used_mode == MRDTX) {
		...
		memzero(st->old_exc, (PIT_MAX + L_INTERPOL) << 2);
		memzero(st->mem_w0, M << 2);
		memzero(st->mem_err, M << 2);
		memzero(st->zero, L_SUBFR << 2);
		memzero(st->hvec, L_SUBFR << 2);
		memzero(st->lspSt->qSt->past_rq, M << 2);
	...
	for (subfrNr = 0, i_subfr = 0; subfrNr < 2; subfrNr++, i_subfr +=
		 ...
		 */
		pre_big(mode, gamma1, gamma1_12k2, gamma2, A_t, i_subfr,
	...
	for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR) {
		...
		if (*used_mode != MR475) {
			subframePreProc(*used_mode, gamma1, gamma1_12k2, gamma2,
		...
		if (*used_mode != MR475) {
			...
			if (evenSubfr != 0) {
				 ...
				 */
				subframePreProc(*used_mode, gamma1, gamma1_12k2,
						gamma2, A, Aq,
						&st->speech[i_subfr],
						st->mem_err, st->mem_w0,
						st->zero, st->ai_zero,
