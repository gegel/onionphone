		      ...
		      TXFrameType frame_type, enum Mode speech_mode)
{
	...
	*stream = toc_byte[mode];
	...
	if (mode == 15) {
		...
		*stream +=
		    (unsigned char)(((speech_mode & 0x0001) << 2) |
				    (speech_mode & 0x0002) |
			     ...
			     int force_speech)
{
	...
	if (noHoming) {
		Speech_Encode_Frame(s->encoderState, mode, speech, prm,
				    &used_mode);
	} else {
		switch (mode) {
		case MR122:
		...
		memzero(&prm[homing_size], (PRMNO_MR122 - homing_size) << 1);
		used_mode = mode;
	}
	if (used_mode == MRDTX) {
		...
		if (s->prev_ft == TX_SPEECH_GOOD) {
			...
			if ((s->sid_handover_debt > 0)
			    && (s->sid_update_counter > 2)) {
				/*
...
//process speech frame 
	if (noHoming)
		Speech_Encode_Frame(s->encoderState, MR475, speech, prm,
				    &used_mode);
	else			//process homing
	{
		for (i = 0; i < 7; i++)
			prm[i] = dhf_MR475[i];	//set homing for decoder
		/* rest of the parameters are zero */
		memzero(&prm[7], (PRMNO_MR475 - 7) << 1);
