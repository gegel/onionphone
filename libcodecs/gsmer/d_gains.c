		   ...
		   int16_t i_w_subfr, int16_t w_w_rx_dtx_w_state)
{
	...
	if (((w_rxdtx_ctrl & RX_NO_TRANSMISSION) != 0)
		...
		if ((w_rxdtx_ctrl & RX_NO_TRANSMISSION) != 0) {
			...
			if (i_w_subfr == 0) {
				*gain_code =
				    w_interpolate_CN_param(w_gain_code_old_CN,
							   w_gain_code_new_CN,
							   w_w_rx_dtx_w_state);
		...
		} else {	/* Invalid or lost SID frame:
		...
		if ((w_rxdtx_ctrl & RX_DTX_MUTING) != 0) {
			...
			w_gain_code_muting_CN =
			    w_mult(w_gain_code_muting_CN, 30057);
	...
	/*-------------------------------------------------------------------*
		...
		v_past_qua_en[0] =
		    w_add(v_past_qua_en[0], w_shl(w_sub(exp, 11), 10));
		...
		if (((w_rxdtx_ctrl & RX_FIRST_SID_UPDATE) != 0)
		    && (i_w_subfr == 0)) {
			w_gain_code_new_CN =
			    w_mult(w_gcode0_CN, w_qua_gain_code[index]);
