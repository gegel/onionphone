    ...
    )
{
	...
	w_Autocorr(w_w_p_window_mid, M, r_h, r_l, w_window_160_80);
	/* w_Autocorrelations */
	...
	/*---------------------------------------------------------------*
		...
		if ((w_txdtx_ctrl & TX_SP_FLAG) == 0) {
			...
			w_CN_w_excitation_gain =
			    compute_w_CN_w_excitation_gain(w_res2);
			...
			w_Syn_filt(Aq, &w_exc[i_w_subfr], w_error, L_SUBFR,
				   w_mem_err, 0);
	    ...
	    /*--------------------------------------------------------------*
			...
			T0 = w_Pitch_fr6(&w_exc[i_w_subfr], xn, w_h1, L_SUBFR,
					 T0_min, T0_max, pit_flag, &T0_frac);
			...
			*ana =
			    w_Enc_lag6(T0, &T0_frac, &T0_min, &T0_max, PIT_MIN,
				       PIT_MAX, pit_flag);
		...
		if ((w_txdtx_ctrl & TX_SP_FLAG) != 0) {
			...
			for (i = 0; i < L_SUBFR; i++) {
				...
				L_temp =
				    w_L_w_mult(w_exc[i + i_w_subfr], gain_pit);
				L_temp = w_L_w_shl(L_temp, 3);
				w_res2[i] =
				    w_sub(w_res2[i], w_extract_h(L_temp));
			}
		...
		*ana++ =
		    w_q_gain_code(code, L_SUBFR, &gain_code, w_txdtx_ctrl,
				  i_w_subfr);
	...
	/*------------------------------------------------------*
		...
		w_Syn_filt(Aq, &w_exc[i_w_subfr], &w_w_synth[i_w_subfr],
			   L_SUBFR, w_mem_w_syn, 1);
		...
		if ((w_txdtx_ctrl & TX_SP_FLAG) != 0) {
			...
			for (i = L_SUBFR - M, j = 0; i < L_SUBFR; i++, j++) {
				w_mem_err[j] =
				    w_sub(w_speech[i_w_subfr + i],
					  w_w_synth[i_w_subfr + i]);

				temp =
				    w_extract_h(w_L_w_shl
						(w_L_w_mult(y1[i], gain_pit),
						 3));
				k = w_extract_h(w_L_w_shl
						(w_L_w_mult(y2[i], gain_code),
						 5));
