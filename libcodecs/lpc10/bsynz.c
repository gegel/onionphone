...
*/
float kexc[26] = {
	0, 8, -16, 26, -48, 86, -162, 294, -502, 718, -728,
	   ...
	   float rms, float ratio, float g2pass)
{
	...
	float lpi0, hpi0;
	float a0 = .125, a1 = .75, a2 = .125 /*, a3=0 */ , b0 = -.125, b1 =
	    .25, b2 = -.125 /*, b3=0 */ ;
	float pulse, sscale, xssq, sum, ssq, gain;
	float xy;
	static float rmso = 0.0, lpi1 = 0.0, lpi2 = 0.0, /*lpi3, */ hpi1 =
	    0.0, hpi2 = 0.0 /*, hpi3 */ ;
...
/*  Load voiced excitation	*/
		...
		for (i = 0; i < ip; i++) {
			exc[ORDER + i] = 0.;
			if (i <= 25)
				exc[ORDER + i] = sscale * kexc[i];
			lpi0 = exc[ORDER + i];
			exc[ORDER + i] =
			    a0 * exc[ORDER + i] + a1 * lpi1 +
			    a2 * lpi2 /*+ a3*lpi3 */ ;
			/*lpi3 = lpi2; */
			lpi2 = lpi1;
			lpi1 = lpi0;
