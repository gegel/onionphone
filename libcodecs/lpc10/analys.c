...
*/
	...
	temp = 0;
	for (i = 1; i <= LFRAME; i++) {
		inbuf[SBUFH - LFRAME + i] =
		    (float)(*(speech + i) * 4096. - bias);
...
*/
	...
	voicing(inbuf, lpbuf + LPFILT_DELAY, vstart, vstart + LFRAME - 1,
		mintau, &voibuf[0][3], &voibuf[1][3]);
#endif
...
*/
	energy(ewin[1][AF - 1] - ewin[0][AF - 1] + 1,
