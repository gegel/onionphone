...

{
	...
	if (mf_syn_begin > mf_FRAME) {
	    ...
	    mf_v_equ(&mf_sigsave[0],&mf_sigsave[mf_FRAME],mf_syn_begin-mf_FRAME);
	}
	else 
	...
	mf_window(&par->fs_mf_mag[0],mf_mf_w_fs_inv,&par->fs_mf_mag[0],NUM_HARM);
    }
	...
	for (i = 0; i < NUM_GAINFR; i++) {
	    mf_noise_est(par->gain[i],&mf_noise_gain,UPCONST,DOWNCONST,
		      MIN_NOISE,MAX_NOISE);

	    /* Adjust gain based on noise level (noise suppression) */
	    mf_noise_sup(&par->gain[i],mf_noise_gain,MAX_NS_SUP,MAX_NS_ATT,NFACT);
	...
	if (gaincnt > 1) {
	    ...
	    gain = ifact_gain*par->gain[gaincnt-1] + 
		(1.0-ifact_gain)*mf_prev_par.gain[NUM_GAINFR-1];
	}
	
	/* Set overall interpolation path based on gain change */
	
	temp = par->gain[NUM_GAINFR-1] - mf_prev_par.gain[NUM_GAINFR-1];
	if (fabs(temp) > 6) {

	    /* Power surge: use gain adjusted interpolation */
	    intfact = (gain - mf_prev_par.gain[NUM_GAINFR-1]) / temp;
	    if (intfact > 1.0)
		intfact = 1.0;
	    if (intfact < 0.0)
		intfact = 0.0;
	...
	mf_interp_array(mf_prev_par.lsf,par->lsf,lsf,intfact,LPC_ORD+1);
	mf_lpc_lsp2pred(lsf,lpc,LPC_ORD);
	
	/* Check signal probability for adaptive spectral enhancement filter */
	sig_prob = mf_lin_int_bnd(gain,mf_noise_gain+12.0,mf_noise_gain+30.0,
			       0.0,1.0);
	...
	pitch = intfact*par->pitch + (1.0-intfact)*mf_prev_par.pitch;
	pulse_gain = SYN_GAIN*sqrt(pitch);
	
	/* interpolate pulse and noise coefficients */
	temp = sqrt(ifact);
	mf_interp_array(mf_prev_pcof,curr_pcof,pulse_cof,temp,MIX_ORD+1);
	mf_interp_array(mf_prev_ncof,curr_ncof,noise_cof,temp,MIX_ORD+1);
	...
	mf_fs_real[0] = 0.0;
	mf_interp_array(mf_prev_par.fs_mf_mag,par->fs_mf_mag,&mf_fs_real[1],intfact,
		     NUM_HARM);
	...
	mf_v_equ(mf_pulse_del,&mf_sigbuf[length+BEGIN-MIX_ORD],MIX_ORD);
	mf_zerflt(&mf_sigbuf[BEGIN],pulse_cof,&mf_sigbuf[BEGIN],MIX_ORD,length);
	...
	mf_v_equ(mf_noise_del,&mf_sig2[length+BEGIN-MIX_ORD],MIX_ORD);
	mf_zerflt(&mf_sig2[BEGIN],noise_cof,&mf_sig2[BEGIN],MIX_ORD,length);
	...
	mf_lpc_synthesis(&mf_sigbuf[BEGIN],&mf_sigbuf[BEGIN],ase_den,LPC_ORD,length);
	mf_v_equ(mf_ase_del,&mf_sigbuf[BEGIN+length-LPC_ORD],LPC_ORD);
	mf_zerflt(&mf_sigbuf[BEGIN],ase_num,&mf_sigbuf[BEGIN],LPC_ORD,length);
	mf_v_equ(&mf_sigbuf[BEGIN-TILT_ORD],mf_tilt_del,TILT_ORD);
	mf_v_equ(mf_tilt_del,&mf_sigbuf[length+BEGIN-TILT_ORD],TILT_ORD);
	mf_zerflt(&mf_sigbuf[BEGIN],tilt_cof,&mf_sigbuf[BEGIN],TILT_ORD,length);
	
	/* Perform LPC synthesis filtering */
	mf_v_equ(&mf_sigbuf[BEGIN-LPC_ORD],mf_lpc_del,LPC_ORD);
	mf_lpc_synthesis(&mf_sigbuf[BEGIN],&mf_sigbuf[BEGIN],lpc,LPC_ORD,length);
	mf_v_equ(mf_lpc_del,&mf_sigbuf[length+BEGIN-LPC_ORD],LPC_ORD);
		
	/* Adjust scaling of synthetic mf_speech */
	mf_scale_adj(&mf_sigbuf[BEGIN],gain,&mf_premf_v_scale,length,SCALEOVER);

	/* Implement pulse dispersion filter */
	mf_v_equ(&mf_sigbuf[BEGIN-DISP_ORD],mf_disp_del,DISP_ORD);
	mf_v_equ(mf_disp_del,&mf_sigbuf[length+BEGIN-DISP_ORD],DISP_ORD);
	mf_zerflt(&mf_sigbuf[BEGIN],mf_disp_cof,&mf_sigbuf[BEGIN],DISP_ORD,length);
		
	/* Copy processed mf_speech to output array (not past frame end) */
	if (mf_syn_begin+length > mf_FRAME) {
	    mf_v_equ(&sp_out[mf_syn_begin],&mf_sigbuf[BEGIN],mf_FRAME-mf_syn_begin);
				
	    /* past end: save remainder in mf_sigsave[0] */
	    mf_v_equ(&mf_sigsave[0],&mf_sigbuf[BEGIN+mf_FRAME-mf_syn_begin],
		  length-(mf_FRAME-mf_syn_begin));
	}
			
	else
	    mf_v_equ(&sp_out[mf_syn_begin],&mf_sigbuf[BEGIN],length);
...
void mf_mf_melp_syn_init()
{
    ...
    for (i = 1; i < LPC_ORD+1; i++)
      mf_prev_par.lsf[i] = mf_prev_par.lsf[i-1] + (1.0/(LPC_ORD+1));
    ...
    if (mf_fsvq_weighted == 0)
	  ...
	  for (i = 0; i < mf_fs_mf_vq_par.num_levels[0]; i++)
	    mf_window(&mf_fs_mf_vq_par.cb[i*NUM_HARM],mf_w_fs,&mf_fs_mf_vq_par.cb[i*NUM_HARM],
		   NUM_HARM);
      }
