...
void mf_melp_chn_write(struct mf_melp_param *par)
{
	...
	for (i = 0; i < par->msvq_stages; i++)
		mf_pack_code(par->msvq_index[i], &bit_ptr, &bit_cntr,
			     par->msvq_bits[i], 1);

	mf_pack_code(par->fsvq_index[0], &bit_ptr, &bit_cntr, FS_BITS, 1);

	/*  Write channel output buffer     */
	for (i = 0; i < NUM_CH_BITS; i++) {
		mf_pack_code(mf_bit_buffer[mf_bit_order[i]], &par->chptr,
			     &par->chbit, 1, CHWORDSIZE);
		if (i == 0)
...
int mf_melp_chn_read(struct mf_melp_param *par,
		     struct mf_melp_param *mf_prev_par)
{
	int erase = 0;
	int i, bit_cntr;
	unsigned int *bit_ptr;
	...
	for (i = 0; i < NUM_CH_BITS; i++) {
		erase |=
		    unmf_pack_code(&par->chptr, &par->chbit,
				   (int *)&mf_bit_buffer[mf_bit_order[i]], 1,
	...
	unmf_pack_code(&bit_ptr, &bit_cntr, &par->fsvq_index[0], FS_BITS, 1, 0);
	...
	if (erase) {
		...
		*par = *mf_prev_par;
		...
		mf_vq_msd2(mf_msvq_cb, &par->lsf[1], (float *)NULL,
			   (float *)NULL, par->msvq_index, par->msvq_levels,
			   par->msvq_stages, LPC_ORD, 0);
		i = FS_LEVELS;
		if (par->uv_flag)
			mf_fill(par->fs_mf_mag, 1., NUM_HARM);
		else {
			/* Decode Fourier mf_magnitudes */
			mf_vq_msd2(mf_fsvq_cb, par->fs_mf_mag, (float *)NULL,
				   (float *)NULL, par->fsvq_index, &i, 1,
				   NUM_HARM, 0);
		}

		/* Decode gain terms with uniform log quantizer */
		mf_mf_q_gain_dec(par->gain, par->gain_index, GN_QLO, GN_QUP,
				 GN_QLEV);
		...
		if (par->uv_flag)
			...
			mf_mf_quant_u_dec(par->pitch_index, &par->pitch,
					  PIT_QLO, PIT_QUP, PIT_QLEV);
			par->pitch = pow(10.0, par->pitch);
		}

		/* Decode jitter and bandpass voicing */
		mf_mf_quant_u_dec(par->jit_index, &par->jitter, 0.0, MAX_JITTER,
				  2);
