...
float mf_double_chk(float sig_in[], float *pcorr, float pitch, float pdouble, int pmin, int pmax, int lmin)

{
	...
	if (temp_pit >= pmin) {
	    ...
	    mf_double_ver(sig_in,&corr,temp_pit,pmin,pmax,lmin);
		...
		pitch = mf_frac_pch(sig_in,pcorr,temp_pit,0,pmin,pmax,lmin);
		break;
	    }
	}
    }
...
void mf_double_ver(float sig_in[], float *pcorr, float pitch, int pmin, int pmax, int lmin)
...
float mf_frac_pch(float sig_in[], float *pcorr, float mf_fpitch, int range, int pmin, int pmax, int lmin)

{
    ...
    c0_T1 = mf_v_inner(&sig_in[cbegin],&sig_in[cbegin+ipitch+1],length);
    c0_Tm1 = mf_v_inner(&sig_in[cbegin],&sig_in[cbegin+ipitch-1],length);
    if (c0_Tm1 > c0_T1) {
    ...
    cT_T1 = mf_v_inner(&sig_in[cbegin+ipitch],&sig_in[cbegin+ipitch+1],length);
    c0_0 = mf_v_inner(&sig_in[cbegin],&sig_in[cbegin],length);
    cT_T = mf_v_inner(&sig_in[cbegin+ipitch],&sig_in[cbegin+ipitch],length);
    cT1_T1 = mf_v_inner(&sig_in[cbegin+ipitch+1],&sig_in[cbegin+ipitch+1],length);
...

{
	...
	for (i = 0; i < mf_NUM_GOOD; i++)
	  mf_good_pitch[i] = (mf_PDECAY*mf_good_pitch[i]) +((1.0-mf_PDECAY)*mf_DEFAULT_PITCH);
    }
...
float mf_pitch_ana(float mf_speech[], float resid[], float pitch_est, float mf_pitch_avg, float *pcorr2)

{
	...
	mf_v_equ(&mf_sigbuf[mf_LPF_ORD],&mf_speech[-mf_PITCHMAX],mf_PITCH_FR);
	temp = mf_frac_pch(&mf_sigbuf[mf_LPF_ORD+(mf_PITCH_FR/2)],&pcorr,
			pitch_est,0,mf_PITCHMIN,mf_PITCHMAX,mf_LMIN);
	...
	if (pcorr < UVMAX)
	  ...
	  pitch = mf_pitch_avg;
	    ...
	    if (temp > LONG_PITCH)
	      ...
	      temp2 = PDOUBLE4;
	    pitch = mf_double_chk(&mf_sigbuf[mf_LPF_ORD+(mf_PITCH_FR/2)],&pcorr,
				    temp,temp2,mf_PITCHMIN,mf_PITCHMAX,mf_LMIN);
	}
    }
