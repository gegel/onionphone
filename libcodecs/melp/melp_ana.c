...
void mf_melp_ana(float sp_in[], struct mf_melp_param *par)
{
	...
	mf_fpitch[END] =
	    mf_find_pitch(&mf_sigbuf[mf_LPF_ORD + (mf_PITCH_FR / 2)], &temp,
			  (2 * mf_PITCHMIN), mf_PITCHMAX, mf_PITCHMAX);

	/* Perform bandpass voicing analysis for end of frame */
	mf_bpvc_ana(&mf_speech[mf_FRAME_END], mf_fpitch, &par->bpvc[0],
		    &sub_pitch);
	...
	mf_window(&mf_speech[(mf_FRAME_END - (LPC_mf_FRAME / 2))], mf_win_cof,
		  mf_sigbuf, LPC_mf_FRAME);
	...
	mf_zerflt(&mf_speech[PITCH_BEG], lpc, &mf_sigbuf[mf_LPF_ORD], LPC_ORD,
		  mf_PITCH_FR);
	...
	par->pitch =
	    mf_pitch_ana(&mf_speech[mf_FRAME_END],
			 &mf_sigbuf[mf_LPF_ORD + mf_PITCHMAX], sub_pitch,
			 mf_pitch_avg, &pcorr);
	bpthresh = BPTHRESH;
	...
	for (i = 0; i < NUM_GAINFR; i++) {
		if (par->bpvc[0] > bpthresh) {
			...
			temp = sub_pitch;
			par->gain[i] =
			    mf_gain_ana(&mf_speech
					[mf_FRAME_BEG + (i + 1) * GAINFR], temp,
					MIN_GAINFR, 2 * mf_PITCHMAX);
		} else {
	...
	if (par->bpvc[0] > bpthresh) {
		...
		mf_zerflt(&mf_speech[(mf_FRAME_END - (LPC_mf_FRAME / 2))], lpc,
			  mf_sigbuf, LPC_ORD, LPC_mf_FRAME);
		mf_window(mf_sigbuf, mf_win_cof, mf_sigbuf, LPC_mf_FRAME);
		mf_find_harm(mf_sigbuf, par->fs_mf_mag, par->pitch, NUM_HARM,
			     LPC_mf_FRAME);
	}
...
void mf_mf_melp_ana_init()
{
	...
	mf_mf_bpvc_ana_init(mf_FRAME, mf_PITCHMIN, mf_PITCHMAX, mf_NUM_BANDS, 2,
			    MINLENGTH);
	mf_mf_pitch_ana_init(mf_PITCHMIN, mf_PITCHMAX, mf_FRAME, mf_LPF_ORD,
			     MINLENGTH);
	...
	if (mf_fsvq_weighted == 0) {
		...
		for (j = 0; j < mf_fs_mf_vq_par.num_levels[0]; j++)
			mf_window(&mf_fs_mf_vq_par.cb[j * NUM_HARM], mf_w_fs,
				  &mf_fs_mf_vq_par.cb[j * NUM_HARM], NUM_HARM);
	}
