...
void AMR475_decode(void *st, UWord8 *serial, Word16 *synth, int bfi)
{
 ...
 if(!serial) serial=temp; //process bad frame if parameter ommited
 if(0x01 & serial[0]) //check SID flag
 {
  speech_mode = MRDTX; 
  frame_type = RX_SID_FIRST; //mark as short sid frame
 }
 if(!(*((unsigned int*)serial))) bfi=1;    //set bfi if zeroed data (or ommited)
 if ( bfi == 1 ) frame_type = RX_SPEECH_BAD; //set type for CN generation
 
 //unpack bits ro parameters
 memset(prm, 0, 2*PRMNO_MR475);
 if(speech_mode == MRDTX) //for sid frame (5 bytes)
 {
  ...
  for ( i = 0; i < PRMNO_MRDTX; i++ ) //read each parameter
  {
   ...
   {
    if(serial[bits>>3]&(((UWord8)1)<<(bits&7))) prm[i]|=((Word16)1<<j);
    bits++;   
   }
  }  
 }
...
/* test for homing frame */
 if ( s->reset_flag_old == 1 ) for ( i = 0; i < 7; i++ ) 
 {
  resetFlag = prm[i] ^ dhf_MR475[i];
  if ( resetFlag ) break;
 }
 if ( ( resetFlag == 0 ) && ( s->reset_flag_old != 0 ) ) 
 for ( i = 0; i < 160; i++ ) synth[i] = EHF_MASK; //duplicate homing - skip it
 else Speech_Decode_Frame( s->decoder_State, MR475, prm, frame_type, synth ); //decode
  /* check whole frame for homing */
 if ( s->reset_flag_old == 0 ) for ( i = 0; i < PRMNO_MR475; i++ ) 
 {
  resetFlag = prm[i] ^  dhf_MR475[i];
  if ( resetFlag ) break;
 }
 if ( resetFlag == 0 ) Speech_Decode_Frame_reset( s->decoder_State ); //reset decoder  
...
/* test for homing frame */
 if ( s->reset_flag_old == 1 ) for ( i = 0; i < homing_size; i++ ) 
 {
  resetFlag = prm[i] ^ dhf[i];
  if ( resetFlag ) break;
 }
 if ( ( resetFlag == 0 ) && ( s->reset_flag_old != 0 ) ) //duplicate homing - skip it
 for ( i = 0; i < 160; i++ ) synth[i] = EHF_MASK; 
 else Speech_Decode_Frame( s->decoder_State, speech_mode, prm, frame_type, synth ); //decode
  /* check whole frame for homing */
 if ( s->reset_flag_old == 0 ) for ( i = 0; i < prmno; i++ ) 
 {
  resetFlag = prm[i] ^  dhf[i];
  if ( resetFlag ) break;
 }
