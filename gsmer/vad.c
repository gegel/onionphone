...
)
{
    ...
    w_er_energy_computation (r_h, scal_acf, w_rvad, scal_w_rvad, &acf0, &pvad);
    ...
    w_er_threshold_adaptation (stat, w_ptch, tone, rav1, scal_rav1, pvad, acf0,
                          w_rvad, &scal_w_rvad, &w_thvad);
...
)
{
    ...
    for (n = 0; n < 8; n++)
    {
        ...
        for (m = 1; m <= 7 - n; m++)
        {
            pp[m] = w_add (pp[1 + m], w_w_mult_r (kk[9 - m], vpar[n]));
                                                          
            kk[9 - m] = w_add (kk[9 - m], w_w_mult_r (pp[1 + m], vpar[n]));
...
Word16 w_er_spectral_comparison (
    Word16 rav1[],
    Word16 scal_rav1,
    Word32 L_av0[]
)
...
void w_er_threshold_adaptation (
    Word16 stat,
    Word16 w_ptch,
    Word16 tone,
    Word16 rav1[],
...
)
{
    ...
    {
        L_temp = L_w_add (w_L_deposit_l (pvad.m), w_L_deposit_l (M_MARGIN));
    ...
    else
    {
        ...
        if (w_sub (pvad.e, E_MARGIN) > 0)
        {
            ...
            L_temp = L_w_add (w_L_deposit_l (pvad.m), w_L_deposit_l (temp));
        ...
        else
        {
            ...
            L_temp = L_w_add (w_L_deposit_l (M_MARGIN), w_L_deposit_l (temp));
