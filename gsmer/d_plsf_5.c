...
)
{
        ...
        || ((w_rxdtx_ctrl & RX_LOST_SID_FRAME) != 0))
    {
        ...
        if ((w_rxdtx_ctrl & RX_NO_TRANSMISSION) != 0)
        {
            ...
            w_interpolate_CN_lsf (w_lsf_old_CN, w_lsf_new_CN, lsf2_q, w_w_rx_dtx_w_state);
        }
        else
    ...
    if (bfi != 0)                               /* if bad frame */
    {
        ...
        for (i = 0; i < M; i++)
        {
            ...
            temp = w_add (w_mean_lsf[i], w_mult (v_past_r2_q[i], PRED_FAC));
    ...
    if ((w_rxdtx_ctrl & RX_SP_FLAG) != 0)
    {
        ...
        {
            w_update_lsf_history (w_lsf_new_CN, w_lsf_new_CN, w_lsf_old_rx);
        }
