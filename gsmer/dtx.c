    ...
    else
    {
        ...
        if ((*w_rxdtx_ctrl & RX_CONT_SID_UPDATE) != 0)
        {
            ...
            else if (w_sub (frame_type, INVALID_SID_FRAME) == 0)
            {
                ...
                {
                    w_w_rx_dtx_w_state = w_add(w_w_rx_dtx_w_state, 1);  
                }
            }
        }
        ...
        if ((*w_rxdtx_ctrl & RX_CNI_BFI) != 0)
        {
            ...
            if (w_sub (TAF, 1) == 0)
            {
                *w_rxdtx_ctrl = *w_rxdtx_ctrl | RX_LOST_SID_FRAME;
                                             
                w_prev_SID_frames_lost = w_add (w_prev_SID_frames_lost, 1);
            }
            else /* No transmission occurred */
            {
                *w_rxdtx_ctrl = *w_rxdtx_ctrl | RX_NO_TRANSMISSION;
...
)
{
    ...
    if (w_sub (nbr_w_errors, VALID_SID_THRESH) < 0)
    {                                                   /* Valid SID frame */
        sid = 2;                                          
    }
    else if (w_sub (nbr_w_errors, INVALID_SID_THRESH) < 0)
    {                                                   /* Invalid SID frame */
        sid = 1;                                          
    }
    else
...
)
{
    ...
    for (j = 0; j < M; j++)
    {
        ...
        {
            L_temp = w_L_mac (L_temp, INV_DTX_HANGOVER, lsf_old[i][j]);
        }
...
)
{
    ...
    for (j = 0; j < M; j++)
    {
        ...
        for (i = 0; i < DTX_HANGOVER; i++)
        {
            L_temp = w_L_mac (L_temp, INV_DTX_HANGOVER_P1, lsf_old[i][j]);
        }
...
void w_update_gain_code_history_tx (
...
void w_update_gain_code_history_rx (
    Word16 new_gain_code,
...
Word16 compute_w_CN_w_excitation_gain (
    Word16 w_res2[L_SUBFR]
)
{
...
)
{
    ...
    for (i = 0; i < DTX_HANGOVER; i++)
    {
        ...
        for (j = 1; j < 4; j++)
        {
            L_temp = w_L_mac (L_temp, 0x1fff, gain_code_old[4 * i + j]);
        }
...
Word16 w_aver_gain_code_history (
    Word16 w_CN_w_excitation_gain,
    Word16 gain_code_old[4 * DTX_HANGOVER]
)
{
    Word16 i;
...
void w_interpolate_CN_lsf (
    Word16 w_lsf_old_CN[M],
