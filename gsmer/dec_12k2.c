...
void w_Decoder_12k2 (
    Word16 w_parm[], /* input : vector of w_w_synthesis parameters
...
)
{
    ...
    */
    ...
    w_D_plsf_5 (w_parm, lsp_mid, lsp_new, bfi, w_rxdtx_ctrl, w_w_rx_dtx_w_state);
    ...
    for (i_w_subfr = 0; i_w_subfr < L_FRAME; i_w_subfr += L_SUBFR)
    {
        ...
        if ((w_rxdtx_ctrl & RX_SP_FLAG) != 0)
        {
            T0 = w_Dec_lag6 (index, PIT_MIN, PIT_MAX, i_w_subfr, L_FRAME_BY2,
                           &T0_frac, bfi);
#if (WMOPS)
                          /* function worst case */
#endif
        ...
        gain_pit = w_d_gain_pitch (index, bfi, w_state, w_prev_bf, w_rxdtx_ctrl);
        ...
        {   /* Use pseudo noise for v_excitation when SP_flag == 0 */
        ...
        w_d_gain_code (index, code, L_SUBFR, &gain_code, bfi, w_state, w_prev_bf,
                     w_rxdtx_ctrl, i_w_subfr, w_w_rx_dtx_w_state);
#if (WMOPS)
                          /* function worst case */
#endif
        ...
        if (w_sub (pit_sharp, 16384) > 0)
        {
            for (i = 0; i < L_SUBFR; i++)
            {
                v_excp[i] = w_add (v_excp[i], v_exc[i + i_w_subfr]);
                                          
            }
            w_w_agc2 (&v_exc[i_w_subfr], v_excp, L_SUBFR);
            w_Syn_filt (Az, v_excp, &w_w_synth[i_w_subfr], L_SUBFR, v_mem_v_syn, 1);
        }
        else
        {
            w_Syn_filt (Az, &v_exc[i_w_subfr], &w_w_synth[i_w_subfr], L_SUBFR, v_mem_v_syn, 1);
        }
