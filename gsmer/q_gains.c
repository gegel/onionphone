...
)
{
    ...
    if ((w_txdtx_ctrl & TX_SP_FLAG) != 0)
    {
        ...
        err_min = w_abs_s (w_sub (*gain, w_mult (gcode0, w_qua_gain_code[0])));
        index = 0;                

        for (i = 1; i < NB_QUA_CODE; i++)
        {
            err = w_abs_s (w_sub (*gain, w_mult (gcode0, w_qua_gain_code[i])));
        ...
        *gain = w_mult (gcode0, w_qua_gain_code[index]);
        ...
        w_past_qua_en[0] = w_add (w_past_qua_en[0], w_shl (w_sub (exp, 11), 10));
    ...
    else
    {
        ...
        if ((w_txdtx_ctrl & TX_PREV_HANGOVER_ACTIVE) != 0 && (i_w_subfr == 0))
        {
        ...
        *gain = w_CN_w_excitation_gain;               
        ...
        {
            aver_gain = w_aver_gain_code_history (w_CN_w_excitation_gain,
                                                w_gain_code_old_tx);
            ...
            err_min = w_abs_s (w_sub (aver_gain, 
                                  w_mult (w_gcode0_CN, w_qua_gain_code[0])));
            index = 0;                            

            for (i = 1; i < NB_QUA_CODE; i++)
            {
                err = w_abs_s (w_sub (aver_gain, 
                                  w_mult (w_gcode0_CN, w_qua_gain_code[i])));
