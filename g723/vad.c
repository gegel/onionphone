...
Flag Comp_Vad( Word16 *Dpnt)
{
    ...
    for ( i = SubFrLen ; i < Frame ; i ++ ) {
        ...
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723_L_msu( Acc0, Dpnt[i-j-1], VadStat.NLpc[j] ) ;
        Tm0 = round_ ( Acc0 ) ;
    ...
    if ( !VadStat.Aen ) {
        VadStat.Nlev = L_g723_add( VadStat.Nlev, L_g723_shr( VadStat.Nlev, 5 ) ) ;
    }
    /* Decay Nlev by small amount */
    else {
        VadStat.Nlev = L_g723_sub( VadStat.Nlev, L_g723_shr( VadStat.Nlev,11 ) ) ;
    }
