...
Word32   Lsp_Qnt( Word16 *CurrLsp, Word16 *PrevLsp )
{
  ...
  */
    ...
    Wvect[LpcOrder-1] = g723_sub( CurrLsp[LpcOrder-1], CurrLsp[LpcOrder-2] ) ;
...
*/ 
            for ( j = 0 ; j < BandInfoTable[k][1] ; j ++ )
                Tmp[j] = g723_mult_r( Wvect[BandInfoTable[k][0]+j],
                                                            LspQntPnt[j] ) ;

            Acc0 = (Word32) 0 ;
            for ( j = 0 ; j < BandInfoTable[k][1] ; j ++ )
                Acc0 = g723_L_mac( Acc0, Tv[BandInfoTable[k][0]+j], Tmp[j] ) ;
...
void  LsptoA( Word16 *Lsp )
{
  ...
  */
        ...
        Acc0 = g723_L_mac( Acc0, Tmp, g723_add( g723_shl( (Word16)(Lsp[i] & 0x007f) ,
  ...
  */
    for ( i = 2 ; i < LpcOrder/2 ; i ++ ) {
        ...
        for ( j = i ; j >= 2 ; j -- ) {
            ...
            Acc0 = L_g723_add( Acc0, L_g723_shr(P[j-2], (Word16) 1 ) ) ;
            P[j] = Acc0 ;

            Acc1 = Q[j-1] ;
            Acc1 = L_mls( Acc1, Lsp[2*i+1] ) ;
            Acc1 = L_g723_add( Acc1, L_g723_shr(Q[j], (Word16) 1 ) ) ;
            Acc1 = L_g723_add( Acc1, L_g723_shr(Q[j-2], (Word16) 1 ) ) ;
            Q[j] = Acc1 ;
        }
