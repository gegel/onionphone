...
Word16   Estim_Pitch( Word16 *Dpnt, Word16 Start )
{
    ...
    for ( i = PitchMin ; i <= PitchMax-3 ; i ++ ) {
        ...
        /* Energy update */
        Acc1 = g723_L_msu( Acc1, Dpnt[Pr+2*SubFrLen], Dpnt[Pr+2*SubFrLen] ) ;
            ...
            if ( Exp <= Mxp ) {
                ...
                if ( Acc0 > (Word32) 0 ) {
                    ...
                    if ( ((Word16)i - Indx) < (Word16) PitchMin ) {
                        ...
                        Acc0 = L_g723_mult( Ccr, Mnr ) ;
                        Acc0 = L_g723_negate(L_g723_shr( Acc0, (Word16) 2 ) ) ;
                        Acc0 = g723_L_mac( Acc0, Ccr, Mnr ) ;
                        Acc0 = g723_L_msu( Acc0, Enr, Tmp ) ;
                        if ( Acc0 > (Word32) 0 ) {
                            Indx = (Word16) i ;
                            Mxp = Exp ;
...
PWDEF Comp_Pw( Word16 *Dpnt, Word16 Start, Word16 Olp )
{
    ...
    for ( i = 0 ; i <= 2*PwRange ; i ++ ) {
        ...
        for ( j = 0 ; j < SubFrLen ; j ++ ) {
            ...
            Acc1 = g723_L_mac( Acc1, Dpnt[Start-(Olp-PwRange+i)+j],
                                            Dpnt[Start-(Olp-PwRange+i)+j] ) ;
        }
...
void  Filt_Pw( Word16 *DataBuff, Word16 *Dpnt, Word16 Start, PWDEF Pw )
{
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        ...
        Acc0 = g723_L_msu( Acc0, Pw.Gain, Dpnt[PitchMax+Start-Pw.Indx+i] ) ;
        ...
        for ( i = 1 ; i <=2*MlqSteps ; i ++ ) {
            ...
            for ( j = 1 ; j < Np ; j ++ ) {
                ...
                for ( l = k ; l < SubFrLen ; l += Sgrid ) {
                    ...
                    Acc0 = WrkBlk[l] ;
                    Acc0 = g723_L_msu( Acc0, Temp.Pamp[j-1],
                            ImrCorr[g723_abs_s((Word16)(l-Temp.Ploc[j-1]))] ) ;
                    WrkBlk[l] = Acc0 ;
                    Acc0 = g723_L_abs( Acc0 ) ;
                    if ( Acc0 > Acc1 ) {
                        Acc1 = Acc0 ;
            ...
            for ( l = SubFrLen-1 ; l >= 0 ; l -- ) {
                Acc0 = (Word32) 0 ;
                for ( j = 0 ; j <= l ; j ++ )
                    Acc0 = g723_L_mac( Acc0, OccPos[j], Imr[l-j] ) ;
            ...
            for ( j = 0 ; j < SubFrLen ; j ++ ) {
                ...
                Acc1 = L_g723_sub( Acc1, L_g723_shr( Acc0, (Word16) 1 ) ) ;
            }
...
void  Fcbk_Pack( Word16 *Dpnt, SFSDEF *Sfs, BESTDEF *Best, Word16 Np )
{
    ...
    for ( i = 0 ; i < SubFrLen/Sgrid ; i ++ ) {
        ...
        if ( Dpnt[(int)(*Best).GridId + Sgrid*i] == (Word16) 0 )
            (*Sfs).Ppos = L_g723_add( (*Sfs).Ppos, CombinatorialTable[j][i] ) ;
        else {
    ...
    switch(WrkRate)  {
        case Rate63: {
            ...
            for ( i = 0 ; i < SubFrLen/Sgrid ; i ++ )  {
                ...
                Acc0 = L_g723_sub( Acc0, CombinatorialTable[j][i] ) ;

                if ( Acc0 < (Word32) 0 ) {
                    Acc0 = L_g723_add( Acc0, CombinatorialTable[j][i] ) ;
                    j ++ ;
        ...
        case Rate53: {
            ...
            if(T0_acelp <SubFrLen-2) {
                ...
                for (i = T0_acelp ; i < SubFrLen; i++)
                    Tv[i] = g723_add(Tv[i], g723_mult(Tv[i-T0_acelp ], gain_T0));
            }
...
*Line, Word16 Sfc )
{
    ...
    for ( k = 0 ; k < (int) Hb ; k ++ ) {
        ...
        for ( i = 0 ; i < SubFrLen ; i ++ ) {
            Acc0 = (Word32) 0 ;
            for ( j = 0 ; j <= i ; j ++ )
                Acc0 = g723_L_mac( Acc0, RezBuf[ClPitchOrd-1+j], ImpResp[i-j] ) ;
        ...
        for ( i = 0 ; i < ClPitchOrd ; i ++ ) {
            ...
            for ( j = 0 ; j < SubFrLen ; j ++ ) {
                ...
                Acc1 = L_g723_add( Acc1, L_g723_shr( Acc0, (Word16) 1 ) ) ;
            }
        ...
        for ( i = 0 ; i < ClPitchOrd ; i ++ ) {
            Acc1 = (Word32) 0 ;
            for ( j = 0 ; j < SubFrLen ; j ++ )
                Acc1 = g723_L_mac( Acc1, FltBuf[i][j], FltBuf[i][j] ) ;
            *lPnt ++ = Acc1 ;
        }
        ...
        for ( i = 1 ; i < ClPitchOrd ; i ++ ) {
            for ( j = 0 ; j < i ; j ++ ) {
                ...
                for ( l = 0 ; l < SubFrLen ; l ++ ) {
                    Acc0 = L_g723_mult( FltBuf[i][l], FltBuf[j][l] ) ;
                    Acc1 = L_g723_add( Acc1, L_g723_shr( Acc0, (Word16) 1 ) ) ;
                }
    ...
    for ( k = 0 ; k < (int) Hb ; k ++ ) {
        ...
        for ( i = 0 ; i < (int) Bound[l] ; i ++ ) {
            ...
            Acc0 = (Word32) 0 ;
            for ( j = 0 ; j < 20 ; j ++ )
                Acc0 = L_g723_add( Acc0, L_g723_shr( L_g723_mult(CorVct[k*20+j], *sPnt ++),
(Word16) 1 ) ) ;
...
Word16   Comp_Info( Word16 *Buff, Word16 Olp, Word16 *Gain, Word16 *ShGain)
{
    ...
    *ShGain = Vec_Norm( Buff, (Word16) (PitchMax+Frame) ) ;
    ...
    for ( i = (int)Olp-3 ; i <= (int)Olp+3 ; i ++ ) {
        ...
        Acc0 = (Word32) 0 ;
        for ( j = 0 ; j < 2*SubFrLen ; j ++ )
            Acc0 = g723_L_mac( Acc0, Buff[PitchMax+Frame-2*SubFrLen+j],
    ...
    *Gain = Tenr;
    ...
    Acc0 = (Word32) 0 ;
    for ( j = 0 ; j < 2*SubFrLen ; j ++ )
        Acc0 = g723_L_mac( Acc0, Buff[PitchMax+Frame-2*SubFrLen-(int)Indx+j],
...
Word16 Ecount, Word16 *Sd )
{
    ...
    if ( Ecount >= (Word16) ErrMaxNum ) {
        ...
        if ( Lag != (Word16) 0 ) {
            ...
            for ( i = 0 ; i < Frame ; i ++ )
                DataBuff[i] = Buff[PitchMax+i] = g723_mult( Buff[PitchMax+i],
                            (Word16) 0x6000 ) ;
        }
        else {
            /* Unvoiced case */
...
PFDEF Comp_Lpf( Word16 *Buff, Word16 Olp, Word16 Sfc )
{
    ...
    Acc0 = (Word32) 0 ;
    for ( j = 0 ; j < SubFrLen ; j ++ )
        Acc0 = g723_L_mac( Acc0, Buff[PitchMax+(int)Sfc*SubFrLen+j],
    ...
    if ( Bindx != (Word16) 0 ) {
        ...
        for ( j = 0 ; j < SubFrLen ; j ++ ) {
            ...
            Acc1 = g723_L_mac( Acc1, Buff[PitchMax+(int)Sfc*SubFrLen+(int)Bindx+j],
                        Buff[PitchMax+(int)Sfc*SubFrLen+(int)Bindx+j] ) ;
        }
        Lcr[1] = Acc0 ;
        Lcr[2] = Acc1 ;
    ...
    if ( Findx != (Word16) 0 ) {
        ...
        for ( j = 0 ; j < SubFrLen ; j ++ ) {
            ...
            Acc1 = g723_L_mac( Acc1, Buff[PitchMax+(int)Sfc*SubFrLen+(int)Findx+j],
                        Buff[PitchMax+(int)Sfc*SubFrLen+(int)Findx+j] ) ;
        }
        Lcr[3] = Acc0 ;
        Lcr[4] = Acc1 ;
...
PFDEF Get_Ind( Word16 Ind, Word16 Ten, Word16 Ccr, Word16 Enr )
{
    ...
    if ( Acc1 > Acc0 ) {
        ...
        if ( Ccr >= Enr )
            ...
            Pf.Gain = g723_mult( Pf.Gain, LpfConstTable[(int)WrkRate] ) ;
        }
...
void  Filt_Lpf( Word16 *Tv, Word16 *Buff, PFDEF Pf, Word16 Sfc )
{
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        Acc0 = L_g723_mult( Buff[PitchMax+(int)Sfc*SubFrLen+i], Pf.ScGn ) ;
        Acc0 = g723_L_mac( Acc0, Buff[PitchMax+(int)Sfc*SubFrLen+(int)Pf.Indx+i],
                                                                Pf.Gain ) ;
    ...
    *ind_gain = G_code(X, rr, &gain_q);
    ...
    if(T0 < SubFrLen-2)
        for (i = T0; i < SubFrLen; i++)    /* code[i] += gain_T0*code[i-T0] */
            code[i] = g723_add(code[i], g723_mult(code[i-T0], gain_T0));
...
void Cor_h(Word16 *H, Word16 *rr)
{
    ...
    if(g723_extract_h(cor) > 32000 ) {
        for(i=0; i<SubFrLen; i++) h[i+4] = g723_shr(H[i], 1);
    }
    else {
        k = g723_norm_l(cor);
        k = g723_shr(k, 1);
        for(i=0; i<SubFrLen; i++) h[i+4] = g723_shl(H[i], k);
    }
  ...
  */
    ...
    cor    = 0;
    for(i=0;  i<NB_POS; i++) {
        cor = g723_L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
        cor = g723_L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
  ...
  */
    ...
    for(k=0; k<NB_POS; k++) {
        ...
        for(i=k+(Word16)1; i<NB_POS; i++ ) {
            ...
            cor = g723_L_mac(cor, *ptr_h1, *ptr_h2); ptr_h1++; ptr_h2++;
            cor = g723_L_mac(cor, *ptr_h1, *ptr_h2); ptr_h1++; ptr_h2++;
            *p3 = g723_extract_h(cor);
                 ...
                 Word16 y[], Word16 *code_shift, Word16 *sign)
{
  ...
  */
    ...
    for(i0=0; i0<SubFrLen/2; i0+=STEP/2) {
        for(i1=2/2; i1<SubFrLen/2; i1+=STEP/2) {
            *ptr_ri0i1++ = i_g723_mult(*ptr1_ri0i1++,
                                    i_g723_mult(p_sign[i0], p_sign[i1]));
            *ptr_ri0i2++ = i_g723_mult(*ptr1_ri0i2++,
                                    i_g723_mult(p_sign[i0], p_sign[i1+1]));
            *ptr_ri0i3++ = i_g723_mult(*ptr1_ri0i3++,
                                    i_g723_mult(p_sign[i0], p_sign[i1+2]));
        }
    }
    ...
    for(i1=2/2; i1<SubFrLen/2; i1+=STEP/2) {
        for(i2=4/2; i2<SubFrLen2/2; i2+=STEP/2) {
            *ptr_ri1i2++ = i_g723_mult(*ptr1_ri1i2++,
                                    i_g723_mult(p_sign[i1], p_sign[i2]));
            *ptr_ri1i3++ = i_g723_mult(*ptr1_ri1i3++,
                                    i_g723_mult(p_sign[i1], p_sign[i2+1]));
    ...
    for(i2=4/2; i2<SubFrLen2/2; i2+=STEP/2) {
        ...
        *ptr_ri2i3++ = i_g723_mult(*ptr1_ri2i3++, i_g723_mult(p_sign[i2], p_sign[i3]));
    }
            ...
            for (i2 = 4; i2 < SubFrLen2; i2 += STEP) {
                ...
                if ( ps2 > thres) {
                    ...
                    for (i3 = 6; i3 < SubFrLen2; i3 += STEP) {
                        ...
                        alp3 = g723_L_mac(alp2, *ptr_ri3i3++, 1);
                        alp3 = g723_L_mac(alp3, *ptr_ri0i3++, 2);
                        alp3 = g723_L_mac(alp3, *ptr_ri1i3++, 2);
                        alp3 = g723_L_mac(alp3, *ptr_ri2i3++, 2);
                        alp  = g723_extract_l(L_g723_shr(alp3, 5));

                        ps3c = g723_mult(ps3, ps3);
                        if( L_g723_mult(ps3c, alpha) > L_g723_mult(psc, alp) ) {
                            psc = ps3c;
                            alpha = alp;
                            ip0 = i0;
                            ip1 = i1;
...
end_search:
    ...
    *sign = 0;
    if(i0 > 0) *sign = g723_add(*sign, 1);
    if(i1 > 0) *sign = g723_add(*sign, 2);
    if(i2 > 0) *sign = g723_add(*sign, 4);
    if(i3 > 0) *sign = g723_add(*sign, 8);
