        ...
        if(curAcf[0] == 0) {
            ...
            for ( i = 1 ; i <= LpcOrder ; i ++ ) {
                ...
                for ( j = i ; j < LpcFrame ; j ++ ) {
                    Acc0 = L_g723_mult( Vect[j], Vect[j-i] ) ;
                    Acc0 = L_g723_shr( Acc0, (Word16) 1 ) ;
                    Acc1 = L_g723_add( Acc1, Acc0 ) ;
                }
 ...
 */
            ...
            for ( j = 0 ; j < LpcOrder ; j ++ )
                Acc0 = g723_L_msu( Acc0, PerLpc[j], CodStat.WghtFirDl[j] ) ;
...
*PrevErr, PWDEF Pw )
{
  ...
  */
        ...
        Acc0 = g723_L_mac( Acc0, Pw.Gain, Temp[PitchMax-(int)Pw.Indx+i] ) ;
        Dpnt[i] = round_ ( Acc0 ) ;
    }
}
...
*PrevErr )
{
  ...
  */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723_L_mac( Acc0, QntLpc[j], CodStat.RingFirDl[j] ) ;
        Acc1 = L_g723_shl( Acc0, (Word16) 2 ) ;
  ...
  */
        ...
        /* FIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723_L_msu( Acc0, PerLpc[j], CodStat.RingFirDl[j] ) ;
        ...
        /* IIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723_L_mac( Acc0, PerLpc[LpcOrder+j], CodStat.RingIirDl[j] ) ;
        Acc0 = L_g723_shl( Acc0, (Word16) 2 ) ;
...
Word32  Spf( Word16 *Tv, Word16 *Lpc )
{
  ...
  */
        ...
        /* FIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723_L_msu( Acc0, FirCoef[j], DecStat.PostFirDl[j] ) ;
        ...
        /* IIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723_L_mac( Acc0, IirCoef[j], DecStat.PostIirDl[j] ) ;
