                                      ...
                                      Word16 *nRandom, LINEDEF *Line)
{
  ...
  */
    ...
    for(iblk=0; iblk<SubFrames/2; iblk++) {
        ...
        *ptr1++ = g723_add( (Word16) SubFrLen, (Word16) (temp & 0x0001) );
        for(i=0; i<NbPulsBlk; i++) {
            *ptr_TabSign++= g723_shl(g723_sub((temp & (Word16)0x0002), 1), 14);
            temp = g723_shr(temp, 1);
        }
    }
    ...
    for(i_subfr=0; i_subfr<SubFrames; i_subfr++) {
        ...
        for(i=0; i<Nb_puls[i_subfr]; i++) {
            ...
            *ptr_TabPos++ = g723_add(g723_shl(tmp[(int)j],1), offset[i_subfr]);
  ...
  */
    ...
    for(iblk=0; iblk<SubFrames/2; iblk++) {
        ...
        L_acc = 0L;
        for(i=0; i<NbPulsBlk; i++) {
            L_acc = g723_L_mac(L_acc, tempExc[(int)ptr_TabPos[i]], ptr_TabSign[i]);
        }
 ...
 */
        ...
        for(i=0; i<NbPulsBlk; i++) {
            j = *ptr_TabPos++;
            curExc[(int)j] = g723_add(curExc[(int)j], g723_mult(temp,
                                                (*ptr_TabSign++)) );
        }
...
Word16 Qua_SidGain(Word16 *Ener, Word16 *shEner, Word16 nq)
{
  ...
  */
    ...
    if(L_y <= 0L) {
        ...
        if(L_y > L_acc) temp = g723_add(g723_shl(iseg,4), j);
        else temp = g723_add(g723_shl(iseg,4), j2);
    }
    else {
        j2    = g723_sub(j, 1);
        temp  = g723_add(base[iseg], g723_shl(j2, iseg_p1));
